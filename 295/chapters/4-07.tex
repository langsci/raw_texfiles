...
\tabref{tab:primary.TAME}  summarizes the primary TAME categories in Japhug. Labels such as `Imperfective' or `Egophoric Present' are conventional names for Japhug-specific morphological verbal categories, and therefore capitalized in this chapter and in the whole grammar following the convention proposed by \citet[674]{haspelmath10categories} and others. The use of these terms makes no implication that they have any crosslinguistic validity or that the Japhug categories are amenable to direct comparison with TAME categories in other languages.

Apart from the Factual Non-Past, all primary TAME categories take one orientation preverb. Some categories, such as the Sensory, are marked by the same preverb on all verbs. Other categories select a preverb belonging to one of the four series (A, B, C or D, see \tabref{tab:TAME.preverb} below and \tabref{tab:orientation.preverbs}, §\ref{sec:kamnyu.preverbs}), whose orientation is lexically specified by the verb (§\ref{sec:lexicalized.orientation}).
...
Three cases of systematic redundancy between stem alternation and orientation preverbs are observed. First, stem III alternation is redundant as a TAME marker (but not as a transitivity marker, §\ref{sec:transitivity.morphology}) with B type preverbs in the Imperfective, the Sensory and the Egophoric. Second, in 3\flobv{} configurations, stem II and C type preverbs redundantly mark Aorist (§\ref{sec:stem2}). Third, stem III is redundant with the contrast between A and C type preverbs to distinguish between Imperative and Aorist forms in the case of transitive verbs; taking \japhug{ndza}{eat} as an example of an alternating transitive verb, its \textsc{2sg}$\to$3 Imperative \forme{tɤ-ndze} `eat it!' (stem III, A preverb) differs from its \textsc{3sg}\flobv{} Aorist \forme{ta-ndza} `s/he ate it' (base stem, C preverb) by both the preverbs and the stems.
...
\textsc{interj} \textsc{qu}-\textsc{prs}-2-be.good \textsc{qu}-\textsc{sens}-2-be.fine \\
...
\midrule
1$\to$\textsc{2sg}& \forme{tu-ta-ndza} \\
\textsc{2sg}$\to$\textsc{1sg}& \forme{tu-kɯ-ndza-a} \\
\midrule
...
\textsc{ipfv}-look \textsc{lnk} everyday \textsc{emph} pigeon three \textsc{ipfv}:\textsc{down}-come-\textsc{pl} \textsc{lnk} heaven \textsc{loc} \textsc{ipfv}:\textsc{down}-\textsc{auto}-come.out-\textsc{pl} \textsc{lnk} \textsc{lnk}  \textsc{3sg}.\textsc{poss}-ground \textsc{aor}:\textsc{down}-reach-\textsc{pl} \textsc{lnk} a.little \textsc{ipfv}-\textsc{refl}-shake-\textsc{pl} \textsc{lnk} \textsc{lnk} pigeon \textsc{3sg}.\textsc{poss}-skin \textsc{dem} \textsc{ipfv}-peel-\textsc{pl} \textsc{lnk} apart \textsc{ipfv}:\textsc{west}-put-\textsc{pl} \textsc{lnk} \textsc{3sg}.\textsc{poss}-inside girl \textsc{sbj}:\textsc{pcp}-\textsc{emph}\redp{}be.beautiful \textsc{emph} three always \textsc{ipfv}:\textsc{west}-\textsc{auto}-come.out \textsc{lnk} \textsc{ipfv}-collect.\textit{Potentilla\_anserina}-\textsc{pl} \textsc{lnk} \textsc{lnk} evening \textsc{loc} again \textsc{filler} \textit{Potentilla\_anserina} \textsc{dem} \textsc{ipfv}:\textsc{up}-\textsc{auto}-take-\textsc{pl} \textsc{3pl} pigeon \textsc{3sg}.\textsc{poss}-skin \textsc{3sg}.\textsc{poss}-in \textsc{dem} \textsc{ipfv}-\textsc{auto}-wear-\textsc{pl} \textsc{lnk} \textsc{lnk}  \textsc{ipfv}-\textsc{refl}-shake-\textsc{pl} \textsc{lnk} \textsc{lnk} one \textsc{3sg}.\textsc{poss}-after one \textsc{emph} heaven \textsc{dem}:\textsc{loc} \textsc{ipfv}:\textsc{up}-disappear-\textsc{pl} always \textbf{\textsc{ifr}.\textsc{ipfv}-be}  \\
\glt `He saw that everyday, three pigeons would come down from the heavens, and as they reached the ground, they would shake themselves, shed the pigeon skins and put them aside, and three beautiful girls would come out from [the skins], and they would collect \textit{Potentilla anserina}, and in the evening, they would take the \textit{Potentilla anserina}, wrap themselves in their pigeon skin, shake themselves and disappear into the heavens one after the other.' (07-deluge)
\japhdoi{0003886\#S62}
\end{exe}
...
\textsc{interj} \textsc{1sg}.\textsc{poss}-mother \textsc{prohib}-\textsc{imp}-2-\textsc{refl}-kill \textsc{lnk} \textsc{1sg} \textsc{ipfv}:\textsc{down}-\textsc{vert}-come-\textsc{1sg} be:\textsc{fact} \\
...
\textsc{1sg} \textsc{ipfv}-1$\to$2-teach \textsc{3sg}.\textsc{poss}-while \textsc{ipfv}-weave-\textsc{1sg} be:\textsc{fact} \\
...
\textsc{ipfv}-2$\to$1-help-\textsc{1sg} \textsc{qu}-be.possible:\textsc{fact} \\
...
hare \textsc{erg} \textsc{dem}:\textsc{pl} \textsc{sbj}:\textsc{pcp}-be.like trick \textsc{ipfv}-do[III] \textsc{ifr}.\textsc{ipfv}-be.usually.the.case \\
...
It is also found instead of the Imperative in the 2$\to$1 configurations (\ref{ex:kukWnAjoa}, \ref{ex:jukWtsWma}, \ref{ex:YWkWmbia.ma}, \ref{ex:YWkWsWBzJWra}), since the Imperative only allows 2$\to$3 forms (§\ref{sec:imp.morphology}).

\begin{exe}
	\ex \label{ex:kukWnAjoa}
	\gll ku-kɯ-nɤjo-a je \\
	\textsc{ipfv}-2$\to$1-wait-\textsc{1sg} \textsc{sfp}  \\
...
\textsc{ipfv}-2$\to$1-take.away-\textsc{1sg} \textsc{sfp} \textsc{1sg}.\textsc{poss}-grandmother \\
...
\textsc{2pl}.\textsc{poss}-daughter \textsc{ipfv}-2$\to$1-give-\textsc{1sg}-\textsc{pl} \textsc{lnk} \textsc{1sg}-alone \textsc{inf}-\textsc{vert}-go \textsc{neg}-can:\textsc{fact}-\textsc{1sg} \\
...
\gll ɯ-ɲɯ́-nɯkɯmaʁ-a nɤ ɲɯ-kɯ-sɯ-βzɟɯr-a \\
\textsc{qu}-\textsc{ipfv}-make.a.mistake-\textsc{1sg} \textsc{add} \textsc{ipfv}-2$\to$1-\textsc{caus}-correct-\textsc{1sg} \\
...
\textsc{3sg}.\textsc{poss}-companion \textsc{pl} \textsc{erg} \textsc{ipfv}-look-\textsc{pl} \textsc{lnk} \textsc{dem} stone \textsc{ifr}-\textsc{peg}-become \textsc{unexp}:\textsc{deg} really \textsc{emph} \textsc{ifr}-regret-\textsc{pl} \\
...
\textsc{lnk} sparrow \textsc{dem} bird \textsc{3pl}-among \textsc{sbj}:\textsc{pcp}-be.small \textsc{indef} \textsc{idph}(II):small.and.cute \textbf{be}:\textsc{fact} \textsc{lnk}  \textsc{3sg} \textbf{be.small}:\textsc{fact} but really \textsc{emph} \textbf{be.smart}:\textsc{fact}  \textsc{lnk}  \textsc{3sg}:\textsc{poss}-eye \textsc{3sg}:\textsc{poss}-border \textsc{dem}  \textsc{pl}  \textsc{sbj}:\textsc{pcp}-be.black \textsc{erg} \textsc{ipfv}-\textsc{inv}-surround \textsc{dem} \textsc{3sg}.\textsc{poss}-on \textsc{loc} a.little \textsc{sbj}:\textsc{pcp}-\textsc{emph}\redp{}be.small \textsc{sbj}:\textsc{pcp}-be.red \textsc{sbj}:\textsc{pcp}-be.like \textbf{exist}:\textsc{fact} \textsc{3sg}:\textsc{poss}-belly \textsc{dem}  \textsc{pl}  \textsc{3sg}:\textsc{poss}-throat \textsc{ipfv}-begin[III] \textsc{lnk}   \textsc{dem}  \textsc{pl}  \textsc{3sg}:\textsc{poss}-tail \textsc{neg}-\textsc{aor}-\textsc{auto}-come.out until \textsc{dem} \textbf{be.white}:\textsc{fact} \\
...
\begin{exe}
\ex \label{ex:GWsata}
\gll si-a ɲɤ-sɯso, tʰa ɣɯ́-sat-a ɲɤ-sɯso \\
...
\gll nɯna-j ɯ́-jɤɣ \\
rest:\textsc{fact}-\textsc{1pl} \textsc{qu}-be.allowed:\textsc{fact} \\
...
\glt `She is seventy, and her foot is already hurting, but even so she still does all sorts of work at home.' (14-siblings)
\japhdoi{0003508\#S48}
\end{exe}
...
\begin{exe}
	...
	\gll kɯre ku-ta-nɤjo \\
	\textsc{dem}.\textsc{prox}:\textsc{loc} \textsc{prs}-1$\to$2-wait \\
...
\begin{exe}
	...
	girl \textsc{dem} \textsc{erg} really \textsc{emph} \textsc{prs}-be.happy-\textsc{1pl}  king also \textsc{1sg}.\textsc{poss}-on really \textsc{prs}-be.kind servant \textsc{pl} also really \textsc{emph}   \textsc{prs}-be.good-\textsc{pl} \textsc{ifr}-say\\
	\glt `The girl said: `We are very happy, the king is very kind to me, the servants are very nice.'' (2002 qaCpa)
\end{exe}

The use of Egophoric with third person subjects is an instance of what has been termed `broad egophoric' \citep[89]{gawne17bodish}. Japhug appears to allow a wide range of third person referents to occur with egophoric. There are examples of Egophoric Present with third person subjects in which the personal involvement of the speaker is not immediately obvious. 

In (\ref{ex:Wrna.kumNAm}), the speaker is a palace guard chasing away a beggar who is asking for alms; the use of the egophoric here may be related to what \citet{simon21egophorique} calls `ego-authoritative', serving to focus on the privileged access to information of the speaker in comparison with the addressee (`you may not know it, but...').
 
\begin{exe}
	\ex \label{ex:Wrna.kumNAm}
	\gll  rɟɤlpu ɯ-tɕɯ ɯ-rna ku-mŋɤm \\ 
	king \textsc{3sg.poss}-son \textsc{3sg.poss}-ear \textsc{prs}-hurt \\
	\glt `The prince's ear is hurting.' \japhdoi{0003530\#S20}
\end{exe}

In (\ref{ex:pAjkhu.kuWnme}), the use of the Egophoric Present may be interpreted either as expressing privileged access to information, or as an indication that the speaker is directly concerned by the event, being among the inhabitants of the district.
...
However, \iai{Tshendzin} explains the form \forme{ku-nɯ-me} `there is not' here as (\ref{ex:tham.kWnA.me}) with the Factual Non-Past, thus only glossing on the tense value.
...
In (\ref{ex:shangban.kosWBzu}), the presence of egophoric may be due to the fact that (\ref{ex:shangban.kosWBzu}) is the daughter of the speaker, even though they were not living together at the time of utterance. 
...
\textsc{irr}-\textsc{pfv}-2-bring \textsc{lnk} \textsc{1sg} \textsc{comit} { } princess \textsc{dem} \textsc{erg} \textsc{dem} town \textsc{gen} \textsc{3sg}.\textsc{poss}-outside \textsc{dem}:\textsc{loc} \textsc{ipfv}-1$\to$2-wait \\
...
It occurs in sentences expressing a state starting from a point in the past until the present moment, as in (\ref{ex:since.kumbGoma}), which would be expressed with the Present Perfect in English.

\begin{exe}
\ex \label{ex:since.kumbGoma}
\gll jɯfɕɯr ɯ-qʰu tɕe ku-mbɣom-a \\
yesterday \textsc{3sg}.\textsc{poss}-after \textsc{lnk} \textsc{prs}-be.busy-\textsc{1sg} \\
\glt `I have been busy since yesterday.' 
\end{exe} 
...
three-year \textsc{aor}-pass-\textsc{1pl} so.much \textsc{emph} be.happy:\textsc{fact}-\textsc{1pl} be.on.good.terms:\textsc{fact}-\textsc{1pl} \\
...
\textsc{aor}:\textsc{west}-2-be.near-\textsc{du} \textsc{lnk} \textsc{2sg}.\textsc{poss}-horse \textsc{3sg}.\textsc{poss}-tail \textsc{emph} \textit{Potentilla\_fruticosa} \textsc{irr}-\textsc{pfv}-2-attach \textsc{2sg} \textsc{indef}.\textsc{poss}-feather so.much \textsc{irr}-\textsc{pfv}-2-\textsc{refl}-\textsc{caus}-be.light \textsc{2sg}.\textsc{poss}-horse wind so.much \textsc{irr}-\textsc{pfv}-\textsc{refl}-\textsc{caus}-be.quick \\
...
{ } always tell:\textsc{fact} \textsc{lnk} completely \textsc{3sg}.\textsc{poss}-hand \textsc{irr}-\textsc{pfv}-2-take[III] \textsc{irr}-\textsc{pfv}-2-continue \textsc{emph} \textsc{irr}-\textsc{pfv}:\textsc{upstream}-2-bring \textsc{lnk} \textsc{3sg}.\textsc{poss}-word \textsc{irr}-\textsc{neg}-\textsc{pfv}-2-listen[III] \\
...
\textsc{interj} \textsc{emph} \textsc{sbj}:\textsc{pcp}-\textsc{emph}\redp{}be.many \textsc{erg} \textsc{irr}-\textsc{pfv}-attack.in.pack-\textsc{pl} \textsc{lnk} \textsc{irr}-\textsc{pfv}-gore-\textsc{pl} \textsc{lnk} \textsc{irr}-\textsc{pfv}-kill-\textsc{pl} \textsc{sfp} \textsc{sens}-think[III]-\textsc{1sg} \textsc{lnk} \\
...
The Imperative is built by combining type A preverbs with stem I or stem III depending on transitivity and number (§\ref{sec:stem.TAME}). It only has second person subject forms, which are however never marked with the second person \forme{tɯ-} prefix, unlike in other TAME categories (§\ref{sec:intr.23}). In addition, transitive verbs can only take a third person object; 2$\to$1 configurations cannot be expressed with the Imperative, and the Imperfective in hortative function is used instead (§\ref{sec:ipfv.hortative}).
...
\midrule
\textsc{2sg}($\to$3) & \forme{tɤ-\rouge{ndze}} & \forme{kɤ-ɤmdzɯ}& \forme{jɤ-ɕe} \\
\textsc{2du}($\to$3) & \forme{tɤ-ndza-ndʑi}& \forme{kɤ-ɤmdzɯ-ndʑi} & \forme{jɤ-ɕe-ndʑi} \\
\textsc{2pl}($\to$3) & \forme{tɤ-ndza-nɯ} & \forme{kɤ-ɤmdzɯ-nɯ}& \forme{jɤ-ɕe-nɯ} \\
\lspbottomrule
\end{tabular}
\end{table}
...
\gll nɤ-kɤ-tʰu ɯ-ɣɤ́ʑu nɤ, [tɤ-tʰe] jɤɣ \\
...
\textsc{tral}-\textsc{imp}-bring[III] be.needed:\textsc{fact} \textsc{lnk} \textsc{tral}-\textsc{inf}-bring \textsc{cond}\redp{}\textsc{neg}-\textsc{aor}-2-can be:\textsc{fact} \textsc{lnk} \textsc{2sg}.\textsc{poss}-root \textsc{2sg}.\textsc{poss}-life throw:\textsc{fact}-\textsc{1pl} \\ 
...
\textsc{dem}.\textsc{prox} path \textsc{dem}.\textsc{prox} \textsc{prohib}-\textsc{imp}-2-throw \textsc{emph} \textsc{imp}-go \\
...
\textsc{dem}:\textsc{loc} \textsc{prohib}-\textsc{imp}-2-put[III] \textsc{lnk} \textsc{indef}.\textsc{poss}-child \textsc{pl} \textsc{3pl}.\textsc{poss}-place be.\textsc{aff}:\textsc{fact} \\
\glt `Don't put her there, it is the place for the children.' (2003kAndzwsqhaj2)
\end{exe}


Unlike the Imperative (§\ref{sec:imp.morphology}), the prohibitive has no constraints on person. It can occurs in 2$\to$1 configurations (\ref{ex:matAkWndzaa}), but also with a first person intransitive (\ref{ex:Za.matAnWnatCi}) or transitive subject (\ref{ex:khramba.matABzea}) and also in very rare cases in the third person (\ref{ex:manAtCaR.ra}).

\begin{exe}
\ex  \label{ex:matAkWndzaa}
\gll ma-tɤ-kɯ-ndza-a tɕetʰa nɤ-χpi pjɯ-fɕat-a \\
\textsc{prohib}-\textsc{imp}-2$\to$1-eat-\textsc{1sg} later \textsc{2sg}.\textsc{poss}-story \textsc{ipfv}-tell-\textsc{1sg} \\
...
soon \textsc{prohib}-\textsc{imp}-rest-\textsc{1du} \textsc{lnk} \\
...
\textsc{dem} lie  \textsc{prohib}-\textsc{imp}-make[III]-\textsc{1sg} be.needed:\textsc{fact} \textsc{sfp} \\
...
\textsc{lnk} \textsc{genr}.\textsc{poss}-tongue \textsc{3sg}.\textsc{poss}-on \textsc{prohib}-\textsc{imp}-stain be.needed:\textsc{fact} \textsc{lnk} \textsc{genr}.\textsc{poss}-tongue \textsc{ipfv}-make.numb \textsc{emph} \textsc{lnk} \textsc{ipfv}-swell be.\textsc{aff}:\textsc{fact} \\
...
The preverb is optional in second person prohibitive forms. For instance with \japhug{ti}{say}, \forme{ma-tɯ-ti} (\textsc{prohib}-2-say) and \forme{ma-tɤ-tɯ-ti} (\textsc{prohib}-\textsc{imp}-2-say) both occur in free variation.
...
\textsc{prohib}-\textsc{imp}-2-be.polite \textsc{imp}-\textsc{refl}-\textsc{caus}-be.full \textsc{sfp} \\
...
\textsc{ipfv}:\textsc{east}-go-\textsc{1sg} \textsc{sfp} \textsc{prohib}-\textsc{imp}-go-\textsc{1sg} \textsc{sfp} \\
...
place \textsc{sbj}:\textsc{pcp}-be.far \textsc{loc} \textsc{ipfv}:\textsc{down}-\textsc{inv}-release \textsc{prohib}-\textsc{imp}-\textsc{inv}-kill be.needed:\textsc{fact} \\
...
\textsc{indef}.\textsc{poss}-meat also \textsc{aor}-\textsc{obj}:\textsc{pcp}-cook \textsc{dem}:\textsc{pl} soon \textsc{prohib}-\textsc{imp}-\textsc{inv}-take.care \textsc{lnk} \textsc{3sg}.\textsc{poss}-top \textsc{loc} \textsc{sbj}:\textsc{pcp}-be.white \textsc{ipfv}-put[III] \\
...
\textsc{dem} \textsc{comp} \textsc{2sg} \textsc{erg} \textsc{prohib}-\textsc{imp}-2$\to$1-\textsc{caus}-be.intelligent-\textsc{1sg} \textsc{erg} \textsc{pst}.\textsc{ifr}-be.better \\
...
\ex \label{ex:kunWZru.Ci.kWma}
 ...
 \glt `The cultivated [variety of \textit{Angelica} sp.] is better (than the wild one), they say, I don't know, maybe it is better.' (17-ndZWnW)
\japhdoi{0003524\#S33}
 \end{exe}
...
In addition, like the Inferential, the Aorist takes the 1/2\textsc{sg}$\to$3 Past transitive suffix \forme{-t} (§\ref{sec:suffixes}, §\ref{sec:other.TAME}). Complete paradigms of transitive and intransitive verbs in the Aorist are presented in §\ref{sec:polypersonal}, and need not be repeated here.
...
\ex \label{ex:aor.pear}
 ...
 \textsc{indef}.\textsc{poss}-child \textsc{sbj}:\textsc{pcp}-<\textsc{auto}>play several exist:\textsc{sens}-\textsc{pl} \textsc{aor}-come[II]-\textsc{pl} \textsc{lnk} \textsc{dem}:\textsc{pl} \textsc{erg}  \textsc{3sg}.\textsc{poss}-apple \textsc{pl} \textsc{inf}-collect \textsc{aor}:3\flobv{}-help-\textsc{pl} \textsc{lnk} \textsc{aor}:3\flobv{}-\textsc{caus}-stand-\textsc{pl} \\
...
\ex \label{ex:pWtWnWGenW.nWtCu}
 ...
 \textsc{2pl} \textsc{aor}:\textsc{down}-2-\textsc{vert}-come[II]-\textsc{pl} \textsc{dem}:\textsc{loc} stone.step \textsc{irr}-\textsc{pfv}:\textsc{down}-2-come.out-\textsc{pl} \textsc{lnk} \textsc{1sg} earth \textsc{idph}(II):soft \textsc{ipfv}:\textsc{down}-come.out-\textsc{1sg} be:\textsc{fact} \\
...
\ex \label{ex:pWtari.nWtCu}
 ...
 soon \textsc{aor}:\textsc{down}-2-go[II] \textsc{lnk} \textsc{dem}.\textsc{prox} \textsc{irr}-\textsc{pfv}:\textsc{east}-2-turn.towards \textsc{lnk} soon \textsc{ipfv}-\textsc{vert}-go-\textsc{pl} be.\textsc{aff}:\textsc{fact} \\
...
\ex \label{ex:tWtAtWtWt}
 ...
 \textsc{cond}\redp{}\textsc{aor}-2-say[II] add \textsc{lnk} \textsc{ipfv}-1$\to$2-kill be:\textsc{fact} \\
...
the.aforementioned ring-\textsc{dim} \textsc{sbj}:\textsc{pcp}-be.like \textsc{aor}:3\flobv{}-give \textsc{dem} \textsc{ifr}:\textsc{east}-turn.towards \\
...
The contrast between upper A-type (\forme{tɤ\trt}, \forme{lɤ\trt}, \forme{kɤ\trt}, \forme{jɤ-}) and D-type preverbs (\forme{to\trt}, \forme{lo\trt}, \forme{ko\trt}, \forme{jo-}) is neutralized when followed by the inverse prefix (see \tabref{tab:A.D.inv}, §\ref{sec:kamnyu.preverbs}), so that transitive verbs without stem II alternation selecting the upper orientations have syncretism between Aorist and Inferential. For instance, the phonetic form \phonet{kóɣndo} is ambiguous between the Aorist 3$'$\fl{}3 \forme{kɤ́-wɣ-ndo} (\textsc{aor}-\textsc{inv}-take) and the Inferential 3$'$\fl{}3 \forme{kó-wɣ-ndo} (\textsc{ifr}-\textsc{inv}-take), both translatable as `someone/it/s/he grabbed him/her'.
...
\gll tɤjpa ka-lɤt \\
snow \textsc{aor}:3\flobv{}-release \\
...
\ex \label{ex:chArAGdWt.chAtsxWB}
 \gll ta-ʁi nɯ kɯ nɯŋa ɯ-ndʐi nɯnɯ \rouge{cʰɤ-rɤɣdɯt} qʰendɤre \rouge{cʰɤ-tʂɯβ}. tɕendɤre nɤki, ta-mar tɯ-tɯ-ɣɟaβ nɯ \rouge{to-ndo} qʰe, ɯ-ŋgɯ nɯtɕu \rouge{ko-ʑɣɤ-mpʰɯr}. \\
...
\tablevspace
Estimative&\forme{ɲɤ-nɤ-mpɕɤr} `s/he found &\forme{pjɤ-nɤ-mpɕɤr} `s/he was finding \\
...
Like the Aorist and the Perfective Inferential, the Past and Inferential Imperfective require the past suffix \forme{-t} in the 1/2$\to$3 forms of open syllable stem transitive verbs (§\ref{sec:suffixes}, §\ref{sec:other.TAME}). However, since only very few transitive verbs are compatible with these two categories, relevant examples such as (\ref{ex:pWnApeta}) are very rare (see also \ref{ex:apWNu.mWpWnWrputa}, §\ref{sec:pst.ifr.ipfv.apodosis}).

\begin{exe}
\ex \label{ex:pWnApeta}
\gll pɯ-nɤ-pe-t-a \\
\textsc{pst}.\textsc{ipfv}-\textsc{estim}-be.good-\textsc{pst}:\textsc{tr}-\textsc{1sg} \\
...
\is{irregularity}
The archaic \textsc{3sg} \forme{kʰɯ-ti} `s/he said' and \textsc{3pl} \forme{kʰɯ-ti-nɯ} `they said' forms with the isolated preverb \forme{kʰɯ\trt}, are attested in a few occurrences in stories told by \iai{Tshendzin}'s mother. They are glossed by \iai{Tshendzin} with the Inferential forms \textsc{3sg}$\to$3 \forme{to-ti} and \textsc{3pl}$\to$3 \forme{to-ti-nɯ}, respectively. 
...
\textsc{3sg}.\textsc{poss}-companion \textsc{erg} \textsc{sens}-\textsc{prog}-\textsc{inv}-\textsc{auto}-wait \\
...
The 1$\to$2 \forme{ta-} seems to be incompatible with the progressive. For instance, to express the meaning `I am waiting for you', only the simple Egophoric Present \forme{ku-ta-nɤjo} (\textsc{prs}-1$\to$2-wait) is possible, the surface form \ipa{kutaznɤjo} can only be analyzed as a causative Imperfective \forme{ku-ta-z-nɤjo} (\textsc{ipfv}-1$\to$2-\textsc{caus}-wait) `I will have you wait for him', not as an Egophoric Progressive $\dagger$\forme{ku-ta-ɤz-nɤjo}.

Other person configurations, including inverse mixed scenarios 3$\to$1/2 (§\ref{sec:indexation.mixed}) and local scenario 2$\to$1 (§\ref{sec:indexation.local}) can be elicited with the Progressive, as in (\ref{ex:YWtAwGznWkhramba}) and (\ref{ex:YWkAsWzgroRa}), though no such examples have yet been found in the corpus.

\begin{exe}
\ex \label{ex:YWkAsWzgroRa}
\gll ɲɯ-kɯ-ɤsɯ-zgroʁ-a \\
\textsc{sens}-2$\to$1-\textsc{prog}-attach-\textsc{1sg} \\
...
\gll ma tɤŋe me nɯtɕu tɕe, zdɯm lu-ɣi ŋu tɕe, nɯ ɯ-ŋgɯ nɯra, tɯ-mɯ kɯ-xtɕɯ\redp{}xtɕi ʑo asɯ-lɤt tɕe tɕe nɯnɯ βɣɤrtsʰi wuma ʑo dɤn. \\
...
\gll woja nɯ ɕɯŋgɯ kɯmaʁ smɤn, nɯnɯ kɯ-fse <gaipian> pɯ-tu nɯnɯ pɯ-asɯ-ndza-a. tɕe nɯ tʰɯ-arɕo koʁmɯz nɤ, nɤʑo kɤ-tɯ-sɯ-ɣɯt nɯ tu-ndze-a. nɤʑo kɤ-tɯ-sɯ-ɣɯt nɯnɯ, <gaipian> ɲɯ-ŋu ɣe, tɕe nɯ ku-osɯ-ndza-a. \\
\textsc{interj} \textsc{dem} before other medicine \textsc{dem} \textsc{sbj}:\textsc{pcp}-be.like calcium.tablet \textsc{pst}.\textsc{ipfv}-exist \textsc{dem} \textsc{pst}.\textsc{ipfv}-\textsc{prog}-eat-\textsc{1sg} \textsc{lnk} \textsc{dem} \textsc{aor}-be.finished.up only.then \textsc{add} \textsc{2sg} \textsc{aor}:\textsc{east}-2-\textsc{caus}-bring \textsc{dem} \textsc{ipfv}-eat[III]-\textsc{1sg}  \textsc{2sg} \textsc{aor}:\textsc{east}-2-\textsc{caus}-bring \textsc{dem} calcium.tablet \textsc{sens}-be \textsc{sfp} \textsc{lnk} \textsc{dem} \textsc{prs}-\textsc{prog}-eat-\textsc{1sg} \\
...
\textsc{dem}.\textsc{prox} glass \textsc{sens}-be.\textsc{aff} \textsc{lnk} \textsc{dem} be.like:\textsc{fact}  thing \textsc{3sg}.\textsc{poss}-inside \textsc{ipfv}:\textsc{downstream}-2-put.in[III] \textsc{lnk} \textsc{appr}-\textsc{acaus}:break \textsc{sfp} \\
...
In addition, like the Aorist, it occurs with the 1/2\textsc{sg}$\to$3 \forme{-t} (§\ref{sec:other.TAME}) as in (\ref{ex:CWtWnAmat}), and selects Stem II (§\ref{sec:stem2}) in the verbs that have an alternation between stemI and II, such as the verb \japhug{ti}{say} in (\ref{ex:CWtWtWt.kW}) and \japhug{ɕe}{go} in (\ref{ex:CanWri.kW}).
...
\textsc{appr}-be.after-\textsc{1sg} \textsc{sfp} \textsc{emph} \textsc{ifr}-think-\textsc{pl} \textsc{lnk} unexpectedly corner four \textsc{emph} \textsc{ifr}-come-\textsc{pl} \\
...
\gll  ɕɯ-kɯ-mpɕa-a kɯ!   \\
\textsc{appr}-2$\to$1-scold-\textsc{1sg} \textsc{sfp} \\
...
night \textsc{loc} \textsc{ipfv}-cry \textsc{lnk} \textsc{interj} \textsc{ipfv}-say be:\textsc{fact} \textsc{lnk} \textsc{dem} \textit{Pucrasia\_macrolopha} be:\textsc{fact} \textit{Pucrasia\_macrolopha} be:\textsc{fact} \textsc{ipfv}-say-\textsc{pl} be:\textsc{fact} \textsc{lnk} \textsc{prob}-be:\textsc{fact} \textsc{lnk} \textsc{neg}-\textsc{genr}:know \textsc{lnk} night \textsc{loc} \textsc{neg}:\textsc{sens}-\textsc{prop}-see \textsc{lnk} \\
...
\ex \label{ex:WBrAkWznAmYoanW}
 ...
 \textsc{dem}:\textsc{medial} \textsc{2pl}.\textsc{poss}-staff \textsc{3sg}.\textsc{poss}-on \textsc{obj}:\textsc{pcp}-write \textsc{dem} \textsc{rh}.\textsc{q}-2$\to$1-\textsc{caus}-watch-\textsc{1sg}-\textsc{pl} \\
...
The negative existential verb  \japhug{me}{not exist} (§\ref{sec:existential.basic}) occurs in combination with the exceptive postposition \japhug{ma}{apart from} (§\ref{sec:exceptive}) and the indefinite \forme{ci} (§\ref{sec:tense.aspect.adverbs}) following a verb in the Non-Past Factual, in collocation with the verb \japhug{sɯso}{think}, to express a wrong expectation `believe/be confident that ... is true'\footnote{\iai{Tshendzin} translates it into Chinese as \ch{满以为是}{mǎnyǐwéishì}{be confident that it is...}. } (whereas in fact it is not the case), as in (\ref{ex:Nu.ci.ma.me}). 
...
\begin{exe}
	...
	\textsc{2pl} also 2-be.happy:\textsc{fact}-\textsc{pl} \textsc{lnk} \textsc{dem} \textsc{2pl}.\textsc{poss}-\textsc{sbj}:\textsc{pcp}-hurt \textsc{pl} \textsc{irr}-\textsc{ipfv}-not.exist prayer \textsc{sfp} \\
