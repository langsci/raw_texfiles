...
Slot -2 includes indexation prefixes, including second person \forme{tɯ-} (§\ref{sec:intr.23}), the \forme{ta-} 1$\to$2  and \forme{kɯ-} 2$\to$1 portmanteau prefixes (§\ref{sec:indexation.local}) and the generic S/O prefix \forme{kɯ-} (§\ref{sec:indexation.generic.tr}). The prefixal element \forme{k(ɯ)-} of the peg circumfix (§\ref{sec:peg.circumfix}) is also located in this slot.
...
\figref{fig:map.inner.prefixes} represents the possible ways in which most inner prefixes (excluding the autive, the anticausative, the denominal prefixes, the human antipassive, the distributed action derivation and the subject-oriented facilitative) can be combined with each other, excluding cases of lexicalized derivations.\footnote{Lexicalized derivations do not necessarily follow these ordering rules, see §\ref{sec:sig.caus.lexicalized} and §\ref{sec:autoben.lexicalized}.} Examples of attested complex forms can be found in the following sections:
...
\item Reflexive \forme{ʑɣɤ-} (\textsc{refl}): §\ref{sec:refl.caus}, §\ref{sec:refl.estimative}, §\ref{sec:reciprocal.other}
\item Estimative \forme{nɤ(ɣ)-} (\textsc{estim}): 
 §\ref{sec:estimative.other.construction}
\end{itemize}
...
\gll \bleu{ɯ-ɲɯ́}-ra nɤ, \bleu{a-mɤ-ɣɯ-nɯ-tɯ́-wɣ}-\rouge{nɯ}-mbi \\
...
Slot +1 only contains the \forme{-t} suffix found in  \textsc{1sg}$\to$3 and \textsc{2sg}$\to$3 of the Aorist, Inferential, Past Imperfective and Apprehensive (§\ref{sec:other.TAME}) of the transitive paradigm (§\ref{sec:indexation.mixed}). It can only be added on open syllable verb stems. It can only be followed by the \textsc{1sg} \forme{-a} indexation suffix. The form \forme{-t} is only found in the dialects of Ercha, Kamnyu and Mangi, all dialects east of Rqakyo (including all Xtokavian dialects) have \forme{-z} instead. A sound change \forme{*-s} $\to$ \forme{-t} seems to have occurred in Kamnyu, but its conditioning is unclear. This suffix is cognate to the suffix \forme{-z} in Zbu, which also occurs in \textsc{3sg}\flobv{} forms (\citealt[160--161]{gong18these}).
...
The only verb forms where slots +1, +2 and +3 are filled are the Aorist or Inferential \textsc{1sg}$\to$3\textsc{du/pl}, as in (\ref{ex:pWmtotandZi}). This is the verb form with the highest suffix-to-prefix ration (3/1).
...
\textsc{2sg}.\textsc{poss}-clothes \textsc{aor}-2-\textsc{auto}-sew \textsc{rh}.\textsc{q}-be \textsc{lnk} \\
...
The \forme{kɯ-} prefixal element competes with indexation prefixes in slot -2 (§\ref{sec:outer.prefixal.chain}). Since the 2$\to$1 portmanteau \forme{kɯ-} prefix has the same shape as the peg prefix, transitive verbs can present ambiguity between 2$\to$1 (§\ref{sec:indexation.local}) and 1$\to$3 (§\ref{sec:indexation.mixed}) configurations, as in (\ref{ex:WmAnWkWnWjmWta}), a verb forms with two possible interpretations.

\begin{exe}
\ex \label{ex:WmAnWkWnWjmWta}
\gll ɯmɤ-nɯ-\rouge{kɯ}-nɯ-jmɯt-a-\rouge{ci} \\
\textsc{prob}-\textsc{aor}-\textsc{peg}/2$\to$1-\textsc{auto}-forget-\textsc{1sg}-\textsc{peg} \\
\glt `It looks I forgot about it.' (\forme{kɯ-} = \textsc{peg})
\glt `It looks like you have forgotten me.' (\forme{kɯ-} = 2$\to$1)
\end{exe}

The second person \forme{tɯ-} and 1$\to$2 portmanteau \forme{ta-} are dominant in slot -2, and the peg prefixal element  \forme{kɯ-} disappears in 2$\to$3 (\ref{ex:WmAnWtWnWjmWtci}) and 1$\to$2 configurations (\ref{ex:WmApWtamto}).  
...
\textsc{prob}-\textsc{aor}-1$\to$2-see-\textsc{peg} \\
...
Bipartite verbs cannot be easily subjected to verbal derivations. The only example that could be elicited is the estimative verb (§\ref{sec:estimative}) \forme{nɤ-stu-mbat} `consider that X tries hard'. Only the quasi-compound form of the bipartite verb can be derived. The derivational prefix \forme{nɤ-} cannot be repeated on the $V_1$ and the $V_2$ (a form such as $\dagger$\forme{nɤ-stu=nɤ-stu} is incorrect).
...
people \textsc{neg}-be.innocent(1):\textsc{fact}-\textsc{pl} \textsc{neg}-be.innocent(2):\textsc{fact}-\textsc{pl} \textsc{sbj}:\textsc{pcp}-not.exist \textsc{dem} a.lot \textsc{ifr}-\textsc{caus}-kill \textsc{ifr}.\textsc{ipfv}-be.needed \textsc{unexp}:\textsc{deg} man \textsc{pl} \textsc{erg} really \textsc{emph} \textsc{ifr}-hate-\textsc{pl} \\
