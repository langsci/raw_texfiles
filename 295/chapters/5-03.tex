...
\textsc{lnk} \textsc{inf}:\textsc{stat}-\textsc{emph}\redp{}be.small \textsc{irr}-\textsc{neg}-\textsc{pfv}:\textsc{down}-\textsc{inv}-\textsc{auto}-drop \textsc{sens}-be.needed because \textsc{unexp}:\textsc{deg} \textsc{dem} \textsc{sens}-be.brittle \textsc{lnk} at.once \textsc{emph} \textsc{ipfv}-\textsc{acaus}:break \textsc{sens}-be.\textsc{aff} \\
...
\ex \label{ex:kunWtaj.rgaj}
 ...
 \textsc{dem} \textsc{ipfv}-\textsc{auto}-put-\textsc{1pl} really \textsc{emph} like:\textsc{fact}-\textsc{1pl} \\
...
\ex \label{ex:tukWnAjoRjoRa.rgaa}
 ...
 \textsc{2sg} \textsc{erg} \textsc{ipfv}-2$\to$1-flatter-\textsc{1sg} \textsc{dem}:\textsc{pl}  like:\textsc{fact}-\textsc{1sg}\\
...
For instance, the complement clause in (\ref{ex:thWnWmblWmnW.je}) contains the imperative/hortative particle \forme{je} (§\ref{sec:fsp.imp}) as well as an interjection, and in (\ref{ex:mAnAtsa.loB.toti}) we find the utterance mitigation particle \forme{loβ} (§\ref{sec:fsp.attitude}).   
...
\textsc{lnk} \textsc{2sg}.\textsc{poss}-father \textsc{erg} \textsc{2sg} {come.back:\textsc{fact}}  \textsc{inf}-think \textsc{erg} house \textsc{3sg}.\textsc{poss}-side \textsc{lnk} soldier three-circle \textsc{aor}:3\flobv{}-\textsc{caus}-release be.\textsc{aff}:\textsc{fact} \textsc{lnk}\\
...
\gll   ``\bleu{ɯʑo} χsɯ-sŋi χsɤ-rʑaʁ ma \rouge{mɯ-pɯ-rɤʑi-a}" ɲɯ-nɯ-sɯsɤm pjɤ-ŋu \\
 \textsc{3sg} three-day  three-night apart.from \textsc{neg}-\textsc{pst}.\textsc{ipfv}-stay-\textsc{1sg} \textsc{ipfv}-\textsc{auto}-think[III] \textsc{ifr}.\textsc{ipfv}-be \\
...
\gll  \rouge{a-tʂɯnlɤn} \bleu{ɲɯ-nɯ-fsɯɣ-a} ɯ-ɲɯ́-tɯ-sɯsɤm nɤ, nɯ tɤ-ste ti ɲɯ-ŋu \\
...
\glt `The [inchworm] moves as if it were measuring the ground span by span, it is not able to fly.' (26-qambalWla)
\japhdoi{0003680\#S72}
\end{exe}
...
bird \textsc{pl} \textsc{inf}:II-cry \textsc{pl} \textsc{aor}:3\flobv{}-start-\textsc{pl} \textsc{pst}.\textsc{ipfv}-have.time.to \\
...
\gll tɯ-mɯnmu ɲɤ-ʑa \\
\textsc{inf}:II-move \textsc{ifr}:\textsc{west}-start  \\
\\
...
Ambiguity between relative clauses on the one hand, and complement clauses on the other hand, is pervasive in the case of verbs of cognition and perception such as \japhug{mto}{see} (§\ref{sec:finite.relative.complement.ambiguity}, §\ref{sec:non-finite.relative.complement.ambiguity}), and concerns both finite and non-finite subordinate clauses. For instance, in (\ref{ex:WftaR.tAkAta}), \forme{tɤ-kɤ-ta} can be either analyzed as an object participle (§\ref{sec:object.participle}) or a velar infinitive (§\ref{sec:velar.infinitives.complement.clauses}). Disambiguization between the two analyses is possible when the verb is dynamic intransitive (§\ref{sec:non-finite.relative.complement.ambiguity}).

\begin{exe}
	\ex  \label{ex:WftaR.tAkAta}
	\gll nɯɕimɯma ʑo iɕqʰa [kɯm nɯtɕu ɯ-ftaʁ tɤ-kɤ-ta] nɯ pjɤ-mto \\
	immediately \textsc{emph} the.aforementioned door \textsc{dem}:\textsc{loc} \textsc{3sg}.\textsc{poss}-mark \textsc{pfv}-\textsc{inf}/\textsc{obj}:\textsc{pcp}-put \textsc{dem} \textsc{ifr}-see \\
...
\begin{exe}
	...
	\textsc{3sg}.\textsc{poss}-tomorrow-morning \textsc{lnk} \textsc{indef}.\textsc{poss}-child \textsc{pl} \textsc{erg} \textsc{dem} \textsc{3sg}.\textsc{poss}-\textsc{sbj}:\textsc{pcp}-search \textsc{ifr}-go-\textsc{pl} \textsc{sens}-be \textsc{lnk} \\
...
\begin{exe}
	...
	goat \textsc{3sg}.\textsc{poss}-\textsc{sbj}:\textsc{pcp}-lead \textsc{indef} \textsc{aor}-come[II] \textsc{lnk} \\
...
In addition, when a com\-ple\-ment-taking verb is in the subject participle form, it is possible for the complement either to be in the expected form (infinitive or finite) usually selected by that verb, or to be in the subject participle form. For instance, in example (\ref{ex:ndZikWsAndu}) the subject participle \forme{kɯ-cʰa} `the one who can' takes a complement whose verb is a subject participle with a possessive prefix coreferent with the object (\forme{ndʑi-kɯ-sɤndu}), instead of the expected \forme{kɤ-} infinitive (or finite clause).
...
\begin{exe}
	...
	\textsc{indef}.\textsc{poss}-boy \textsc{dem} \textsc{ipfv}-\textsc{apass}-plant \textsc{lnk} girl \textsc{dem} \textsc{erg} again \textsc{nmlz}:\textsc{action}-weave \textsc{ipfv}:\textsc{downstream}-make[III] \textsc{lnk} flower \textsc{pl} \textsc{ipfv}-sew \textsc{lnk} \textsc{ipfv}-\textsc{auto}-stay-\textsc{du} \textsc{ifr}.\textsc{ipfv}-be \\
...
\begin{exe}
	...
	\gll tɕendɤre tɯ-ɕkʰo pjɯ́-wɣ-nɯ-βzu qʰe, nɯ kɤ-ɣndʑɯr ɯ-spa nɯ pjɯ́-wɣ-ɕkʰo tɕe nɯ-rom kóʁmɯz cʰɯ́-wɣ-ɣndʑɯr ra tɕe, ɯnɯnɯra ɣɯ-tu-mɯrki tu-ndze ŋu. \\
...
\begin{exe}
	...
	song \textsc{pl} \textsc{ipfv}-make-\textsc{pl} \textsc{nmlz}:\textsc{action}-dance \textsc{pl} \textsc{ipfv}-make-\textsc{pl} \textsc{ifr}.\textsc{ipfv}-be.usually.the.case \textsc{sens}-be \\
...
With an intransitive verb, the simultaneous constructions occur with a dual or plural subject, and means that several individuals referred do an action together at the same moment, as \forme{tɯ-tɯ-nɯmdar} `jumping together' in (\ref{ex:tWtWnWmdar.pjABzundZi}). The auxiliary \forme{βzu} can even take the indefinite orientation prefixes \forme{jɤ-}/\forme{ja-} as in (\ref{ex:tWtWCe.jaBzundZi}) (from §\ref{sec:simultaneous.action.nominal}) when occurring with a motion verb.
...
\begin{exe}
	...
	bear \textsc{comit} basket \textsc{dem}:\textsc{pl}  \textsc{simult}-\textsc{nmlz}:\textsc{action}-cause.to.roll.down \textsc{emph} \textsc{ifr}:\textsc{down}-make \\
	\glt `He made the basket with the bear [in it] roll down (together).' (2011-13-qala) 
\end{exe}

The comitative (§\ref{sec:comitative}) can be used as in (\ref{ex:tWtWtsxaB.Zo.pjABzu}) to link two nouns referring to the patients that undergo the action together.\footnote{See also (\ref{ex:tWtWrqoR.koBzu}) in §\ref{sec:simultaneous.action.nominal}. }
...
\begin{exe}
	...
	\textsc{2pl} \textsc{simult}-\textsc{nmlz}:\textsc{action}-help \textsc{aor}-1$\to$2-make-\textsc{pl} \\
...
The verb \japhug{βzu}{make} is also combined with noun-verb compound action nominals (§\ref{sec:action.nominal.compounds}) as semi-object. Two such compound nominals have been identified. With \japhug{kʰramba}{lie} as first element, this construction has a simulative meaning `pretend to do $X$' (where $X$ stands for the second element of the compound, \citealt{jacques23simulative}) as in (\ref{ex:khrambaqur}) and (\ref{ex:khrambanAre}). 

\begin{exe}
	\ex \label{ex:khrambaqur}
	\gll [kʰramba-qur] ma-tɤ-kɯ-βzu-a \\
	lie-help \textsc{prohib}-\textsc{imp}-2$\to$1-make-\textsc{1sg} \\
...
\begin{exe}
	...
	\glt `I suppose that he will perform this task well.' (elicited)
\end{exe}

\begin{exe}
	\ex \label{ex:tuRnWa}
	\gll tu-ʁnɯ-a ɕti ma ɯʑo kɯ ta-tɯt ŋu maʁ mɤ-xsi \\
	\textsc{ipfv}-suspect-\textsc{1sg} be.\textsc{aff}:\textsc{fact} \textsc{lnk} \textsc{3sg} \textsc{erg} \textsc{aor}:3\flobv{}-say[II] be:\textsc{fact} not.be:\textsc{fact} \textsc{neg}-\textsc{genr}:know \\
...
\gll  ɯ-pʰɯ ɲɯ-wxti tɕe, nɯra tʰamtɕɤt ma-tɤ-tɯ-ɣɤ-wxti, aʑo nɯ tu-nɯ-χti-a [nɯra stʰɯci kɯ-wxti] ɯ-pʰɯ ma-tɤ-tɯ-βze, [kɯ-tʂaŋ] ci tɤ-βze \\
   \textsc{3sg}.\textsc{poss}-price \textsc{sens}-be.big \textsc{lnk} \textsc{dem}:\textsc{pl} all \textsc{prohib}-\textsc{imp}-2-\textsc{caus}-be.big \textsc{1sg} \textsc{dem} \textsc{ipfv}-\textsc{auto}-buy[III]-\textsc{1sg}   \textsc{dem}:\textsc{pl} as.much \textsc{sbj}:\textsc{pcp}-be.big \textsc{3sg}.\textsc{poss}-price \textsc{prohib}-\textsc{imp}-2-make[III] \textsc{sbj}:\textsc{pcp}-be.fair a.little \textsc{imp}-make[III] \\
...
cliff \textsc{3sg}.\textsc{poss}-down \textsc{dem}:\textsc{loc} { } \textsc{ipfv}-1$\to$2-\textsc{caus}-stay \textsc{lnk} \textsc{indef}.\textsc{poss}-water \textsc{dem} every.day \textsc{emph} up.there { } \textsc{2sg}.\textsc{poss}-on \textsc{dem}:\textsc{loc} \textsc{ipfv}:\textsc{down}-\textsc{sbj}:\textsc{pcp}-release \textsc{emph} \textsc{ipfv}-1$\to$2-\textsc{caus}-become be:\textsc{fact} \textsc{lnk} \\
...
\subsubsection{Periphrastic estimative} \label{sec:estimative.sWpa}
...
The combination of \forme{sɯ-pa} with participles is semantically similar to the estimative \forme{nɤ-} derivation (§\ref{sec:estimative}). For instance, the second clause in (\ref{ex:kWmWm.tusWpanW}) can be glossed as (\ref{ex:YWnAmWmnW}) with the estimative verb \japhug{nɤmɯm}{find tasty} (in this example, the ergative on \japhug{tʰoŋraʁ}{alcohol in bucket} is also a comparee marker, and does not mark transitive subject).

\begin{exe}
\ex \label{ex:YWnAmWmnW}
\gll tʰoŋraʁ nɯ kɯ ɲɯ-nɤ-mɯm-nɯ \\
bucket.alcohol \textsc{dem} \textsc{erg} \textsc{sens}-\textsc{estim}-be.tasty-\textsc{pl} \\
\glt `They consider alcohol in bucket to be tastier.' (elicited)
\end{exe}


Unlike the \forme{nɤ-} estimative derivation,\footnote{The \forme{nɯ-} denominal prefix can be sporadically used with a estimative meaning (§\ref{sec:denom.tr.nW}), for instance  \japhug{nɯʁgra}{treat as an enemy} from \japhug{ʁgra}{enemy}, but it synchronically different from the estimative \forme{nɤ\trt}, though these two prefixes are historically related (§\ref{sec:sW.caus.history}). } the \forme{sɯ-pa} periphrastic estimative can be used with nouns as parameters, instead of stative verbs. As shown in (\ref{ex:NgumdZWG.tutasWpa}), the direct object is the person/entity that the subject considers to have the property described by the parameter, while the parameter $Y$ (here the noun \japhug{ŋgumdʑɯɣ}{leader}) is a semi-object, not indexed on the verb.

\begin{exe}
\ex \label{ex:NgumdZWG.tutasWpa}
\gll nɤʑo ŋgumdʑɯɣ tu-ta-sɯ-pa ŋu \\
\textsc{2sg} leader \textsc{ipfv}-1$\to$2-\textsc{caus}-do be:\textsc{fact} \\
\glt `I consider you to be [my] leader.' (elicited)
\end{exe}

The participle clauses in the periphrastic estimative construction (such as in \ref{ex:kWBdi.tusWpanW} and \ref{ex:kWmWm.tusWpanW} above) can be analyzed as headless participial relative clauses (§\ref{sec:constr.participial.clause}). The literal meaning of this construction is thus `consider $X$ to be something that is $Y$.' 


\subsubsection{Simultaneity} \label{sec:bare.dental.inf.sWpa}

In addition to its periphrastic causative and estimative functions discussed in the previous section, the causative verb \japhug{sɯpa}{cause to do} also occurs in a construction expressing simultaneity between two actions. 
...
\textsc{3sg}.\textsc{poss}-\textsc{bare}.\textsc{inf}:say \textsc{comit} \textsc{1sg}.\textsc{poss}-\textsc{inf}:\textsc{II}-go  one  \textsc{aor}-\textsc{caus}-do-\textsc{pst}:\textsc{tr}-\textsc{1sg}   \\
...
Both sigmatic (§\ref{sec:sig.causative}) and velar  (§\ref{sec:velar.causative}) causative derivations from adjectival verbs can be used as com\-ple\-ment-taking verbs, expressing the manner in which the action described in the complement clause takes place.
...
Modality in Japhug is encoded by TAME categories (§\ref{sec:TAME.modal}), derivations such as the abilitative (§\ref{sec:abilitative}), various sentence final particles (§\ref{sec:fsp.epistemic}), and most importantly by com\-ple\-ment-taking modal auxiliaries and noun-verb collocations (§\ref{sec:nouns.cognition.complement}). This section provides a description of the use of some of the most common auxiliaries. 
...
\textsc{lnk} \textsc{ipfv}-give[III]-\textsc{1sg} be.allowed:\textsc{fact} \textsc{lnk} \textsc{2sg}.\textsc{poss}-together.with \textsc{ipfv}:\textsc{up}-2$\to$1-take.away-\textsc{1sg} be.needed:\textsc{fact} \\
...
Impersonal modal verbs are also found with Imperative (§\ref{sec:imp.compl}, §\ref{sec:prohib.function}) and Irrealis (§\ref{sec:irrealis.complement.clauses}) complements.
...
The verbs \japhug{kʰɯ}{be possible} and \japhug{jɤɣ}{be allowed} both encode epistemic modality, but the former is used in the case of possibility due to external circumstances, while the latter expresses permission by the speaker (\ref{ex:kAnWrAZi.jAG}),\footnote{See also (\ref{ex:YWkhama.jAG}) in §\ref{sec:intransitive.invariable}. } the addressee (in interrogative forms such as \forme{ɯ́-jɤɣ}, see \ref{ex:YWtWkhAm.WjAG}, §\ref{sec:interrogative.W.morpho}), or another referent (\ref{ex:YAtWGnW.mWjjAG}). 
...
\ex \label{ex:tukWqura.mAtWcha}
 ...
 \textsc{1sg}.\textsc{poss}-life \textsc{inf}-save also \textsc{neg}-2-can:\textsc{fact} \textsc{ipfv}-2$\to$1-help-\textsc{1sg} also \textsc{neg}-2-can:\textsc{fact}  \\
...
All examples in the corpus of \japhug{cʰa}{can} with a verb in \textsc{2sg}$\to$\textsc{1sg} form in the complement clause have \textsc{2sg} indexation as in (\ref{ex:tukWqura.mAtWcha}) above and (\ref{ex:YWkWCWGmua}) below.

\begin{exe}
\ex \label{ex:YWkWCWGmua}
\gll [aʑo ɲɯ-kɯ-ɕɯɣ-mu-a] mɤ-tɯ-cʰa  \\  
  \textsc{1sg} \textsc{ipfv}-2$\to$1-\textsc{caus}-be.afraid-\textsc{1sg} \textsc{neg}-2-can:\textsc{fact} \\
...
As a modal verb, it either takes infinitive complement clauses (\ref{ex:kAtshi.tasWsonW}, see also §\ref{sec:velar.infinitives.complement.clauses}), or finite clauses (\ref{ex:kuGWta.nWsWsota}, \ref{ex:YWtWsANo.tAtWsot}, see also §\ref{sec:finite.complement}), with subject coreference between the complement and the main clause.  
...
\textsc{1sg}.\textsc{poss}-song \textsc{ipfv}-2-listen \textsc{aor}-2-want-\textsc{pst}:\textsc{tr} \textsc{lnk} \textsc{irr}-\textsc{cisl}-\textsc{pfv}-2$\to$1-invite-\textsc{1sg} \textsc{lnk} \textsc{dem} be.\textsc{aff}:\textsc{fact} \\
...
\ex \label{ex:kAlAt.pjWtasWxCAt}
 ...
 thread \textsc{inf}-make a.little \textsc{ipfv}-1$\to$2-teach \\
...
\ex \label{ex:tukWsWrYoa}
 ...
 \textsc{1sg} \textsc{2sg}.\textsc{poss}-apple a.little \textsc{ipfv}-2$\to$1-\textsc{caus}-taste-\textsc{1sg} \\
...
\textsc{1sg} \textsc{pst}.\textsc{ipfv}-\textsc{emph}\redp{}be.small-\textsc{1sg} \textsc{emph} \textsc{loc} dysentery \textsc{aor}-meet-\textsc{1sg} \textsc{lnk} \textsc{dem} \textsc{inf}-hurt \textsc{aor}-experience-\textsc{pst}:\textsc{tr}-\textsc{1sg} \\
...
\ex \label{ex:kAsat.tAkWrANgat}
 ...
 \textsc{aor}-2-go[II] \textsc{lnk} {  } person \textsc{obj}:\textsc{pcp}-kill \textsc{aor}-\textsc{sbj}:\textsc{pcp}-be.about.to {  } two 2-see[III]:\textsc{fact} \textsc{lnk} \\
...
\ex \label{ex:akWCaB.tArANgat}
 ...
 \textsc{1sg} \textsc{1sg}.\textsc{poss}-\textsc{sbj}:\textsc{pcp}-catch.up \textsc{aor}-be.about.to-\textsc{pl} \textsc{lnk}  \textsc{2sg} \textsc{erg} sorcery triratna \textsc{irr}-\textsc{pfv}:\textsc{west}-2-turn.towards \\
...
\ex \label{ex:tWmaR.mANgrAl}
 ...
 \textsc{2sg} 2-not.be:\textsc{fact} at.all \textsc{neg}-be.usually.the.case:\textsc{fact} \textsc{lnk} \textsc{2sg} apart.form \textsc{1pl}.\textsc{poss}-companion \textsc{aor}-\textsc{sbj}:\textsc{pcp}-stay not.exist:\textsc{fact} \\
...
The subject of the infinitival clause is always coreferent with that of \forme{fse} or \forme{stu}, but its object is not usually indexed on the matrix verb. For instance, although \japhug{fsraŋ}{protect}, `save' and \japhug{bɯwa}{carry on the back} are transitive verbs that can index first or second person objects (see for instance \ref{ex:aZo.tutafsraN}, §\ref{sec:indexation.local}), in (\ref{ex:kAfsraN.tChi.tustea}) and (\ref{ex:kAbWwa.tChi.atAste})  the \textsc{2sg} object is not indexed on the matrix verb \forme{stu}, otherwise the forms \forme{tu-ta-stu} (\textsc{ipfv}-1$\to$2-do.like) and \forme{a-tɤ-tɯ́-wɣ-stu} (\textsc{irr}-\textsc{pfv}-2-\textsc{inv}-do.like) would be expected. These examples show that the complement clause can saturate the direct object of \forme{stu}.
...
Alternatively, indexing the object of the infinitival clause is also possible, as shown by (\ref{ex:kAfsraN.tChi.tutastu}), a sentence whose meaning is the same as that of (\ref{ex:kAfsraN.tChi.tustea}).

 \begin{exe}
\ex \label{ex:kAfsraN.tChi.tutastu}
\gll [nɤʑo kɤ-fsraŋ] tɕʰi tu-ta-stu? \\
\textsc{2sg} \textsc{inf}-protect what \textsc{ipfv}-1$\to$2-do.like[III]  \\
\glt `How [can] I save you?' (elicitation based on \ref{ex:kAfsraN.tChi.tustea})
\end{exe} 

Similative manner verbs are also often used in a variety of complementation strategies. First, they occur with a coordinated clause (generally in the Factual Non-Past) expressing the purpose of the action, as in §\ref{sec:coordination.comp.str}. Second, they can take as semi-object a totalitative relative (§\ref{sec:totalitative.relatives}) as object (§\ref{sec:relative.core.arg}) as in (\ref{ex:tWtastu.tostu2}) in the meaning `do everything in the same way as $X$' (where $X$ is the transitive subject of the relative). Third, \forme{fse} and \forme{stu} are found in serial verb constructions (§\ref{sec:svc.similative.verb}).
...
\textsc{1du}  \textsc{unexp}:\textsc{deg} \textsc{inf}-weave really \textsc{emph} be.expert:\textsc{fact}-\textsc{1du} \\
...
\gll [kɤ-nɯrdoʁ] ri mɤ-aɲaj ma kɯ-ndɯ\redp{}ndɯβ ɕti \\
...
Other nouns like \japhug{ɯ-sɯm}{mind} (§\ref{sec:nouns.cognition.complement}) on the other hand select as possessor the experiencer, which may not be \textsc{3sg}, and the complement clause is not syntactically the possessor. For instance, in (\ref{ex:tutWndAm.nAsWm.WCe}) the possessor of \forme{-sɯm} is \textsc{2sg}, not \textsc{3sg} as would be expected if the finite clause \forme{nɯ-βdaʁmu nɯ tu-tɯ-ndɤm} were the possessor of this noun.
...
\textsc{inf}-work \textsc{genr}.\textsc{poss}-wish at.all \textsc{neg}-come:\textsc{fact} \\
