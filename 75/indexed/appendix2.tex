...
In this Appendix, the number of tokens with the different phonetic characteristics are listed separately for CR and \isi{NCR girls}.  The minimum \isi{pitch} and the maximum \isi{pitch} are identical for several tokens because tokens with a \isi{pitch} more than two standard deviations from the mean were reassigned the \isi{pitch} at the cutoff point (i.e. 76.44Hz for tokens with especially low pitches and 353.90Hz for tokens with especially high pitches).  This was done in order to keep these tokens from biasing results.  The maximum values for the \isi{F2} target at the nucleus may appear higher than would be expected for tokens of /a/.  This is at least partly due to the fact that monophthongisation did not only occur through the lowering of \isi{F2} (and raising of \isi{F1}) in the offglide but also a shift in \isi{F2} in the nucleus.  
