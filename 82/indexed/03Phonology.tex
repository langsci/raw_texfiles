...
\largerpage[-1]
The consonant inventory is rather symmetrical, with the dental and \isi{retroflex} places of articulation displaying the most well"=developed system of manner contrasts. The ancient (\iliOIA) contrast between the three sibilants /s ʂ ɕ/ is preserved \citep[375]{cardonaluraghi2009}, with the present voicing contrast probably not evolving until quite recently, partly through lenition of voiced affricates, partly through foreign loans. 

While the \isi{plosive} and \isi{fricative} sets show a~contrast in voicing except for in the (marginal) labial and glottal places of articulation, voiced counterparts are missing in the small \isi{affricate} set. Mirroring that is a~general allophonic variation (see below) between voiced \isi{fricative} and \isi{affricate} pronunciations. The voiced palatal \isi{fricative} could equally well be treated as an~\isi{affricate}, as that is the more common allophone (especially in the A dialect), but to provide more symmetry to the system, I have chosen to include it among the voiced fricatives,\footnote{As pointed out by \citet[34]{zoller2005}, this particular asymmetry within the \isi{affricate}/\isi{fricative} sets is a~feature shared by a~number of languages of northern Pakistan, due to a ``lenition process which is more advanced in case of the voiced phonemes than in case of the voiceless phonemes''.} while in the common transcription it is represented as \textit{ǰ}.


The post"=velar (or uvular) place of articulation is represented by a~voiceless post"=velar or uvular \isi{plosive} /q/ alone. This marginal phoneme is only pronounced distinctly post"=velar by some educated speakers~-- and even then rather inconsistently~-- when occurring in loanwords of mainly Perso"=\iliArabic origin. In many speakers' pronunciation, however, it normally tends to approximate a~velar \isi{fricative} pronunciation [x], thus not contrasting with the phoneme /x/. The fricatives /z, x, ɣ/ are rather frequent in present"=day Palula, and many of the words probably have a~long history in the language, although they to a~large extent are found in vocabulary borrowed from languages in the immediate region, and to a~much lesser extent are found in inherited vocabulary affected by phonological processes. A labio"=dental [f] is sometimes heard in more recent loans, primarily from \iliUrdu and \iliEnglish, but with many speakers it alternates freely with or is entirely replaced by the native voiceless aspirated bilabial \isi{plosive} /pʰ/, hence /f/ is considered a~marginal phoneme. 


The voiced \isi{retroflex} \isi{fricative} /ʐ/ is also a~marginal phoneme, but it is included for comparative reasons; an~even more rarely occurring voiced \isi{retroflex} \isi{affricate} sound [ɖʐ] is tentatively analysed as an~allophone of the same phoneme. 


There is insufficient proof to regard a~velar \isi{nasal} [ŋ] as a~phoneme independent from /n/, as it only occurs before /k/ and /ɡ/, or as a~variant pronunciation of /nɡ/: [ŋɡ]$\sim$[ŋ]. 
...
The \isi{retroflex} consonants are in some descriptions called ``retracted'' \citep[16]{schmidtkohistani2008} or ``cerebrals'' \citep{morgenstierne1941}; it has been questioned whether these consonants in \iliHKIA languages are \isi{retroflex} in the same sense or to the same extent as in the main \iliNIA languages or in \iliDravidian languages. 
...
Intervocalically, the voiced plosives are often slightly fricativised. They frequently occur in clusters with /h/ (see \sectref{subsec:3-4-1}). Some of the voiceless aspirated plosives show lenition. An example is /pʰ/, which has an~alternating pronunciation [f]$\sim$[ɸ]$\sim$[pʰ], as in /pʰeːrimaː/ \textit{(pheerimaá)} `Ferima (\textsc{place name)}'. 
...
Affricates occur at three places of articulation, dental, \isi{retroflex} and palatal, but with respect to frequency the dentals are quite limited as compared to the other two. The explanation of the missing voicing contrast is partly explainable (as already commented on above) by the overlap or neutralisation between the \isi{affricate} and \isi{fricative} sets. 


There is also a~less consistent neutralisation of the contrast between aspirated dental (\ref{ex:3-1}) and \isi{retroflex} voiceless (\ref{ex:3-2}) affricates and their \isi{fricative} counterparts (but as far as I have been able to observe, never between aspirated voiceless palatal affricates and fricatives), apparently limited to certain lexical items. 
...
As already pointed out in connection with the affricates, there is a~close link between the \isi{affricate} set and the \isi{fricative} set, with some overlaps and neutralisations taking place between them. The voiced palatal \isi{fricative} is alternately realised as [ʑ] and [dʑ] (more often with an~\isi{affricate} pronunciation in A) and the voiced \isi{retroflex} as [ʐ] and [ɖʐ], whereas /z/ seems to occur consistently as [z] and never with an~\isi{affricate} pronunciation.
...
The voiced \isi{retroflex} \isi{fricative} is extremely rare, occurring only in a~few words. [ɖʐ] is most likely an~allophone of it, as in /ʐʰaɳʐiːr/ \textit{(ẓhaṇẓíir)} `chain': [ɖʐʰaɳɖʐ îːɾ]. 


There is a~strong affinity between /h/ and historical \isi{aspiration} (\sectref{subsec:3-4-1}), especially when occurring in clusters of voiced consonants and /h/, in which case it is mostly realised as [ɦ]. Historical occurrences of word medial /h/ through movement to \isi{syllable} onsets have most likely been reinterpreted as voiced \isi{aspiration}. In the present language, a single /h/ only rarely occurs intervocalically, and even then often with an~interpretational ambivalence: \textit{(rhayíi)} `footprints': /rhajiː/ or /rahiː/.
...
Phonetically there are at least five places of articulation attested for nasals: labial, dental, \isi{retroflex}, palatal and velar. The palatal \isi{nasal}, however, is analysed as deriving from a~sequence of /n/ + a~palatal consonant, as it never occurs in any other environment. The same analysis may be applied to the velar \isi{nasal}, where the sequence /n/ + a~velar stop usually is the likely source.


The case is a~little more complicated with the \isi{retroflex} \isi{nasal}, /ɳ/. Although it is clear in some cases that retroflexion is the result of assimilation with an~adjacent \isi{retroflex} consonant, this cannot always be concluded. Whereas a~\isi{retroflex} \isi{nasal} normally does not occur word initially (although the word /ɳiɳeː/ \textit{(ṇiṇeé)} `popcorn' can be cited as an~isolated counterexample), it contrasts intervocalically with dental /n/, compare /deːni/ and /deːɳi/ in (\ref{ex:3-3}), and word"=finally, /kan/ and /kaɳ/ in (\ref{ex:3-4}). 
...
The labial and the dental nasals are very frequent in the language, as these two segments are part of some of the most productive \isi{inflectional} forms in the language.
...
There is one \isi{lateral approximant}: /l/: [l]($\sim$[ɫ] B).
...
In the speech of my main A consultant, the front"=most \isi{approximant} /w/ is usually pronounced bilabially [β̞], but with many speakers this phoneme seems to alternate between a~bilabial and something close to a~labiodental [ʋ] pronunciation.
...
Environment as well as \isi{accent} (see \sectref{subsec:3-4-3}) further influences the exact pronunciation of each of the ten vowels. Under certain conditions, some neutralisations take place (see \sectref{subsec:3-2-2}). 
...
While there is a~consistent contrast between all the five vowel qualities as well as a~contrast in length, when the vowels are accented (see \sectref{subsec:3-4-3} for details on \isi{accent}), these contrasts are fewer and less convincing when the vowels are unaccented. The two main dialects also show some differences in this regard. 


Whereas B maintains a~word"=final /a/ vs. /e/ contrast~-- as is clearly evidenced in the morphological contrast between the general \isi{oblique} \isi{inflection} \textit{-a}, as in /ˈdiːɕa/ `in the village', and the \isi{genitive} singular \textit{-e}, as in /ˈdiːɕe/ `of the village', of many masculine nouns~-- there is no evidence of contrast between these two unaccented short vowels in A (where the unaccented \isi{genitive} ending instead is /iː/). In contrast, a~non"=variable masculine ending [u] in B, corresponds to two different (but grammatically identical) masculine endings [o] and [u] in A. Curiously, the realisations of these two variants are in complementary distribution, although there is no obvious phonological motivation behind it. When preceded by /aː/ (in the previous \isi{syllable}), the allophone is [u], while it is [o] when preceded by any other vowel, as can be seen in (\ref{ex:3-7}).
...
A typical \isi{syllable} in Palula is an~open \isi{syllable} consisting of a~consonant and a~vowel. This is the most common type when the \isi{syllable} is unaccented. Long, as well as short vowels (\ref{ex:3-9}) could constitute the nucleus of such a~\isi{syllable}: CV or CVV. There are monosyllabic words (/be/, /wiː/) which conform to this basic CV pattern, but most words are polysyllabic, consisting of two or more CV (or CVV) syllables (such as /ɡuː.li/ and /ku.ɳaː.koː.miː/).
...
The closed"=\isi{syllable} pattern, CV(V)C, is also a~very common \isi{syllable}, see examples in (\ref{ex:3-10}), and the most common one in accented syllables. This type occurs in monosyllabic as well as in polysyllabic words. Commonly, however, a~word is made up of a~combination of open and closed syllables. 
\newpage
...
There is a~maximum onset of three consonants in Palula words, as can be seen in (\ref{ex:3-12}). These are clusters of voiced consonants only, whose third member always is /h/ (phonetically realised as [ɦ]).  One type, whose middle member is /r/ preceded by a \isi{plosive}, go back to old (or secondarily formed) voiced aspirates followed by /r/. The aspirates have been reinterpreted as \isi{plosive} + /h/ clusters, and a subsequent realignment has taken place, whereby the more sonorant /h/ has changed to the position closest to the \isi{syllable} nucleus: /brhoː/ `brother' < */bhraː/ < \textit{bhr\'{\={a}}tr̥-}. Another type, whose middle member is one of the two approximants /j/ or /w/ (if we go with the analysis presented above, \sectref{subsec:3-2-3}) preceded by a \isi{plosive} or a \isi{nasal}, have arisen through de"=syllabification of a short unaccented closed vowel, in some cases subsequent to vowel metathesis, such as is the case (in A) with /ɡwheːɳiː/ `Pashtun' (< /ɡhweːɳiː/ < /ɡhueːɳiː/ < /uɡheːɳiː/, the latter which is still the form heard in the conservative B dialect). That other clusters similar to the last-mentioned type seem to be in the process of evolving is evidenced by co-existing forms: /ukʰaːndu/$\sim$/kʰwaːndu/ \textit{(ukháandu)} `is coming/going up \textsc{msg}', perhaps pointing to a~preference for Cw and Cj clusters vis-à-vis word initial V-syllables. 
...
Initial two"=consonant clusters, see (\ref{ex:3-13}), share many of the features already mentioned for three"=consonant onsets. The second member of such a~cluster is either /r/ (most of them of considerable age), an~\isi{approximant} (with a recent history, derived along the same lines as was presented above for /ɡwheːɳiː/), or /h/ (which is either historical voiced \isi{aspiration} reinterpreted as a cluster, or a new initial cluster arisen through laryngeal metathesis). Usually, but not exclusively, /r/ is preceded by a~\isi{plosive} (in the majority of cases a voiceless one). Voiceless aspirated plosives in clusters with /r/ are rare indeed, the verb /pʰrajaːnu/ `send' is the only example found so far in the data with a following /r/. Approximant may be preceded by \isi{plosive}, fricatives or nasals. Nearly any voiced consonant may precede /h/ in initial clusters (the only exceptions in the data being the "new" phoneme /ɣ/, and the distributionally limited consonants /ɳ/ and /ɽ/).
...
Two"=consonant clusters in coda position, see (\ref{ex:3-14}), seem to be \isi{subject} to a~much higher degree of variability, although the position also seems to be slightly more permissive than the onset. The more frequently occurring type observed at word boundaries consists of \isi{nasal} + \isi{plosive}/\isi{affricate}/\isi{fricative}, the other types being more marginal in occurrence. 
...
The final \isi{affricate} or \isi{fricative} is always articulated, even if sometimes only weakly, whereas the
\isi{nasal} (homorganic with the \isi{affricate} or \isi{fricative}) is sometimes~-- more with some speakers than
others and depending on word emphasis~-- phonetically absent but leaves a~trace of \isi{nasalisation} on
the preceding vowel. Even the \isi{nasal} + \isi{plosive} sequences are \isi{subject} to much variability. With some
speakers and dialects, one of the phonemes in the sequence is altogether absent, sometimes the \isi{nasal}
...
As for the realisation of the final /st/ and /ʂʈ/-clusters, there are differences between different speakers, and possibly between different dialects as well. My B informants tend to articulate both members of the cluster, even in final position, though the \isi{plosive} is somewhat softened, whereas my A informants seem to prefer to omit the \isi{plosive} altogether in final position, e.g [ɡɦ\v{o}ːʂ] `house', [n̪aːs] \textit{(náas)} `nose'. However, in all speech varieties both the \isi{fricative} and the \isi{plosive} are clearly present when occurring medially, i.e., in the corresponding inflected forms: [ɡɦoːʂʈá], [nastí].


A special case is the final cluster /ndr/ in /jaːndr/, see (\ref{ex:3-15}). This is the only three"=\isi{consonant cluster} at a~word boundary discovered so far, but its exact phonetic realisation is not entirely easy to define in terms of segments. With some speakers, the /n/ is clearly articulated, whereas the /dr/ part is only faintly present, and in other pronunciations the final /r/ gets a~schwa"=like sound attached to it, in practice making /dr/ the onset of an~additional \isi{syllable}. As with the above"=mentioned clusters occurring at the end of singular nouns, the same cluster stretching over a~\isi{syllable} boundary in an~inflected form of the same \isi{noun} is clearly and unambiguously articulated: /jaːn.dra/ `mills'.
...
Clusters occurring in \isi{syllable} onsets and \isi{syllable} codas intervocalically, see (\ref{ex:3-16}), are \isi{subject} to the same restrictions as the clusters at word boundaries described above, but a few of them, particularly those containing /h/, are extremely rare in that position. 
...
Apart from those, a number of other consonant clusters not permitted at word"=boundaries are found word"=internally. However, in all of those cases the clusters are analyzable as occurring across a \isi{syllable} boundary (and not seldom across a morpheme"=boundary as well). Only a few examples are shown in (\ref{ex:3-17}).
...
In the present description, seven consonants, all voiceless, with \isi{aspiration} as a~secondary articulation have been postulated: /pʰ/, /tʰ/, /ʈʰ/, /kʰ/, /ʦʰ/, /ʈʂʰ/, and /ʨʰ/. The two affricates /ʦʰ/ and /ʈʂʰ/ are treated with a~higher degree of tentativeness, as their contrastiveness vis-à-vis their non-aspirated counterparts is far less convincing than is the case with the other five aspirated segments, and would deserve further, more detailed, investigation.\footnote{At least as far as /ʈʂ/ is concerned, it seems, it is almost by ``default'' more or less clearly aspirated. A similar hesitation has been expressed on the /ʈʂ/-/ʈʂʰ/ contrast in \iliKhowar \citep[239]{endresenkristiansen1981}.} There are, however, certain characteristics (synchronic as well as diachronic) that the voiceless aspirated sounds share with clusters of voiced consonants and /h/ (as described in \ref{subsec:3-3-2}), suggesting an alternative (or perhaps complementary), unitary treatment of \isi{aspiration} as a~feature of a~word (or more correctly the lexical stem), as is already reflected in the consistent use of \textit{h} in the Palula common transcription.\footnote{It is also possible that a~more careful analysis will result in all instances of /h/ (here considered a~consonant in its own right when occurring alone in \isi{syllable} onset) being treated in the same way.} 
...
Most voiced consonants can occur in clusters with /h/, from plosives to approximants (as described in \ref{subsec:3-3-2}). This generous occurrence of \isi{aspiration} (in the wider sense) is not a~feature of most other languages in the immediate region, possibly with the exception of \iliIndusKohistani, where \iliOIA \isi{aspiration}, like in Palula, has been preserved and where \isi{aspiration} is concomitant with most of its consonants \citep[19--25]{hallberghallberg1999}. 


Whereas the phonetic realisation of the \isi{aspiration} with the voiceless consonants is more or less equal to a~secondary pronunciation of the voiceless segment, the ``breathiness'' affecting the pronunciation of the following vowel is somewhat mobile within the \isi{syllable}, and for some words even beyond the realm of that \isi{syllable}. Especially in B, there is a~fluctuation in some words, as seen in (\ref{ex:3-20}) and (\ref{ex:3-21}), between a~realisation as a single intervocalic /h/"=segment and the occurrence of /h/ as part of a word"=initial cluster, as described above, the intervocalic /h/ probably representing an~older pattern.\footnote{Possibly this is preserved to a~larger extent in the conservative variety spoken in Puri (mainly agreeing with Biori Palula), where the following \isi{nominative}"=\isi{oblique} alternation was recorded: /brhuː/ \textit{(bhruú)} `brother' vs. /brahu/ `brothers'. The unstable character of the phoneme /h/ and voiced \isi{aspiration} in \iliKalasha is also commented on by \citet[50]{morchheegaard1997}.}
...
The \isi{aspiration} feature (whether synchronically a cluster with /h/ or a voiceless aspirated consonant) has multiple diachronic sources: One is the \iliOIA \isi{aspiration}, thus preserved in Palula to an~extent not evidenced in the major \iliShina varieties,\footnote{On the contrary, \citet[30]{schmidtkohistani2008} state that the voiced aspirates in modern Kohistani \iliShina have come into the language through borrowing, while they have been lost in \iliOIA voiced aspirate cognates.} such as in /ɡhuːɽu/ \textit{(ɡhúuṛu)} `horse' {\textless} \iliOIA \textit{ɡhōṭa} \citep[4516]{turner1966}. Another is the above"=mentioned intervocalic /h/ advanced to a~more word"=initial position. Finally, an~old point of \isi{aspiration} can be advanced, such as in /ɡhoːʂʈ/ \textit{(ɡhoóṣṭ)} `house' {\textless} \iliOIA \textit{ɡōṣṭhá} \citep[4336]{turner1966}. Other words may have followed other routes, possibly further reinforced by the rising \isi{pitch} of a~second mora \isi{accent} (see \sectref{subsec:3-4-3} below). In any case, not all occurrences of \isi{aspiration}, even when concomitant with plosives, are justified or explained solely by etymology (as pointed out by \citealt[57]{morgenstierne1932}).


Further study is needed to determine to what extent \isi{aspiration} is preserved in two aspirated words that are compounded. There is an~indication that the primary stressed part of the compound keeps its \isi{aspiration}, while the other point of \isi{aspiration} is entirely or partly deaspirated (compare with the comment in footnote \ref{fnt:ftn32} on \textit{Grassman's Law}): /dhut/ \textit{(dhut)} `mouth' + /ɡhaːnu/ \textit{(ɡháanu)} `large' {\textgreater} /dutaɡhaːnu/ \textit{(dutaɡháanu)} `talkative'.


The interaction between \isi{accent} and \isi{aspiration} is another topic for further research. As breathiness (or a voiced cluster with /h/) quite often precedes a~second"=mora accented long vowel, the feature may have influenced or reinforced the rising \isi{pitch} of that \isi{accent}. However, it should be pointed out that breathiness also occurs before unaccented, as in /ʑhamatroː/ \textit{(ǰhamatroó)} `son"=in"=law', as well as first"=mora accented vowels, as in /oːɖhoːl/ \textit{(ooḍhóol)} `flood'; hence the two suprasegmental features are, in essence, independent.\footnote{This contrasts with the situation in \iliGawri and \iliTorwali (both languages belonging to the Kohistani group of \iliHKIA languages), where \isi{aspiration} is only contrastive with voiceless consonants, whereas breathiness is a~feature only optionally concomitant with low tone (\citealt[92]{baart1999b}; \citealt[36--37]{lunsford2001}). } 
...
As mentioned earlier, obligatory \isi{nasalisation} seems to be a~marginal suprasegmental feature associated with an~extremely limited number of lexemes, among which figure those in (\ref{ex:3-23}). It is unclear whether in all those lexemes there is a~historical loss of a~\isi{nasal} segment. In any case, there are too few instances to postulate any individual nasalised vowel phonemes, or even less a whole set of nasalised vowels. Neither are there in the data any examples of a lexical distinction being made solely by contrasting an~oral vowel with a~corresponding nasalised vowel. 
...
A phonological word in Palula may carry one, and only one, \isi{accent}. Phonetically the \isi{accent} is primarily realised as relatively higher \isi{pitch}, accompanied to some extent by higher amplitude. Generally speaking, in a~single word, \isi{accent} is associated with high \isi{pitch}, and the corresponding lack of \isi{accent} is associated with low (or default) \isi{pitch}. 

The \isi{accent}"=bearing unit is the vocalic mora, which means that \isi{accent} can be associated with a~short vowel (as in (\ref{ex:3-24})), or with the first mora of a~long vowel (as in (\ref{ex:3-25})), or with the second mora of a~long vowel (as in (\ref{ex:3-26})).
...
\label{ex:3-25}
\begin{tabularx}{\textwidth}{ l l l }
\multicolumn{3}{ l}{First"=mora \isi{accent} on long vowel:}\\
/ɖôːk/ &
\textit{(ḍóok)} &
`back'\\
/pûːtri/ &
...
\label{ex:3-26}
\begin{tabularx}{\textwidth}{ l l l }
\multicolumn{3}{ l}{Second"=mora \isi{accent} on long vowel:}\\
/bǎːt/ &
\textit{(baát)} &
`talk, word, issue'\\
/kuɳǎːk/ &
...
This means that the \isi{pitch accent}\footnote{Pitch \isi{accent} here is of the kind also observed in e.g., Lithuanian \citep[73--82]{szemerenyi1996}, a~mora \isi{accent} which is ``free within limits''.} (henceforth only ``\isi{accent}'') has one of the following phonetic manifestations:
\begin{enumerate}
\item[a)] high level or falling on a~short vowel [\'{}], represented in this work with an~acute \isi{accent} mark: \textit{á} (only in polysyllabic words, elsewhere without marking), 
\item[b)] rising on a~long vowel [\v{}], represented with an~acute \isi{accent} mark on the second vowel symbol: \textit{aá}, or 
\item[c)] falling on a~long vowel [\^{}], represented with an~acute \isi{accent} mark on the first vowel symbol: \textit{áa}.
\end{enumerate}
A word, as referred to here, is either a~bare stem, such as /děːs/ \textit{(deés)} `day', /paːɽú/ \textit{(paaṛú)} `magician', or a~stem with one or more suffixes added to it, such as /deːs-ôːm-iː/ \textit{(dees-óom"=ii)} `of the days', /ʑhôːn-um/ \textit{(ǰhóon"=um)} `I will know'. Even though some combinations of syllables and accents are more common than others and there are restrictions on \isi{accent} placement (see below), the location of the \isi{accent} within a~given word is not entirely predictable. Therefore, \isi{accent} in Palula must be defined as lexical. 


Difference in \isi{accent} placement is in a~few cases, as in (\ref{ex:3-27}), the only phonemic contrast between two lexical items.
...
\subsubsection*{The position of \isi{accent} on stems}

Accent falls either on the final or the penultimate \isi{syllable} in a~non"=verbal lexical stem, as seen
in (\ref{ex:3-28}) and (\ref{ex:3-29}); whereas in verbal stems, as in (\ref{ex:3-30}), the \isi{accent}
is always on the final \isi{syllable}. In terms of vocalic moras, only one of the three last moras, of any lexical stem in Palula, may be the locus of the \isi{pitch accent}.
...
There are two types of suffixes: those that carry their own \isi{accent}, which will be referred to as
\isi{accent}"=bearing suffixes, and those that are \isi{accent}"=neutral. When a~suffix of the first type is added
to a~stem, the \isi{accent} of the stem is eliminated, and the word \isi{accent} falls on the suffix. When
a~suffix of the second type is added to a~stem, the stem \isi{accent} may be retained. However, under
certain conditions, even in the latter case, the \isi{accent} (defined by the lexical stem) shifts from
...
The most productive \isi{accent}"=bearing suffixes (mainly verbal) are presented in \tabref{tab:3-5}. Some of them also have other allomorphs, such as \textit{-áan}: \textit{-éen}, \textit{-áand}, and \textit{-éend}.


Some of these suffixes may be added cumulatively, in which case the \isi{accent} is carried by the last \isi{accent}"=bearing suffix.


As with the \isi{accent}"=bearing suffixes, only the most productive \isi{accent}"=neutral suffixes are presented in \tabref{tab:3-6}, excluding possible allomorphs.
...
Some quantitative and qualitative morphophonemic alternations relating to the position of the \isi{accent} in the word will be dealt with in \sectref{subsec:3-5-1}.
...
A number of segmental modifications (primarily in the nominal paradigm) are related to \isi{accent}, or more precisely to the position of the \isi{accent} within the word; whether they are described as synchronically productive processes or the result of a~diachronic process, the latter also offering some explanations to the more regular dialectal variation observed. 



\tabref{tab:3-7} illustrates the main types of alternations: \textit{ḍheér--ḍheerí} has an~\isi{accent} alternating between the stem and an~\isi{accent}"=bearing suffix without any concomitant segmental modification (see \sectref{subsec:3-4-3} and \tabref{tab:3-5}), \textit{kuṇaák--kuṇaaká} has an~\isi{accent} shifting from the stem to an~\isi{accent}"=neutral suffix without any concomitant segmental modification (see \sectref{subsec:3-4-3} and \tabref{tab:3-6}), \textit{ṣiṣ--ṣiṣóom} has an~\isi{accent} shifting from the stem to an~affix"=neutral suffix with accompanying suffix modification (the unaccented allomorph being \textit{-am}), and \textit{haál--halá} has an~\isi{accent} shifting from the stem to an~affix"=neutral suffix with accompanying stem vowel modification. These four and some of their more salient subtypes will be further discussed and illustrated in the rest of this subsection.
...
\subsubsection*{Accent alternation (\isi{accent}"=bearing suffix) without modification}

A stem inflected with one of the \isi{accent}"=bearing suffixes presented in \tabref{tab:3-5} will without
exception carry its \isi{accent} on the \isi{accent}"=bearing unit of the suffix. Some further examples are given in \tabref{tab:3-8}.
...
\subsubsection*{Accent alternation (\isi{accent}"=neutral suffix) without modification}

In some cases, the \isi{accent} shifts to the suffix in spite of it being an~\isi{accent}"=neutral suffix. While
...
lexical stem \isi{accent} is on the last mora, as in \tabref{tab:3-9}.
...
While this is true for all polysyllabic stems, there are quite many monosyllabic stems for which \isi{accent} shift remains non"=predictable from a~purely synchronic perspective. On the one hand, there are those final"=mora accented stems that do not produce an~\isi{accent} shift (\tabref{tab:3-10}), and on the other hand, there are those non"=final mora accented stems that contrary to expectation do undergo an~\isi{accent} shift (the latter will be discussed further below).\footnote{An alternative, and possibly more economic, way of describing \isi{accent} patterns in Palula, would be to regard final"=mora \isi{accent} as the default \isi{accent} on any phonological word (whether inflected or not), and any from that deviating placement as specified in the lexicon.}
...
\isi{accent}"=neutral plural \isi{oblique} suffix \textit{-am} and the \isi{genitive} suffix \textit{-e} (in B only).

The alternation in the paradigms to some extent reflects general vowel shifts in the language
\textit{(a {\textgreater} aa, aa {\textgreater} oo/uu, ee {\textgreater} ii)} conditioned by \isi{accent},
and, in the B dialect, also by \isi{syllable structure}, to which I will have reason to return to in the
...
In the B dialect, \textit{\--am} regularly takes the form \textit{-áam} in closed syllables, and \textit{-úum} in open syllables (as, for example, when followed by a~\isi{genitive} suffix), as illustrated in \tabref{tab:3-12}. The unaccented \isi{genitive} suffix \textit{-e} in B corresponds regularly to an~accented form \textit{-í}.
...
In the nominal paradigm of the A dialect, some vowel modifications affecting the \isi{nominative} have been blocked by \isi{accent} shift in the inflected forms, resulting in alternations between \textit{aa} and \textit{a}, which is obvious with the nouns in \tabref{tab:3-13}. The lengthening of the accented vowels has produced a~second"=mora \isi{accent} in polysyllabic stems and vowels preceded by \isi{aspiration} (in the wider sense, see \ref{subsec:3-4-1}). 
...
\lsptoprule
Form with\newline stem \isi{accent} &
&
Form with\newline suffix \isi{accent} &
\\\midrule
\textit{páaš-um} &
`I will see' &
\textit{paš-áan-u} &
...
be discussed at length in the chapters on \isi{noun} and verb morphology, see \chapref{chap:4} and
...
in the nominal (\tabref{tab:3-16}) and verbal paradigms (\tabref{tab:3-17}) of anticipatory fronting (``\isi{umlaut}'') of \textit{aa} to \textit{ee} when preceding an~\textit{i} in the following \isi{syllable}. The anticipation normally does not occur if the \textit{a} is short. The nouns `glass' and `book' in \tabref{tab:3-16} show that this process has been productively extended even to relatively recent loans. 
...
\lsptoprule
Form without \isi{umlaut} &
&
Form with \isi{umlaut} &
\\\midrule
\textit{baát} &
`word, issue' &
\textit{beetí} &
...
In addition to that, \isi{umlaut} is applied to: a) verbal suffixes (or the final part of the \isi{perfective} stem) in anticipation of a~following adjectival feminine agreement suffix -\textit{i} (\tabref{tab:3-18}), b) the adjectival stem in anticipation of a~feminine agreement suffix (\tabref{tab:3-19}), and c) to derivations of various kinds in which the \isi{derivational} suffix contains \textit{i} (\tabref{tab:3-20}).
