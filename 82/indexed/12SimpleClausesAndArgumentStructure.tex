...
In this chapter, I present the different types of simple clauses found in Palula. In the first section (\sectref{sec:12-1}), nonverbal or nominal clauses are introduced, and the presence vs. absence of an~overt \isi{copula} is discussed. In the second section (\sectref{sec:12-2}), verbal clauses are introduced, with a~focus on subclassification of verbs based on \isi{argument structure} and transitivity. At the end of that section (\sectref{subsec:12-2-8}), a~tentative analysis of conjunct verbs and the syntactic role of the \isi{host element} in these constructions is offered. 


\section{Nonverbal predicates}
\label{sec:12-1}

Nonverbal predication corresponds to a few different forms or constructions. The particular type of construction and the copular form a~nominal predication typically occur with is primarily a~question of phrasal identity (whether it is another NP, an~\isi{adjective} \isi{phrase}, or a~\isi{locative} expression). The choice is also related to the semantic distinction between equation/identification, property and location, as well as to \isi{tense}.
...
As seen in examples (\ref{ex:12-1})--(\ref{ex:12-3}), \isi{predicate} \isi{noun} phrases typically occur without an overt \isi{copula} in the present \isi{tense}. \citet[337]{masica1991} mentions this feature in Eastern \iliNIA languages and in \iliSinhalese, where NP + NP as well as NP + AP clauses without a~\isi{copula} are normative, but he also remarks that it is not found in the more central \iliUrduHindi. \citet[118--122]{baart1999a}, likewise, describes left"=out copulas as possible in some \iliGawri clauses expressing identity, which also seems to be permitted, or even typical, in \iliKhowar (own observations). The \isi{copula} may be left out in Palula regardless of the \isi{phrase} being used for classification (ascriptively) or identification. 
...
The unmarked word order in such clauses is \textsc{sbj - prd}, as in the examples, but a~reverse word order, for focus and various other \isi{discourse} purposes, is also possible with this type of \isi{clause}, as in (\ref{ex:12-4}) and (\ref{ex:12-5}). There are also clauses with discontinuous predicates, as in (\ref{ex:12-6}).
...
The absence of an~overt \isi{copula} is, as noted above, normally confined to the present \isi{tense}, whereas clauses of the above type with past \isi{tense} reference, as in (\ref{ex:12-7}) and (\ref{ex:12-8}), appear with a~Past \isi{tense} \isi{copula} \textit{de}, reflecting a~widespread pattern for languages that allow so"=called \isi{copula} dropping (\citealt[34]{pustet2003}; \citealt[120]{givon2001a}) as well as other South Asian or neighbouring languages where \isi{copula}"=less sentences are allowed or normal (\citealt[339]{masica1991}; \citealt[121]{baart1999a}). However, it should be pointed out that an equally plausible analysis (although not adopted in this work) is to regard the form \textit{de}, which bears no formal resemblance to present \isi{tense} \textit{hin-}, as a past \isi{tense} marker without any actual copular meaning attached to it, regardless of its occurrence with nonverbal predicates (as described in this section) or its \isi{auxiliary} use in periphrastic tense-aspect formations (see \sectref{subsec:9-1-5}). 
...
However, the presence of a~\isi{copula} in the past \isi{tense} is not an~absolute, as past"=\isi{tense} \isi{copula}"=less sentences do occur, especially in list"=like discourses such as in (\ref{ex:12-9}).
...
Another (normally \isi{intransitive}) verb \textit{ɡir-} `turn' may rarely be used in a~similar way, carrying the approximate meaning `X turns into Y'.
...
A special type of subjectless \isi{copular clause} is illustrated in (\ref{ex:12-11})--(\ref{ex:12-12}), thus referring to a~temporal setting introduced earlier. Often such expressions occur at the beginning of a~story. The standard Past \isi{copula} \textit{de} is used.
...
While clauses with a~\isi{predicate} \isi{adjective} \isi{phrase} do occur without an~overt \isi{copula}, they seem to do so with less regularity than those with a~\isi{predicate} \isi{noun} \isi{phrase}. While it is left out in some fixed expressions, such as in greetings, a~present"=\isi{tense} form \textit{hin-} (agreeing in \isi{gender} and number with the NP) of the standard \isi{copula} is normally present in running \isi{discourse}, as seen in examples (\ref{ex:12-13}) and (\ref{ex:12-14}). 
...
The nearly obligatory \isi{copula} dropping with nominal predicates compared to the optional or variable pattern with adjectival predicates may imply that the present"=\isi{tense} \isi{copula} \textit{hin-} is not entirely devoid of meaning \citep[8, 31, 66]{pustet2003}. This is also supported by the \isi{existential} use of \textit{hin-} (\sectref{subsec:12-1-3}), but that is a~matter for further research.



Examples (\ref{ex:12-15}) and (\ref{ex:12-16}) show that with past"=\isi{tense} reference, \textit{de} is used, just as with the nominal predicates.
...
When simple"=present (\textit{hin-}) or past (\textit{de}) time reference is not sufficient, the relevant forms of the \isi{existential} (see \sectref{subsec:12-1-4}) verb \textit{háans-} `stay, remain, find oneself, be present' is used in place of the standard \isi{copula}. In (\ref{ex:12-17}), the longer duration of the happiness needs to be expressed with the Past \isi{Imperfective} of \textit{háans-}.
...
As with \isi{noun} \isi{phrase} predicates, \isi{adjective} \isi{phrase} predicates, as in example (\ref{ex:12-18}), also occur with \textit{bhe-} `become' as a~\isi{copula}.
...
Some predicative adverbial phrases can function just like predicative \isi{adjective} phrases. However, in such clauses the adverbial \isi{phrase}, `alone' in (\ref{ex:12-19}), in actual fact denotes a~property of the \isi{subject}.
...
In clauses with a~\isi{predicate} \isi{locative} \isi{phrase}, the \isi{copula} is always overt. With present"=time reference the standard \isi{copula} is used in its present"=\isi{tense} form \textit{hín-} (agreeing in \isi{gender} and number with the \isi{subject} NP). The semantics of such clauses, examples (\ref{ex:12-20})--(\ref{ex:12-21}), is to declare something or someone referred to by a~\isi{definite} NP as present (or absent) in a~certain location.
...
With past \isi{tense} reference the \isi{Pluperfect} of the verb \textit{háans-} `stay, remain, find oneself, be present', as in (\ref{ex:12-22}), is used.
...
Normally the \isi{subject} precedes the \isi{locative} \isi{predicate} \isi{phrase}, but this can be reversed, as in (\ref{ex:12-23}), when the focus is shifted, although the order \isi{locative}"=\isi{subject} is more typical of the closely related \isi{existential} construction (see \sectref{subsec:12-1-4}).
...
\spitzmarke{Existentials.} Although similar to copular clauses with a~\isi{locative} expression, the function of existentials is not to specify the location of a~known entity but to assert the existence of some previously unintroduced entity. 


Therefore, while the subjects of local copular clauses are \isi{definite}, the subjects of \isi{existential} clauses are \isi{indefinite}. Normally the word order is also the reverse of most local copular clauses, i.e., the \isi{locative} expression precedes the \isi{subject} \isi{noun} \isi{phrase}, and can probably not be considered a~\isi{predicate} in the same sense as in the copular \isi{locative} expressions. The verb used is either \textit{háans-} `stay, remain, find oneself, be present', especially with past time reference (\ref{ex:12-25}), or the present form (\ref{ex:12-24})~-- and occasionally the Past form~-- of the standard \isi{copula}.
...
In many \isi{existential} expressions, (\ref{ex:12-26})--(\ref{ex:12-28}), there is no overt location at all.
...
The (normally) \isi{intransitive} verb \textit{yhe-} `come' could in some sentences be analysed as having a~function similar to \textit{háans-} with the approximate meaning `X comes into existence', such as in (\ref{ex:12-29}).
...
Another similarly functioning verb is \textit{lhaíǰ-} `to be found', the \isi{passive} or \isi{valency}"=reduced form of \textit{lhay-} `find'.


\spitzmarke{Possessives.} There are two types of possessive constructions, both of which are similar in structure to the \isi{existential} expressions, where the possessor is expressed either as a~\isi{genitive} NP or postpositionally (with `from'). There is an~approximate but not absolute correspondence between the first construction, in (\ref{ex:12-30})--(\ref{ex:12-32}), and inalienable possession and the second construction, in (\ref{ex:12-33})--(\ref{ex:12-34}), and alienable possession.
...
\spitzmarke{Transitive copular clauses.} The verb \textit{the-} `do' can be used as the \isi{transitive} equivalent of the adjectival copular \textit{bhe-} `become', as shown in example (\ref{ex:12-35}).
...
Like most IA languages, there is a~strict distinction between \isi{intransitive} and \isi{transitive} verbs in Palula. Almost without exception, a~verb stem is either \isi{intransitive} or \isi{transitive} and cannot be ambivalent or polyvalent as far as transitivity is concerned. There is, on the other hand, fairly productive \isi{valency}"=changing morphology (as described in \sectref{sec:8-5}) by which a~stem can increase or decrease its \isi{valency}. A verb's transitivity is primarily diagnosed on the basis of two factors: 1) the absence/presence of \isi{ergative} morphology in the \isi{perfective} and 2) the aspectual shift between \isi{accusative} \isi{alignment} and \isi{ergative alignment} as far as \isi{verb agreement} is concerned (see \sectref{sec:11-1}). 


Within each of these two main categories, \isi{intransitive} verbs and \isi{transitive} verbs, there is another distinction made between simple \isi{intransitive}/\isi{transitive} verbs and \isi{intransitive}/\isi{transitive} verbs with an~\isi{indirect object}. I am here using a~very broad definition of an~\isi{indirect object}, as a~non"=\isi{nominative}/non"=\isi{ergative} argument, usually coded by a~\isi{postposition}. 


The four resulting argument structures (in \tabref{tab:12-2}) cover a~large majority of all verbs. Also a~few verbs displaying a~non"=standard pattern are discussed, as well as \isi{complement}"=taking verbs and the somewhat analytically challenging conjunct verbs.
...
The typical pattern for \isi{intransitive} verbs is to take a~single argument in the form of a~\isi{subject} \isi{noun} \isi{phrase} coded in the \isi{nominative}: \textbf{NP\textsc{sbj}} \textbf{V}. This pattern is exemplified in (\ref{ex:12-36}) and (\ref{ex:12-37}) with the verbs `die' and `break', respectively.
...
Many such verbs (\tabref{tab:12-poc}) are process verbs and the \isi{subject} \isi{noun} \isi{phrase} has a~semantic role that (following \citealt[125]{givon2001a}) could be described as a~patient"=of"=change, whether human, animate or inanimate.
...
Other verbs (\tabref{tab:12-pos}) can take as a~single argument an~agent \isi{subject} or a~patient"=of"=state \isi{subject}.
...
The typical pattern for \isi{transitive} verbs is to take as arguments: a) one \isi{subject} \isi{noun} \isi{phrase}, always coded in the \isi{nominative} in the \isi{imperfective}, while in the \isi{perfective} some NPs are non"=optionally coded in a~non"=\isi{nominative} case (\isi{oblique} or \isi{ergative}), and b) one \isi{direct object} \isi{noun} \isi{phrase} coded in the \isi{nominative} or the \isi{accusative} (again depending on the nature of the NP, but regardless of aspect, see \sectref{sec:11-2}). The unmarked word order is \isi{subject} preceding \isi{direct object}: \textbf{NP\textsc{sbj}} \textbf{NP\textsc{do}}~\textbf{V}.
...
The \isi{direct object} in many \isi{transitive} verbs of this type (\tabref{tab:12-poco}) has a~patient"=of"=change role corresponding rather closely with the role of the \isi{subject} of the corresponding \isi{intransitive} verb, for example \textit{mar-/mhaaré-} `die/kill', \textit{phooṭé-/phooṭá-} `break(\textsc{itr})/break(\textsc{tr})'.
...
Following \citet[127]{givon2001a}, such verbs could be further classified according to several types of change: a) creation (e.g., \textit{samá-}), b) destruction (e.g., \textit{kha-}), c) change in physical condition (e.g., \textit{phooṭá-}), d) change in surface conditions (e.g., \textit{pičhá-}), e) change in internal qualities (e.g., \textit{taapé-}). It is especially in the categories c. and e. that we find the most parallels between the \isi{direct object} and the \isi{subject} in typical \isi{intransitive} verbs.


Some less prototypically \isi{transitive} verbs (\tabref{tab:12-exps}) also conform to this pattern, e.g., those with experiencer"=subjects rather than agent"=subjects as in the verbs exemplified above.
...
The next major type of \isi{intransitive} verbs take, in addition to a~\isi{subject}, an~\isi{indirect object} (the term here used in a~broad sense, which will become even more obvious when discussing the parallel situation with \isi{transitive} verbs taking an~\isi{indirect object}). The \isi{indirect object} occurs mostly as a~\isi{postpositional phrase}, but occasionally as a~non"=\isi{nominative} \isi{noun} \isi{phrase}. In the typical cases the \isi{subject} is an~agent and the \isi{indirect object} a~\isi{locative}: \textbf{NP\textsc{sbj}} \textbf{PP/NP\textsc{io}} \textbf{V}.
...
For some of these verbs, especially those which are already spatially defined (e.g., \textit{whe-} and \textit{ukhé-}), the argument status of the \isi{indirect object} is somewhat doubtful, and they may alternatively be classified as simple \isi{intransitive} verbs with possible (but optionally occurring) \isi{locative} complements or elaborators \citep[304--305]{allerton2006}.
...
The contrast between simple \isi{intransitive} verbs and \isi{intransitive} verbs with an~\isi{indirect object} is more or less parallel to a~contrast between simple \isi{transitive} verbs and \isi{transitive} verbs with an~\isi{indirect object}. We recognise here quite a~few verbs as \isi{transitive} (or \isi{causative}) counterparts of the verbs presented in the section above and sometimes they also happen to be transparent morphological derivations of those (compare with \textit{lanɡ-/lanɡá-} `cross/take across'). 


To the pattern of simple \isi{transitive} verbs is added an~\isi{indirect object}, occurring mostly as a~\isi{postpositional phrase} but alternatively as a~non"=\isi{nominative} \isi{noun} \isi{phrase}. Again, the \isi{indirect object} is a~very broadly defined argument type that includes a~whole range of postpositional phrases and \isi{noun} phrases coded as non"=core participants. In the typical case the \isi{subject} is an~agent, the \isi{direct object} a~patient whose physical location is being changed, and the \isi{indirect object} a~\isi{locative}: \textbf{NP\textsc{sbj}} \textbf{PP/NP\textsc{io}} \textbf{NP\textsc{do}} \textbf{V}. 
...
Many verbs of this type (\tabref{tab:12-trmot}) code events of movement (concrete as well as abstract) caused by a~human agent, some of them inclusive of a~vertical specification (as some of the \isi{intransitive} verbs with an~\isi{indirect object}).
...
Other events (\tabref{tab:12-ben}), coded with verbs traditionally labelled bi- or \isi{ditransitive}, can easily be seen as extensions of these, where the \isi{indirect object} instead of being a~locational goal (often) is a~human ``goal'' with a~benefactive role. There is, not surprisingly, some overlap between these two, so that some of the verbs may be used with a~\isi{locative} non"=human goal as well as with a~benefactive human goal.
...
In fact, not even \textit{de-} `give', the verb most typically fitting the description \isi{ditransitive} in a~comparative perspective, is essentially or inherently \isi{ditransitive}. It is merely a~\isi{transitive} verb with an~\isi{indirect object} which has a~certain preference for the goal"=role, but which shows quite some variability in this respect (compare with \sectref{subsec:12-2-6}, \sectref{subsec:12-2-8}). \citet[43]{baart1999a} notes a~similar range of usage and variability in the \isi{valency}"=pattern of GIVE in \iliGawri.
...
In (\ref{ex:12-51}), the content, i.e., the `water', is coded as a~\isi{direct object} (agreeing with the \isi{finite} verb) and the vessel as a~\isi{locative} \isi{indirect object}, whereas in (\ref{ex:12-52}), the vessel is coded as a~\isi{direct object} (agreeing with the \isi{finite} verb) and the content in the \isi{genitive}.\footnote{It is equally possible to interpret the \isi{genitive} NP `of water' in this sentence as a~dependent of `pot', rather than an ``\isi{indirect object}'', thus making the sentence a~simple \isi{transitive} one; however, that is a~less likely interpretation in the sentence \textit{tíi baalṭí wíiyii puuríli} `She filled the bucket with water', where the \isi{direct object} instead precedes the \isi{genitive} NP.} 


Some verbs coding transactions, such as `ask for' in (\ref{ex:12-53}) and (\ref{ex:12-54}), seem to take a~\isi{direct object} and two potential indirect objects, although in actual usage it seems only one of these indirect objects at a~time appears as an~independent sentence argument. 
...
\spitzmarke{Dummy"=\isi{subject} or subjectless verbs.} Although Palula  has a tendency to avoid subjectless clauses, there are a~few instances, particularly in weather expressions (\tabref{tab:12-opt}), when that is possible.
...
In both of these cases, the verb is strictly limited to certain forms (those with masculine singular agreement) and each is, generally speaking, only used with one particular \isi{noun}, \textit{beedhré-} with \textit{aaɡhaá} and \textit{muč-} with \textit{báaṣ (}B \textit{baṣ)} `rain'. In the latter case, the \isi{noun} needs to be explicitly included in the A variety, whereas it is often left implicit in B. In any case, it is clear that even when the \isi{subject} is present, it does not add anything semantically to the \isi{clause} that is not already implicit in the meaning of the verb.


Some other weather expressions make use of the verb \textit{de-} `give' (\tabref{tab:12-wea}), which normally is a~\isi{transitive} verb with an~\isi{indirect object}, but appears here as formally \isi{intransitive} (rendering `fall' a~better gloss for \textit{de-}). However, it would also be possible to analyse these expressions as \isi{subject}"=deprived, resulting in the object formally filling the \isi{subject} position, thus agreeing with the verb in the \isi{imperfective} as well as in the \isi{perfective}. 
...
\spitzmarke{\isi{Intransitive} verbs with a~non"=\isi{nominative} experiencer.} A special case of \isi{intransitive} verbs with an~\isi{indirect object} (\sectref{subsec:12-2-4}) can be said to be used for a~number of constructions where a~sensation is coded as the \isi{nominative} \isi{subject}~-- agreeing with the verb~-- whereas the human or animate experiencer appears as a~non"=\isi{nominative} (often coded like or perceived as a~\isi{locative}) NP or PP. Some examples can be seen in \tabref{tab:12-nnom}. Formally the pattern is equal to \sectref{subsec:12-2-4}, but due to the special status of the non"=\isi{nominative} argument, this particular construction deserves special treatment here. Note that the verbs used in these constructions have more generic meaning and scope when used in other, non"=experiential, clauses: \textit{de-} `give', \textit{ṣač-} `adhere to, climb', \textit{yhe-} `come', \textit{ḍhak-} `touch'. Some of these are illustrated in examples (\ref{ex:12-58})--(\ref{ex:12-60}).
...
Another construction in which the experiencer also is ``demoted'' to non"=\isi{nominative} coding is found with e.g., some bodily sensations, as in (\ref{ex:12-62}), where the body part occurs, in this case `teeth', as the head of the \isi{subject} NP, and the experiencer appears as the \isi{genitive} modifier `my'.
...
\spitzmarke{\isi{Intransitive} verbs with a~postpositional object.} What can be viewed as a~metaphoric extension of the pattern of \isi{intransitive} verbs with an~\isi{indirect object} (see \sectref{subsec:12-2-4}) is the pattern of \isi{intransitive} verbs with a~dative or patient \isi{indirect object} (\tabref{tab:12-ipost}). Such indirect objects are coded with \isi{oblique} case and a~\isi{postposition} although they in a~semantic sense can be said to be primary objects.
...
\spitzmarke{Transitive verb with a~\isi{genitive} object.} One verb, the \isi{transitive} \textit{ǰe-} `hit, beat', is exceptional in that it codes the \isi{direct object}~-- or at least what seems to be the only explicit object~-- in the \isi{genitive} (instead of the usual \isi{nominative}/\isi{accusative}), as seen in (\ref{ex:12-64}).
...
That we are indeed dealing with a~formally (and not only logically) \isi{transitive} verb is confirmed by the \isi{ergative} pattern in the \isi{perfective} (i.e., non"=\isi{nominative} coding of the \isi{subject} and non"=\isi{subject} \isi{verbal agreement}). Here, however, another anomaly shows up, namely that the verb agrees, seemingly by default, in the feminine singular, even when the explicit object is masculine, as in (\ref{ex:12-65}), or plural.
...
\citet[43]{baart1999a}, who observes the same anomaly in \iliGawri with a~verb meaning `hit, beat' (where it happens to be one sense of the verb `give'), suggests by pointing to a~synonymous construction that this particular pattern is historically elliptic, leaving out a~feminine singular \isi{noun} meaning `stroke', which is itself the \isi{direct object} being modified by the \isi{genitive} \isi{noun} \isi{phrase}. 


The \isi{genitive} coding does not show up when \textit{ǰe-} is used as a~\isi{verbaliser} in a~conjunct (\sectref{subsec:12-2-8}).


\subsection{Verbs with clausal complements}
\label{subsec:12-2-7}

The syntax of verbs that take a~\isi{clause} as their \isi{complement} will be treated in detail elsewhere (\sectref{sec:13-5}). As a~part of this chapter, only some major subtypes, as they relate to \isi{argument structure}, are pointed out and exemplified.


\spitzmarke{Modality verbs.} With modality verbs (\tabref{tab:12-mod}), the \isi{subject} of the main \isi{clause} headed by a \isi{finite} \isi{modality verb} is coreferent with the \isi{subject} of the \isi{complement clause}. The latter is left unexpressed. The \isi{complement}"=\isi{clause} verb appears either as an~\isi{Infinitive} or a~\isi{Verbal Noun}. Quite a~few modality verbs are conjunct verbs (the ones in the right column).
...
\footnotetext{This \isi{modality verb}, which as a~non"=modal verb corresponds to the \isi{imperfective} stem
  \textit{ṣač-} of a~very polysemous verb with meanings such as `climb, argue, be lit', also
  implies aspectuality and occurs in my data only in the \isi{perfective}, hence the \isi{perfective} stem
  given.}
...
The scope of these verbs, the grammatical categories available to them, and the degree of their grammaticalisation varies extensively. For example, some of them are modality verbs only, as is the case of \textit{bha-}, whereas some other verbs in this group, such as \textit{ṣáat-}, display multiple membership. Although they sometimes occur in otherwise perfectly parallel constructions (as far as surface \isi{valency} is concerned), their inherent transitivity/intransitivity and argument coding is kept intact. That is, with the \isi{transitive} \isi{modality verb} \textit{bha-}, the \isi{direct object} of the \isi{complement clause} is coreferent with the \isi{direct object} of the main \isi{clause} (\ref{ex:12-68}), whereas with the \isi{intransitive} \isi{modality verb} \textit{ṣáat-,} the \isi{direct object} of the \isi{complement clause} stands outside the \isi{argument structure} of the \isi{modality verb} (\ref{ex:12-69}).
...
\spitzmarke{Manipulation verbs.} The (human) agent"=\isi{subject} of a main \isi{clause} headed by a~\isi{manipulation verb} manipulates the behaviour of a~manipulee. The latter is coreferential with the agent of the \isi{complement clause}. The \isi{complement}"=\isi{clause} verb often appears as a~\isi{Verbal Noun} coded as an~\isi{indirect object}, \textit{bainií the} in (\ref{ex:12-70}), of the main \isi{clause}. 
...
There is also a~special \isi{causative} construction, using \textit{ṣaawaá}, the converb of \textit{ṣaawá-} `dress (someone), turn on (e.g., a~light or a~fire)' preceded by the manipulee coded as an~\isi{indirect object}. The inflected \isi{finite} verb in this construction could be any verb derived causatively (see \sectref{subsec:8-5-1}). The \isi{causative} verb in (\ref{ex:12-71}) is derived from \textit{aṭé-} `bring'.
...
\spitzmarke{Perception, cognition and utterance (\isi{PCU}) verbs.} The \isi{subject} of a~\isi{PCU} verb (\tabref{tab:12-pcu}) perceives or cognises a~state or event, or utters a~proposition concerning a~state or event. The \isi{complement clause} corresponds to what is perceived, cognised or uttered. Some such verbs are only or primarily used with a~clausal \isi{complement}, whereas a~few of them display multiple membership.
...
There are reasons for not regarding the \isi{complement clause} as a~\isi{direct object}, one being the non"=verb"=final position of the \isi{complement clause}, the other being that the structure stays the same regardless of the ``inherent'' \isi{argument structure} of the \isi{PCU} verb being used. In the examples (\ref{ex:12-72}) and (\ref{ex:12-74}), \textit{mané-} `say' and \textit{khooǰá-} `ask' are both \isi{transitive} verbs with indirect objects (although explicit only in (\ref{ex:12-74})), whereas \textit{buǰ-} `understand' in (\ref{ex:12-73}) is \isi{intransitive}. Transitive \isi{PCU} verbs with a~\isi{complement} display masculine"=singular agreement in the \isi{perfective}, by default. This is similar to the ``mismatch'' between argument coding and the \isi{valency} patterns with the \isi{intransitive} vs. \isi{transitive} modality verbs above.
...
Normally the \isi{complement clause} is preceded by the \isi{complementiser} \textit{ki}, as in the examples above, but there are also other (and additional) strategies available, some of them with relevance only for individual \isi{PCU} verbs (see \sectref{subsec:13-5-1}). 


\subsection{Valency patterns of {conjunct verb} constructions}
\label{subsec:12-2-8}

Conjunct verbs (see \sectref{subsec:8-6-1}) are special in that the host (i.e., the element that combines with a~\isi{verbaliser} to form a~\isi{conjunct verb}) in some of them stands in no grammatical relation whatsoever to other parts of the sentence, whereas with others it functions as a~\isi{direct object} of the \isi{verbaliser} and as such controls agreement in the \isi{perfective}. Using various diagnostics, some scholars (\citealt[201]{verma1993}; \citealt[165]{mohanan1993}) have suggested that only some of these combinations are ``true'' conjunct verbs (particularly those of the former kind), while the rest are normal syntactic combinations of nouns and verbs. 



This, however, seems somewhat oversimplified and would possibly exclude (as pointed out by \citealt[160]{masica1993}) the more productive types in the language as well as obscure the mechanisms by which this pattern has come about in the first place. Therefore I prefer to regard all of them, at least preliminarily, as conjunct"=verb constructions, but on different levels of lexicalisation or grammaticalisation, and with a~possible subclassification into: a) conjuncts with a~host contributing to the \isi{argument structure} (what I refer to as ``non-incorporating'' below) and b) conjuncts with hosts playing no role in \isi{argument structure} and agreement patterns (referred to below as ``incorporating''). The latter is supported by a~distinction suggested by \citet[69--74]{jaeger2006} between light"=verb constructions (by him defined more narrowly than \citealt{butt2010}) and periphrastic constructions (discussed by \citeauthor{jaeger2006} particularly with reference to so"=called `do'-periphrasis), also acknowledging that the two constructions in fact form a~cross"=linguistic continuum and may share functional properties in individual languages. 



\subsubsection*{Conjunct verbs with \textit{the-} `do'}

The most productive means of forming conjunct verbs is with \textit{the-} `do', and we accordingly notice a~wide variety of constructions and possible argument structures for conjuncts formed this way. For a~large part of them, but far from all, the host is a~relatively recent loan from another language. All of these constructions are \isi{transitive}, which is obvious from the \isi{ergative alignment} in the \isi{perfective}. While \citet[201]{verma1993} suggests a~subclassification of conjuncts into three types~-- lexically complex, syntactically complex and a~purely analytical sequence (the latter only superficially looking like a~\isi{conjunct verb})~-- I see no reason to make a~distinction between the two latter categories. Instead I will distinguish between \textit{incorporating conjunct verbs} and \textit{non-incorporating conjunct verbs} only, basically following Haig (\citeyear{haig2002}) in his work on complex predicates in Kurdish.


\spitzmarke{Incorporating \textit{the}-conjuncts (without host"=\isi{verbaliser} agreement).} These conjuncts (examples listed in \tabref{tab:12-3}) are internally complex but externally unitary. While being phonologically two words (each carrying its own \isi{accent}), the complex is syntactically equal to simple \isi{transitive} verbs.
...
This also means that the \isi{direct object} in such constructions is an~argument of the complex as a~whole, and the complex can be analysed as a~single \isi{predicate}. It is still possible, however, for the host to be separated from the \isi{verbaliser} by, for instance, a~\isi{negation}. All conjunct \textit{the}-verbs with an~\isi{adjective} host, such as \textit{tanɡ the-} exemplified in (\ref{ex:12-75})~-- or at least what is an~\isi{adjective} in other uses or in a~donor language~-- belong to this particular type. What is more interesting, however, is that we also find complexes of this type formed with a~\isi{noun} as a~host (at least in a~historical"=etymological sense) that in no way participates as an~NP in the \isi{argument structure}. The conjunct \textit{káaṇ the-} in (\ref{ex:12-76}) is an~example. 
...
I suggest that a~\isi{host element}, which may be homonymous with another \isi{noun} in the language~-- such as \textit{káaṇ} `ear' in \textit{káaṇ the-} `listen'~-- with which it is historically related, has in fact ceased to constitute an~NP in this construction. And in the case of a~loan from another language~-- such as the Perso"=\iliArabic \isi{noun} \textit{široó} in \textit{široó the-} `start'~-- it may never even have been interpreted as an~NP from the very start (which is probably the case with the \iliEnglish origin element \textit{rekaáḍ} in \textit{rekaáḍ the-} `record' as well). In such a case it does not matter how much of a~\isi{noun} it is in the donor language. This process of lexical incorporation \citep[203]{verma1993} is simply a~convenient way of deriving new verbs, combining the semantics (or some aspects of it) of the \isi{host element} with the ``verbness'' of the \isi{verbaliser} with the resulting verb behaving, as far as \isi{argument structure} is concerned, in no way different from any other simple \isi{transitive} verb (compare with \citealt[286]{haspelmath1993} for Lezgian). In some cases it is not entirely unlikely that the complex as a~whole is a~calque from another language, without even going through a~process of lexical incorporation, as there are several close parallels to Palula conjuncts in \iliUrdu as well as in \iliPashto. The \isi{conjunct verb} of this type as a~whole is lexical in nature \citep[199]{verma1993}.



\spitzmarke{Non"=incorporating \textit{the}-conjuncts (with host"=\isi{verbaliser} agreement).} In the next type of \isi{conjunct verb}, the \isi{verbaliser} does agree with the nominal host (in the \isi{perfective}), thus, if holding on to the conjunct idea, meaning that the \isi{predicate} agrees with an~element internal to itself \citep[168]{mohanan1993}. Examples of conjuncts of this kind are given in \tabref{tab:12-4}.
...
One solution to the somewhat puzzling situation of the host being part of the \isi{argument structure} is to analyse the \isi{noun} in the complex as itself assuming the role of a~\isi{predicate} (\citealt[204--212]{verma1993}; \citealt[164--170]{mohanan1993}). This \isi{predicate} chooses a~suitable \isi{verbaliser} to team up with and contributes itself to the total number of arguments of the \isi{clause}, but at the same time it assigns roles and case marking to its ``own'' arguments. I hold, however, that it is sufficient and far easier to see the \isi{valency} pattern displayed (e.g., the host coded as a~\isi{direct object} and the occurrence of indirect objects, i.e., certain postpositional phrases) as a~property (or lexical specification) of each individual construction in its entirety. 
...
The complexes  considered ``purely analytical'' sequences by \citet[201]{verma1993} and not ``true'' conjuncts only differ from the ones already discussed in that they do not include a~\isi{postpositional phrase} in their \isi{valency} pattern. To treat them as different phenomena on the basis that they are logically \isi{intransitive}, i.e., not having a ``logical object'' in addition to the \isi{direct object}"=coded ``formal'' host, is not a~very convincing argument; they are in fact only ``logically \isi{intransitive}'' in the trivial sense of translation equivalence (as pointed out by \citealt[157]{masica1993}). These would in Palula be conjuncts such as \textit{kráam the-} `work', \textit{nimóos (}B \textit{nimáas) the-} `pray', exemplified in (\ref{ex:12-81}), and \textit{hiǰrát the-} `migrate'. 
...
Just as with the other non-incorporating conjuncts, the verb agrees with the nominal host in \isi{gender}/number (in the \isi{perfective}), the latter thus being part of its \isi{argument structure} in the form of a~\isi{direct object}. 


Looking at the conjunct \textit{hiǰrát the-} `migrate' in (\ref{ex:12-82}), we also notice that this indeed specifies a~particular \isi{valency} pattern, including a~source (realised by \textit{taayúu} `from there' in the example) in addition to the \isi{subject} and the \isi{direct object}. 
...
The distinction suggested by Verma becomes even more superficial when considering a~conjunct like \textit{bhootíi the-} `plough', which can code the non"=\isi{nominative} argument denoting `field' alternatively as a~\isi{locative} NP (\isi{oblique} case), as in (\ref{ex:12-84}), or a~\isi{postpositional phrase}, as in (\ref{ex:12-83}).
...
Another argument for treating all these instances of \textit{the}-conjuncts as a~single phenomenon is the means of passivisation/\isi{valency} reduction. While other \isi{transitive} nouns are amenable to morphological passivisation with the affix \textit{-íǰ} (see \sectref{subsec:8-5-2}), \textit{the}-conjuncts~-- regardless of subtype -- are passivised or \isi{valency}"=reduced by substituting \textit{the-} for \textit{bhe-} `become', the other main \isi{verbaliser} (see below), as shown in (\ref{ex:12-85})--(\ref{ex:12-87}).
...
The other productive means of forming conjunct verbs is with \textit{bhe-}, for which some examples are given in \tabref{tab:12-6}. Here we do not observe the same variety of construction types and argument structures as with the \textit{the}-conjuncts, the most obvious reason being that all complexes with \textit{bhe-} are \isi{intransitive} and thus bound to have fewer potential arguments. For the most part, but again not exclusively, the \isi{host element} is often an~element copied from another language. 


While \isi{verbaliser}"=host agreement seemed to be relatively common with \textit{the}-conjuncts, this is, as far as I have been able to observe, not found with \textit{bhe}-conjuncts outside of \isi{passive} constructions derived from \textit{the}-conjuncts with host agreement exemplified above in (\ref{ex:12-85})--(\ref{ex:12-87}). 
...
Although a~number of these conjuncts are made up of the same \isi{host element} as some of the \textit{the}-conjuncts, such as \textit{teér bhe-} in (\ref{ex:12-88}) and \textit{ǰamá bhe-} in (\ref{ex:12-89}) they are not simply derived from these, as the meaning of the corresponding \textit{bhe}-conjunct is not necessarily \isi{passive}, but rather they stand in the same kind of relationship to one another as any other \isi{transitive}/\isi{intransitive} pairs with or without a~common base segment.
...
The historical"=etymological identity of the \isi{host element} as far as part of speech is concerned is not unitary: a) \textit{teér} originates in \iliPashto where it is an~\isi{adjective} `passed, gone by, done, accomplished, spent (as time), over, lapsed, that has been', and even there is used in a~conjunct, in some tenses morphologically incorporated into the verb (\textit{teredal}), with a~meaning very similar to the Palula conjunct; b) \textit{ǰamá} is of Perso"=\iliArabic origin and is listed as a~\isi{noun} or verbal \isi{noun} in \iliUrdu, \iliPashto as well as in \iliPersian (the three most likely donor candidates) with an~approximate meaning `collection, assembly, congregation, whole'; c) \textit{raál} is used in Palula outside of this construction as an~\isi{adverb} meaning `high'. The crucial point is that it is not interpreted as an~NP, even when the origin happens to be nominal. 


As with many of the \textit{the}-conjuncts, some \textit{bhe}-conjuncts are also specified for an~\isi{argument structure} involving postpositionally coded participants, such as `Machoke' in (\ref{ex:12-90}), and `he' in (\ref{ex:12-91}). Some of these indirect objects would fit the description ``human dative''. 
...
\largerpage
\spitzmarke{Incorporating \textit{de}-conjuncts (without host"=\isi{verbaliser} agreement).} These conjuncts (examples given in \tabref{tab:12-7}) are internally complex but externally unitary, as we saw with the incorporating \textit{the}-conjuncts above. 
...
Often no exact meaning can be assigned to the \isi{host element}, and the basic meaning of \textit{de-} has been considerably bleached, with the result that the complex meaning seldom can be deduced or guessed from its individual components. The two conjuncts in \tabref{tab:12-1} are exemplified in (\ref{ex:12-92}) and (\ref{ex:12-93}).
...
These complexes are \isi{intransitive}, but other non"=\isi{nominative} arguments may appear, which also opens up the possibility for analysing e.g., \textit{ḍhoó/ḍhoowá de-} as taking a~non"=\isi{nominative} experiencer \isi{subject} (either explicitly or implicitly), see \sectref{subsec:12-2-6}. 


\spitzmarke{Non"=incorporating \textit{de}-conjuncts (with host"=\isi{verbaliser} agreement).} In other \textit{de}"=conjuncts (\tabref{tab:12-8}), the \isi{verbaliser} does agree with the nominal host (in the \isi{perfective}), and is therefore an~argument of the \isi{clause}. 
...
It is obvious from (\ref{ex:12-94}) and (\ref{ex:12-95}) that the \isi{host element} in this case is
perceived as an~NP, even when it does not appear synchronically as a~\isi{noun} in the language outside of
...
Most conjuncts of this type, such as \textit{daawát de-} `invite' in (\ref{ex:12-98}), can also take a~clausal \isi{complement}, thus function as \isi{PCU}"=verbs (see \sectref{subsec:12-2-7}).
...
\spitzmarke{Conjuncts with other verbs.} There are a~few other combinations of a~verb and a~\isi{host element} that may be considered conjuncts (\tabref{tab:12-9}), some of them already pointed out as participating in non"=standard \isi{valency} patterns (\sectref{subsec:12-2-6}). In particular the \isi{transitive} \textit{ǰe-} `hit' used as a~\isi{verbaliser} should be mentioned.
