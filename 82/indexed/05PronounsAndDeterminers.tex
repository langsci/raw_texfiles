...
\multicolumn{1}{l}{Accusative\footnote{\textit{máa=the} `to me' is special. As far as I know, it is only
 together with this particular \isi{postposition} that the vowel of the \isi{pronoun} is lengthened. What this
 shows is that the \isi{pronoun} forms a~phonological word with the \isi{postposition} and has gone through
...
That means that, whereas most nouns only distinguish between \isi{nominative} and \isi{oblique} case (beside the \isi{genitive} forms), and the agent of \isi{transitive} \isi{perfective} clauses is expressed in the \isi{oblique}, the plural personal pronouns have a~unique \isi{ergative} \isi{pronominal} form; compare the \isi{ergative} \textsc{1pl} \textit{asím} in example (\ref{ex:5-2}) with the \isi{nominative} \textit{be} and \isi{accusative} \textit{asaám} in example (\ref{ex:5-1}). 
...
The first- and second"=person singular pronouns on the other hand make fewer distinctions than most full nouns, with their two"=way case contrast. As in Kohistani \iliShina, the \isi{oblique} case has merged with the \isi{nominative} \citep[82]{schmidtkohistani2008}. The \isi{nominative} is thus used for \isi{intransitive} subjects, as the \textsc{1sg} \textit{ma} in example (\ref{ex:5-3}), all direct objects, as the \textsc{1sg} \textit{ma} in example (\ref{ex:5-5}), as well as for \isi{transitive} agents in the \isi{imperfective}, while the \isi{genitive} covers the possessor of a~possessive construction, as the \textsc{1sg} \textit{míi} in example (\ref{ex:5-3}), as well as the agent of \isi{perfective} clauses, as exemplified by \textsc{1sg} \textit{míi} in (\ref{ex:5-4}). 
...
However, in uses with \isi{nominative} \isi{pronominal} forms, as in (\ref{ex:5-9}), it takes on a~meaning similar to `I myself'. Note that this, in Palula, is a~function distinct from that of the reflexive pronouns (see \sectref{sec:5-4}).
...
A~reduplicated form of a~\isi{pronoun} and the suffix, as in (\ref{ex:5-10}), is used in approximately the same sense.
...
The particle \textit{eé} can also be used with  a \isi{demonstrative}, as in (\ref{ex:5-11}). 
...
As mentioned in the introduction to this chapter, the \isi{demonstrative} pronouns are part of a~larger system of pro"=forms (pro"=NPs, pro"=adverbs, pro"=adjectives, etc.), displaying a certain degree of symmetry (\tabref{tab:5-2}). Most (but not all) of those pro"=forms have similar"=looking sets consisting of a~proximal, a~\isi{distal}, a~\isi{remote} and an~\isi{indefinite}/\isi{interrogative} member (see \citealt[187--188]{haspelmath1993} for some strikingly similar sets in Lezgian).
...
Remote
&
Indefinite/{\allowbreak}\isi{interrogative}\\\midrule
Attributive &
\textit{anú, eenú} &
\textit{aṛó, eeṛó} &
\textit{eesó} &
...
\textit{eetáa} &
\textit{ɡóo} \\
Location II\footnote{It is only with a~following \isi{postposition} that these forms have a~locational interpretation. Used alone they (ergatively) code the \isi{subject} of a~\isi{perfective} \isi{transitive} \isi{clause}.} &
\textit{aníi=} &
\textit{eeṛíi=} &
\textit{eetíi=} &
\textit{kíi=} \\
...
Somewhat simplified, a~consonantal element (\textit{-n-, -ṛ-, -s-/-t-} or \textit{k-/ɡ-}) regularly indicates the \isi{deictic} function. Additional dimensions are relevant to certain subsets, such as case, number, \isi{gender} and emphasis for the adnominally and nominally used demonstratives, animacy for \isi{indefinite}/\isi{interrogative} pronouns, and spatial specification (primarily) for locational pro"=forms. For further discussion and examples, see \sectref{subsec:5-2-2}, \sectref{subsec:5-2-7}, \sectref{subsec:7-1-1}--\sectref{subsec:7-1-2}, \sectref{subsec:7-1-4}--\sectref{subsec:7-1-5}, \sectref{subsec:14-2-2}, \sectref{subsec:14-2-4} and \sectref{subsec:14-3-2}.

\subsection{Demonstratives and third person}
\label{subsec:5-2-2}


As already hinted at above, a~basic three"=way distance differentiation is used extensively with demonstratives and has at least to a~certain degree been extended to the realm of unstressed third"=person reference. I have chosen to regard all of these as demonstratives, of which many can and are indeed being used as third"=person pronouns, some more so than others. It is even typologically speaking problematic to make any clear"=cut distinction between personal pronouns and demonstratives, a~fact pointed out, among others, by \citet[206]{himmelmann1996} and \citet[123--124]{kibrik2011}, which is why it makes most sense to present them together. A distinction, however, that needs to be made within the category of demonstratives, is that between \isi{pronominal} (or pro"=NPs) and adnominal demonstratives. As pointed out by \citet[206]{himmelmann1996}, pronominally used demonstratives tend, from a~typological perspective, to occur with a~lower frequency and in fewer contexts than adnominally used demonstratives. Since the \isi{pronominal} demonstratives often are derived from the adnominal ones, the former are usually more complex. 



As can be seen in \tabref{tab:5-3}, the basic adnominal demonstratives occur in only two different forms in each \isi{deictic} set, as compared to the much more complex system of \isi{pronominal} demonstratives that we will discuss shortly. The only contrast is one between the \isi{demonstrative} functioning attributively with a~masculine singular \isi{nominative} referent vs. a~referent agreeing with its head in any other case, \isi{gender} or number category. The forms are identical to the pronominally used masculine singular \isi{nominative} and feminine singular \isi{nominative}, respectively (see below), a~situation not uncommon in languages in general \citep[214]{himmelmann1996}. 
...
Non"=\isi{nominative}/{\allowbreak}Plural/{\allowbreak}Feminine\\\midrule
Proximal &
...
In example (\ref{ex:5-12}), the masculine \isi{noun} \textit{xooṛá} `betrothal' first occurs in the \isi{nominative} and is preceded by the adnominal \isi{demonstrative} in its masculine singular \isi{nominative} form, then it occurs in the \isi{genitive} and is then preceded by the adnominal \isi{demonstrative} in its other (non"=\isi{nominative}, plural or feminine) form.
...
\tabref{tab:5-4}, are number, case, \isi{gender} and distance/visibility, the first three also found with Palula nouns.\footnote{Some of the forms do not occur in text data but are affirmed through direct elicitation as possible forms.} We can also differentiate between so"=called strong and weak forms of demonstratives (or at least for most of them). Initial \textit{ee-} in the strong forms in A (the ones displayed in \tabref{tab:5-4}) regularly corresponds to B \textit{ha-}. The pronouns in the weak \isi{remote} sets of the demonstratives (\textit{so}, etc.), are the default choice and most frequently used as typical third person pronouns, with easily accessible \isi{discourse} referents \citep[432--433]{diessel2006}, and could be said to stand particularly close functionally to the personal pronouns proper. 
...
The masculine--feminine \isi{gender} distinction is found only in the singular \isi{nominative}, compare (\ref{ex:5-13}) and (\ref{ex:5-14}), whereas masculine and feminine are syncretised in the plural, as is evident when comparing (\ref{ex:5-15}) with (\ref{ex:5-15b}). 
...
The primary function of the distance differentiation is to distinguish three basic degrees of distance from the speaker/experiencer, where the proximal is used with referents close at hand, the \isi{distal} with referents further removed from the speaker, and the remaining category, here labeled ``\isi{remote}'', is used to refer to something or someone out of sight. 


The weak--strong opposition is related to accessibility. The strong forms tend to be picked for \isi{deictic} or \isi{anaphoric} use in order to keep track of less accessible \isi{discourse} referents, or to contrast another referent or switch from one referent to another. They are also the ones used in relative"=correlative clauses and in most cases as adnominal demonstratives. In the example sentence (\ref{ex:5-16}), the speaker has for some time been talking about making doors when building a~house, but is here switching to talking about windows instead. The weak forms, on the other hand, are used almost exclusively non"=contrastively and to pick out the most natural or accessible referent in a~\isi{discourse}. In example (\ref{ex:5-17}) the antecedent of the \isi{pronoun} (the cupboard) is very close and there is no ambiguity in what is referred to.
...
Thus, it is the weak form that functions most like a~third"=person \isi{pronoun}. The contrast is similar to the distinction \citet[417--419]{givon2001a} makes between ``stressed independent pronouns'' and ``unstressed \isi{anaphoric} pronouns''. Palula has also developed \isi{definite} articles, formally identical to the \isi{nominative} weak \isi{remote}/non"=visible demonstratives (see \sectref{subsec:5-2-6}). This strong--weak continuum mirrors in many respects the relative \isi{anaphoric} distance of \citet[419]{givon2001a}, as well as the grammaticalisation cline described by \citet[432]{diessel2006}. However, the absence of an~absolute distinction between demonstratives and third"=person pronouns across the board in Palula, and the presence of a~number of borderline cases, may very well reflect an~ongoing grammaticalisation \citep[213]{himmelmann1996}. The weak \isi{distal} forms with \textit{ṛ} and \textit{l}, respectively, are mere pronunciation variants, of which only the \textit{l}-variant is phonologically possible in B. Nor is there anything indicating any significance in the alternation between the strong \isi{distal} forms beginning with \textit{a-} and \textit{ee-}, respectively, but more detailed analysis is needed to be certain.



There is also another continuum that cuts across the entire paradigm. While the \isi{remote} category is primarily a~phoric device \citep[131]{saxena2006}, with a~high frequency in, for instance, narrative \isi{discourse}, and only secondarily functions deictically, the opposite holds for the proximal category, which is primarily \isi{deictic} and only rarely functions phorically. The mid"=level category, the \isi{distal}, is used extensively in both functions. An indirect effect of the differences in \isi{deictic} scope is the ability to signal a~contrast in the presence--absence of the third"=person object or person in the speech situation. It is normally only when a~person or object is present in the speech situation that the proximal and \isi{distal} terms are used (exophorically or anaphorically), whereas the \isi{remote} assumes their absence. The same holds for a~distinction between narration and direct speech - especially crucial in oral story"=telling. Along with other \isi{quotative} devices, the proximal and \isi{distal} terms refer to the outside world as perceived by the quoted speaker, whereas the narrator (whether identical to the former or not) would use the corresponding \isi{remote} terms to refer to the same entities.



\citet[204--205, 207--212]{schmidt2000} and \citet[134--136]{schmidtkohistani2001} describe similar effects obtained in the use of different sets of pronouns in some other \iliShina varieties. In Tileli there are two sets of third"=person pronouns, where one set signals visibility and first"=hand knowledge, whereas the other one codes what is invisible, unknown or indirectly inferred. In Kohistani \iliShina, a~similar discrimination can be made between information derived by visual means and information received by other means, through contrasting a~proximal set of demonstratives with a~\isi{remote} set. 



Three particular functions (\isi{exophoric}, \isi{anaphoric} and \isi{discourse}"=\isi{deictic}) will be further discussed and exemplified below (\sectref{subsec:5-2-3}--\sectref{subsec:5-2-5}), following the pragmatic taxonomy of demonstratives suggested by \citet[205--254]{himmelmann1996} and discussed by \citet[432]{diessel2006}. I do not have any clear examples of recognitional use, a~fourth category; however, there is evidence for the development of a~\isi{definite}, or ``\isi{anaphoric} article'' \citep[486]{juvonen2006}, contrasting with a~similarly developing \isi{indefinite} article. These two will be discussed in \sectref{subsec:5-2-6}. A secondary spatial specification will also be touched upon briefly in \sectref{subsec:5-2-7}.


\subsection{Exophoric use}
\label{subsec:5-2-3}

The \isi{exophoric}, or situational, use orients the hearer in the outside world and points to an~entity in the speech situation. This could be said to be the basic or original use of the \isi{demonstrative}. In Palula, it is primarily the proximal and \isi{distal} levels that enter into this function, and it is almost without exception the strong forms that are being used, whether \isi{pronominal} or adnominal.
...
\end{exe}
This stands in contrast to the past times that this particular narrative focuses on, which are elsewhere in this \isi{discourse} referred to as \textit{eesé waxtíi} `of those times', thus using the \isi{remote} term.
...
The \isi{distal} term \textit{aṛó} (B \textit{haṛó}) `that' points to something a~little farther away, but still fully visible and identifiable, as in (\ref{ex:5-24}), where it refers to something in the actual utterance situation, and in (\ref{ex:5-25}), where it refers to something in the narrated situation. 
...
It is probably futile to try and give absolute measures of when the proximal term is used as compared to when the \isi{distal} term is used. It is more a~matter of relative distance, as perceived by the speaker/experiencer in the situation. There are no apparent animacy restrictions on any of these terms when used exophorically, which seems to be the case with the corresponding terms in, for instance, Kohistani \iliShina \citep[135]{schmidtkohistani2001}. In as far as the \isi{remote} term is used exophorically, it is in referring to an entity that is \textit{not} present or visible in the utterance situation. 


\subsection{Anaphoric use}
\label{subsec:5-2-4}

The \isi{anaphoric}, or tracking, use is used to keep track of referents in \isi{discourse}, and can been seen as being derived from the \isi{exophoric} use by analogical extension, from pointing in the outside world to ``pointing'' to referents within the \isi{discourse}. The \isi{remote} is used particularly often for this, but the \isi{distal} sets are also frequently used, whereas the use of the proximal seems highly limited in this regard. Depending on how easily accessible the referents are, either the strong or the weak form may be used. The more accessible, the more likely it is that the weak (unstressed) form is chosen, whereas the greater the need to refocus the hearer's attention, the more likely it is that the strong (stressed) form is selected. 


In a~passage, exemplified in (\ref{ex:5-26}), belonging to a~procedural \isi{discourse}, the \isi{remote} term is used, both in its weak and its strong form.
...
Although the strong form tends to be used as an~\isi{anaphoric} \isi{demonstrative}~-- with a~refocusing or contrastive function \citep[432]{diessel2006}~-- and the weak tends to be used as a~personal \isi{pronoun}, the distinction between the two is not all that clear and is deserving of further research.

When the \isi{distal} term is used anaphorically (without any additional exophorical function), it is usually coreferential with a~main character or a~referent with special focus. Whereas the other referents, in an~exposé on the benefits of keeping goats, in (\ref{ex:5-27}), are referred to by the \isi{remote} term, the goats are repeatedly~-- although not exclusively~-- referred to by the \isi{distal} term (mostly in the singular). 
...
When the term is used in this \isi{discourse}, it is first in the strong form, but as it continues to be referred to, the weak form is used, thus functioning more like a~regular third"=person \isi{pronoun}.

In a~particular historical account, of which (\ref{ex:5-28}) is part, two groups of people are referred to, and while the \isi{remote} term is used for either of them when it is clear which one is intended, the \isi{distal} is used at a~point when there is a~need for special emphasis or clarification.
...
\end{exe}
The latter could alternatively be described as an~instance of the proximal term being used as (an unstressed) third"=person \isi{pronoun}, since there is no competing antecedent, and the \isi{pronoun} is not really used in order to refocus the listeners.

\subsection{Discourse"={deictic} use}
\label{subsec:5-2-5}

The \isi{discourse}"=\isi{deictic} use has to do with entire propositions being referenced rather than with tracking individual \isi{discourse} participants. Its function is to combine chunks of \isi{discourse} \citep[432]{diessel2006}, and is thus more abstract in its scope. In this rather specialised function, it seems the \isi{distal} term is used most of the time. It may be either anticipatory, as in (\ref{ex:5-30}), or refer back to a~rather long preceding chunk of \isi{discourse} as in (\ref{ex:5-31}).
...
Although the typical use seems to be \isi{pronominal}, it also occurs adnominally, as in (\ref{ex:5-32}). It usually occurs in its strong form, but it may occasionally be less emphasised, as in (\ref{ex:5-33}), where the weak form refers back to the preceding proposition, thus ``naming'' what has already been defined through \isi{discourse}.
...
\end{exe}
If the \isi{demonstrative} is immediately introducing a~chunk of \isi{discourse}, the proximal term can also be used, as in (\ref{ex:5-34}), probably with the additional effect that the listener is invited into a~private exchange between a~husband and his wife. 
...
The same `plan' or `consultation' is a~little later, in (\ref{ex:5-35}), referred to with the \isi{distal} term, then from the perspective of a~secret listener, standing outside the house where the exchange was taking place. The \isi{remote} term does not seem to be frequently used \isi{discourse}"=deictically. A possible example may be the one in (\ref{ex:5-36}), where the \isi{demonstrative} refers to a~point in time in a~narrative, the `night' following the events described in the immediately preceding chunk of \isi{discourse}. However, this rather specialised function, \citet[225]{himmelmann1996} defines as a~subtype of \isi{discourse}"=\isi{deictic} use.
...
Sometimes, as in (\ref{ex:5-37}), there is also an~ambiguity as to whether the \isi{demonstrative} refers anaphorically to a~particular \isi{discourse} referent or to a~preceding chunk of \isi{discourse}. Here, the planks as well as the totality are possible referents for the \isi{demonstrative}.
...
Closely related to adnominally"=used demonstratives, and still possibly a~subcategory of them, are the forms \textit{so} and \textit{se} (formally the weak forms of the \isi{remote} set), used for signalling \isi{definite} or previously introduced and firmly established entities in the \isi{discourse}. Their distribution (\tabref{tab:5-5}) is the same as that of the adnominal demonstratives: the form \textit{so} is used preposed to a~masculine singular \isi{nominative} head, whereas \textit{se} is used with a~head in any other case, \isi{gender} or number. 
...
Other (non"=\isi{nominative}/plural/feminine) head\\\midrule
\textit{so} &
...
Contrasting in definiteness and identifiability with \textit{so/se} used in this way, is the ``\isi{indefinite} article'' \textit{áa} or \textit{áak} (B \textit{a} or \textit{ak}), derived from, and in one of its forms still homonymous with, the \isi{numeral} \textit{áak} `one'. While \textit{áa} is used for introducing a~participant, \textit{áa míiš} `a (certain) man', \textit{so/se} is used in order to refer to or track an~already earlier introduced participant, \textit{so míiš} `the/that man'. The examples (\ref{ex:5-38})--(\ref{ex:5-41}) are all taken from one and the same story, where the two main characters are a~man and a (male) monster. At the very beginning of the story, in (\ref{ex:5-38}), the man is introduced, with a~full \isi{noun} and the \isi{indefinite} \textit{áa}.
...
a~third"=person \isi{pronoun} only (i.e., by the \isi{remote} \isi{demonstrative}, as described above), and we learn
...
introduced by a~full \isi{noun} preceded by the \isi{indefinite} \textit{áa} and a~descriptive attribute,
`hairy'.
\begin{exe}
\ex
\label{ex:5-39}
...
In the next sentence, in (\ref{ex:5-40}), the \textit{ṭhaaṭáaku} `monster' is referred to by a~full \isi{noun}
only, without any \isi{determiner} or attribute (which perhaps has some sort of intermediate status
between the \isi{indefinite} and the \isi{definite}), while \textit{míiš} `man', when he is referred to
again, is done so through the use of a~full \isi{noun} marked with the \isi{definite} \textit{so}.
\begin{exe}
\ex
\label{ex:5-40}
%modified
...
In the same way, when the monster is referred to in the following sentence, in (\ref{ex:5-41}), after the man is mentioned, he too is referred to by using a~full \isi{noun} and the \isi{definite} \textit{se} (that form because the head is in the \isi{oblique} case).
\begin{exe}
\ex
\label{ex:5-41}
%modified
...
Having exemplified this use of the ``\isi{definite} article'' primarily as a~tracking device, it should be pointed out that there are no clear and unambiguous examples of it being used as identifiable in the larger situation or associative"=anaphorically, two uses that are typical for \isi{definite} articles, while not generally associated with demonstratives in general (\citealt[485]{juvonen2006}; \citealt[233]{himmelmann1996}). Therefore, since the use of adnominal \textit{so/se} is not dramatically different from the \isi{anaphoric} use of adnominal demonstratives, an~alternative labelling of it would be ``\isi{anaphoric} article'' \citep[486]{juvonen2006}, but it also comes close to what \citet[230--239]{himmelmann1996} includes in the recognitional use of demonstratives. What sets it apart from the (strong) \isi{anaphoric} use (see \sectref{subsec:5-2-4}), however, and thus points to it being further grammaticalised towards a~more typical \isi{definite} article interpretation as ``identifiable'' in general \citep[485]{juvonen2006}, is its frequent and near"=obligatory presence with already introduced and well"=established referents in a~\isi{discourse}, its weak, thus phonetically reduced, form and its paradigmatic contrast with the \isi{indefinite} article.


Along the same lines, the use of \textit{áa/áak} exemplied above does not set it apart as radically estranged from its use as a~\isi{numeral}, but it could be said to have gone through an~initial grammaticalisation stage (as a~phonetically reduced presentative marker) towards a~full"=blown \isi{indefinite} article \citep[486]{juvonen2006}.
...
In addition to the basic differentiation between \isi{remote}, \isi{distal} and proximal, there are a~number of further spatially defined specifications that can be made. To the demonstratives (particularly in their adnominal use) of the \isi{distal} set, certain spatial adverbs (\sectref{subsec:7-1-2}) can be added (or fused with the demonstratives) to indicate finer degrees of distance as well as vertical and horizontal position in relation to the speaker, as shown in \tabref{tab:5-6}.
...
What is sometimes referred to as a~category of distinct possessive pronouns were already introduced above as \isi{genitive} case forms of the personal and \isi{demonstrative} pronouns, and are exemplified in (\ref{ex:5-42}) and (\ref{ex:5-43}).
...
There is one frequently used \isi{pronoun}, \textit{teeṇíi,} that can be described as reflexive. It
...
with the \isi{clause} \isi{subject} (whether explicit or implicit).
...
\end{exe}
However, when the reflexive occurs in the object position, as in (\ref{ex:5-48}), it seems necessary to use the construction \textit{teeṇíi zaán} `own self', where the \isi{reflexive pronoun} remains in the possessive.
\begin{exe}
\ex
\label{ex:5-48}
%modified
...
The \isi{pronoun} \textit{akaadúi} (literally `one' + a~segment \textit{aa} + `other') is reciprocal,
...
The \isi{finite} verb agrees in the plural, and if the agent/\isi{direct object} is a~\isi{pronoun} that makes a~case differentiation between agent and \isi{direct object}, it occurs in the \isi{accusative} as in (\ref{ex:5-51}).
\begin{exe}
\ex
\label{ex:5-51}
%modified
...
The \isi{reciprocal pronoun} may, however, also occur in case"=inflected forms, such as the \isi{genitive} form in (\ref{ex:5-52}). 
\begin{exe}
\ex
\label{ex:5-52}
%modified
...
The area of \isi{indefinite} pronouns and their distribution needs further research, but there are strong indications that most, if not all, \isi{indefinite} pronouns are also used as \isi{interrogative} pronouns (as further exemplified in \sectref{subsec:14-2-2}). As mentioned above (\sectref{subsec:5-2-1}), the \isi{indefinite}"=\isi{interrogative} pronouns (\tabref{tab:5-iipro}) belong together with the demonstratives in the larger system of pro"=forms. An animacy distinction is also made here, compare (\ref{ex:5-53}) and (\ref{ex:5-54}), not otherwise part of the \isi{pronominal} system.
...
See also \sectref{subsec:14-3-2} for a~possible development of a~set of negative \isi{indefinite} pronouns (\textit{ɡá=bi=na} `nothing', \textit{koó=bi=na} `nobody', \textit{ɡóo=bi=na} `nowhere'), based on the basic \isi{indefinite} ones.
...
There are no distinct relative pronouns in Palula. Instead, the demonstratives and the \isi{indefinite}"=\isi{interrogative} pronouns are used in relative constructions, or in the functional equivalents of relative clauses (see \sectref{sec:13-6}).