...
sick'. \todo{Please correct sentence} As indicated previously, (\chapref{sec:1}), the two issues
...
outlined in \sectref{sec:3.3} and evaluated based on some JC data.  An
...
constructions, and in -- SR -- in question phrases, the existence of a \todo{please check this quote and the ``in -- SR -- in'' expression}
...
show the same range of possibilities, as shown in \REF{ex:3:13}:
...
intransitive constructions with Stative meanings \REF{ex:3:15}.
...
\tabref{extab:3:17} shows Winford’s categorization of property items
...
\begin{table}
  \caption{Property Concepts in CECs (adapted from
    \citealt[184]{Winford1993}).}
  \label{extab:3:17}
  \fittable{ \small
...
compatible with Progressive aspect”.\footnote{While I do not accept the
  ...
  aspectual status of the items (see discussion in \chapref{ch:5},
...
\footnotetext{As will be discussed in \chapref{ch:5}, categorisation of
  ...
  that the similar form \textit{bradi} in Sranan may appear in both
  Stative and Non-stative use. Also, there are indications of
  variability in its behaviour in JC as well.}  
...
\textsc{1s} \textsc{poss} father greean \textsc{art} grass    \\
...
\ea
% (26)
(Human Propensity)    Jamaican Creole\\
\ea[]{
...
The examples \xxref{ex:3:22}{ex:3:27} above show JC items \textit{waid\slash lang\slash braad}
...
\gll dem \textbf{red/blak} di skert.\\
        3\textsc{pl} red/black \textsc{art} skirt  \\
...
like stative verbs” (p. 188).  What the examples in \xxref{ex:3:29}{ex:3:31} point to
...
ongoing Process.  \chapref{ch:4} will set the background for the
