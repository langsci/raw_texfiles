...
In the approach that I take here, the notion of inherent aspect is
...
As I will outline in my analysis of dual aspectual forms in \chapref{ch:5}, 
...
etc., that Aspect is compositional (see \chapref{ch:1} for discussion),
...
specified end time (Habitual or Generic in this case -- Atelic\todo{Check dashes}), both b
and c refer to situations having a fixed end point and as such are
both Telic.  In this sense, using \citegen{Vendler1967} terminology
...
the original State.  Thus each situation expressed in \REF{ex:4:3}
...
The difference between Stative and Non-stative predicates has been
...
characterize events.  In \sectref{sec:4.2.2}, I will look at the
...
The Event Structure in \REF{ex:4:5} is associated with lexical
...
to any other event.  This representation in \REF{ex:4:5} may be
compared with \citet{Jackendoff1996} which presents a State as a
situation which “just sits there with no dependence on time -- only a \todo{check dash}
...
Although the representations in \REF{ex:4:5} and \REF{ex:4:6} are
...
A Transition is one of the two event types which are characterised by
...
(Change of state).  This is shown below in \REF{ex:4:7}:
...
Event Structure and the pure State shown in \REF{ex:4:6} is that the
former is linked to a Process as shown in \REF{ex:4:7}, whilst a pure
...
The structure representing this is shown below in \REF{ex:4:8}:
...
\REF{ex:4:8} and the explanations associated with the notion of
...
His representation is shown below in \REF{ex:4:9}:

\ea%9
\label{ex:4:9}

{\citegen[322]{Jackendoff1996} Process}
\begin{center}
\begin{tikzpicture}
...
\node (spboundaxis) [above = of crosssection] {[sp-bound axis]\footnote{Refers to the structure-preserving (sp-)binding relationship between Situation, Path and Time axes (\citealt{Jackendoff1996}, p. 322)}}; 
...
As shown here, a Process is presented as three axes (the Situation,
...
\sectref{sec:5.2.2}, such items are inherently associated with a State
...
analysis of CEC dual aspectual forms in \chapref{ch:5}. I will posit
...
Activity, as will be discussed in \sectref{sec:5.2}. \citet{Dowty1979}
...
Agent in the expression.  As shown below in \REF{ex:4:15} the verbs
...
does not permit this alternation.  Compare \REF{ex:4:16}:
...
\ea[*]{The cloth \textbf{cut.}\footnote{Though this is not a possibility
  ...
  e.g.: JC \textit{Di klaat kot.} `The cloth is\slash has been cut'.  What
...
\REF{ex:4:19} are Atelic although (\ref{ex:4:19}b) and (\ref{ex:4:19}c) contain internal arguments
...
due to the fact that the Non-stative verb \textit{run} in \REF{ex:4:19}
...
In \chapref{ch:5}, I will provide an analysis of dual aspectual forms in
