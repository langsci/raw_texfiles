...
Clauses can be combined in several ways. Two or more main clauses can be linked by juxtaposition or by using a \isi{coordinating conjunction}\is{Conjunction} (\sectref{sec:11.2}). Alternatively, one clause may contain another as \isi{subordinate clause}: various categories of verbs take a clausal \isi{complement} (\sectref{sec:11.3}); nouns may be modified by a \isi{relative clause}\is{Clause!relative} (\sectref{sec:11.4}); adverbial clauses serve as an adjunct in a \isi{main clause} (\sectref{sec:11.6}).
...
Old Rapa Nui did not have a \isi{coordinating conjunction}\is{Conjunction}. Both phrases and clauses were linked by simple juxtaposition (i.e. asyndetic coordination, see \citealt[7]{Haspelmath2007}). \REF{ex:11.1} shows juxtaposed clauses, while \REF{ex:11.2} contains a string of juxtaposed noun phrases.
...
In modern Rapa Nui, the conjunction\is{Conjunction} \textit{{\ꞌ}e}\is{e ‘and’@{\ꞌ}e ‘and’|(} ‘and’ (probably a \ili{Tahitian} borrowing\is{\ili{Tahitian} influence}\footnote{\label{fn:504}Concerning the origin of \textit{{\ꞌ}e} in \ili{Tahitian}: although it is phonologically identical to \ili{French} ‘et’, the fact that \textit{{\ꞌ}e} is already common in the 1838 \ili{Tahitian} Bible translation (\citealt{TeBibilia1996}) indicates that it predates \ili{French} influence. A similar conjunction\is{Conjunction} (spelled \textit{e}, \textit{{\ꞌ}e} or \textit{ē}) occurs in \ili{Pa’umotu}, \ili{Rarotongan} and \ili{Mangarevan}, but not in \ili{Marquesan} and \ili{Māori}.}) is used to link clauses and phrases; it occurs in clause- and phrase-initial position.
...
\setcounter{enumi}{3}
\item 
To indicate a larger break in a sentence. This often involves a shift to a different type of information (indicated by a different \isi{aspect} marker\is{Aspect marker}) or a shift in subject:
\end{enumerate}
...
When noun phrases marked with prepositions are coordinated, the \isi{preposition} is repeated, including the accusative marker\is{i (accusative marker)} \textit{i}; the last item may be preceded by \textit{{\ꞌ}e} as in (\ref{ex:11.17}–\ref{ex:11.18}), but juxtaposition is also common, as in (\ref{ex:11.19}–\ref{ex:11.20}):
...
In old texts, \isi{disjunction} is expressed by juxtaposition:
...
\setcounter{enumi}{1}
\item 
Nominalised \isi{complement} clauses\is{Clause!nominalised}, in which the verb is introduced by the article \textit{te}; it may be preceded by the \textsc{acc} marker \textit{i}, as in the following example:
\end{enumerate}
...
It depends on the matrix verb which type of construction is used. Only types 1 and 2 involve a proper \isi{complement}, that is, a constituent which is syntactically an argument of the matrix verb. For lack of a better term, constructions of types 3 and 4 will sometimes be referred to as “\isi{complement}” or “\isi{complement} clause” in the following sections, but one should bear in mind that this does not imply that they are syntactically a \isi{complement} of the verb.

Types 3 and 4 are quite similar; in fact, 3 is a subset of 4, with the following two restrictions:

\begin{itemize}
\item 
Asyndetically coordinated clauses generally have identical \isi{aspect} marking\is{Aspect marker}; in type 4, the \isi{aspect} marking of the \isi{complement} clause is independent from that of the \isi{main clause}.
...
In addition to these four types of constructions, the same matrix verbs may also have a involve monoclausal constructions: nominal arguments and serial verbs. An example of a serial verb construction with \isi{complementation} function is the following:
...
\is{Verb!perception}
Perception verbs like \textit{u{\ꞌ}i} ‘to see, watch’, \textit{hakaroŋo} ‘to listen’ and \textit{ŋaro{\ꞌ}a} ‘to hear’ can be followed by a nominal \isi{complement} (\sectref{sec:8.6.4.2}), or by a clause which is syntactically independent of the perception verb (type 4 above). The latter will be discussed in the following subsections.

\subsubsection[Use of aspectuals ]{Use of aspectuals} \label{sec:11.3.1.1}

When a perception verb is followed by a clause describing the perceived event, the range of \isi{aspect} markers in this clause is limited: \textit{ka}, \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} and \textit{e} are used, while \textit{i} and \textit{he} do not occur. The absence of perfective \textit{i} is not surprising: events which are over and done with are usually not the object of perception. The absence of neutral \textit{he} is not unexpected either: \textit{he} is not able to provide the necessary temporal/aspectual link between the two clauses.

\subparagraph{Contiguity marker \textit{ka}} When the clause expresses an activity or event which is perceived while it is happening, it is often marked with the contiguity marker \textit{ka}. \textit{Ka}\is{ka (\isi{aspect} marker)} (\sectref{sec:7.2.6}) expresses simultaneity\is{Simultaneity} between the event of perception and the event which is perceived: both take place at the same time. 
...
\subparagraph{Perfect \isi{aspect} \textit{ko V {\ꞌ}ā}} When the clause expresses a state of affairs which is perceived, it is marked with the perfect \isi{aspect}\is{Aspect!perfect} \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} (\sectref{sec:7.2.7}). This state of affairs may be the result of an event which has taken place before; what is seen is not the event itself but a situation from which the event can be inferred. 

\is{Verb!perception}The \textit{ko}{}-marked \isi{complement} is often a stative verb or a temporal noun like \textit{pō} ‘night’; the perfect \isi{aspect}\is{Aspect!perfect} expresses that this state has come about in some way, without specifying how. In \REF{ex:11.35} it is night because it has \textit{become} night, and the ship is far from Rapa Nui because it has been \textit{moving} further and further away. 
...
\subparagraph{Imperfective \textit{e}} The third aspectual used after verbs of perception is \isi{imperfective} \textit{e}, usually followed by the continuity marker \textit{{\ꞌ}ā/{\ꞌ}ana}. While \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} indicates a state which has come about, \textit{e V {\ꞌ}ā}\is{e (\isi{imperfective})!e V {\ꞌ}ā} underlines the continuous nature of a situation, without implying the process by which it has come about (\sectref{sec:7.2.5.4} on \textit{e V {\ꞌ}ā}\is{e (\isi{imperfective})!e V {\ꞌ}ā}).
...
How should these constructions be analysed? At first sight, the \isi{complement} clause in (\ref{ex:11.38}–\ref{ex:11.41}) can be considered as a \isi{relative clause}\is{Clause!relative} to the object. One argument against this is the function of the \isi{aspect} marker\is{Aspect marker}: whereas relative clauses\is{Clause!relative} marked with \textit{ka} usually express an event posterior\is{Posteriority} to that in the surrounding clause(s) (\sectref{sec:11.4.3}), in these examples the \textit{ka}{}-marked clauses express an event \is{Simultaneity}simultaneous to the perception event of the matrix clause. Moreover, as \REF{ex:11.39} shows, the clause may follow a \isi{proper noun}, even though proper nouns\is{Noun!proper} normally do not take relative clauses\is{Clause!relative}.

A second possibility would be to regard the object NP and the \isi{complement} clause as two complements of the perception verb. This would mean that perception verbs\is{Verb!perception}, which normally take one \isi{complement}, take two complements in this construction. Such an analysis would only be plausible if these arguments fulfilled different semantic roles. However, the \isi{noun phrase} and the clause do not express different semantic roles connected to the action; neither do they express two instances of the same semantic role (*‘I saw him \textit{and} coming’); rather, they are two aspects of a single semantic role: the nominal \isi{complement} refers to the perceived referent, while the clause expresses an action which is not only performed by that entity, but also part of the same perceived situation. 

Therefore it seems more plausible to consider the nominal \isi{complement} and the \isi{complement} clause as a single constituent. The fact that the \isi{noun phrase} can be marked with \textit{ko}\is{ko (prominence marker)} (which is the default case marker in the absence of other markers) is an argument for this analysis. Constructions (\ref{ex:11.38}–\ref{ex:11.39}) suggest that this \isi{noun phrase} can be raised to the object position of the matrix verb.
\is{Verb!perception}
...
\is{Verb!aspectual verb}This identical marking is not limited to \isi{aspect} markers\is{Aspect marker}. In \REF{ex:11.45}, both verbs are marked with the negator \textit{kai}. In \REF{ex:11.46}, \textit{ha{\ꞌ}amata} is the verb of a bare \isi{relative clause}\is{Clause!relative!bare} (\sectref{sec:11.4.5}), which is characterised by the absence of an \isi{aspect} marker; the \isi{complement} verb \textit{tu{\ꞌ}u} is likewise unmarked.
\is{haamata ‘to begin’@ha{\ꞌ}amata ‘to begin’}
...
As (\ref{ex:11.42}–\ref{ex:11.45}) show, the S/A of the second verb is often placed in the subject\is{Subject!raising} position of the matrix clause. However, it may also be placed after the \isi{complement} verb:
...
\is{Verb!aspectual verb}A second construction is that in which the \isi{complement} is expressed as a nominalised\is{Verb!nominalised} verb (i.e. preceded by the determiner \textit{te}). This \isi{complement} may have the accusative marker\is{i (accusative marker)} \textit{i}\is{i (accusative marker)} as in \REF{ex:11.48}, but usually this marker is omitted, as in \REF{ex:11.49}:
...
Despite the nominalised character of the \isi{complement}, it still has verbal characteristics: its Patient (\textit{i te mā{\ꞌ}ea era} in \REF{ex:11.48}) is marked with \textit{i}.
\is{haamata ‘to begin’@ha{\ꞌ}amata ‘to begin’}
\is{Verb!aspectual verb}
\subsubsection{\textit{Oti} ‘finish’}\label{sec:11.3.2.2}
\is{oti ‘to finish’}
The verb \textit{oti} has several senses: ‘to be finished, done, over’ (e.g. a story), ‘to run out’, ‘to be the only one’. One common use is ‘to finish doing something’, where \textit{oti} is followed by a \isi{complement} clause.

The \isi{complement} verb is nominalised\is{Verb!nominalised}, i.e. marked with the article \textit{te}. Sometimes it is preceded by the accusative marker\is{i (accusative marker)} \textit{i}, in other cases \textit{i} is omitted.\footnote{\label{fn:506}In \ili{Māori}, the \isi{complement} verb never has the accusative marker\is{i (accusative marker)}; \citet{Hooper1984Unusual} argues that the \isi{complement} verb is the subject of \textit{oti}.}  As the examples show, the subject of the second verb may be placed in the subject position of \textit{oti}\is{Subject!raising} as in \REF{ex:11.50} and \REF{ex:11.52}, or follow the \isi{complement} verb as in \REF{ex:11.51} and \REF{ex:11.53}.
...
When the \isi{complement} verb is transitive, the Patient may be raised to the subject position of \textit{oti}, showing that the \isi{complement} clause is passivised\is{Passive}: 
...
\textit{Oti} as a matrix verb with a \isi{complement} may also be expressed in a serial verb\is{Serial verb} construction. For examples, see (\ref{ex:7.181}–\ref{ex:7.182}) in \sectref{sec:7.7.3}.
\is{oti ‘to finish’}

\subsubsection{\textit{Hōrou} ‘hurry’}\label{sec:11.3.2.3}
\is{horou ‘to hurry’@hōrou ‘to hurry’}
\textit{Hōrou} ‘to hurry, (be) quick’ is used as an adjective or \isi{adverb}\is{Adverb}, but more commonly it is a main verb taking a clausal argument. This argument can be expressed in a variety of ways: 
...
\item 
as a serial verb\is{Serial verb} as in \REF{ex:11.56}, with repetition of the \isi{aspect} marker\is{Aspect marker}, but nothing else between the two verbs (\sectref{sec:7.7} on serial verbs); 
...
As \REF{ex:11.58} shows, the subject of the second verb may be raised to the subject\is{Subject!raising} position of \textit{hōrou} (in this case, the Patient is raised, showing that the \isi{complement} clause is passivised\is{Passive}).

\subsubsection{\textit{Oho} ‘go, about to’}\label{sec:11.3.2.4}
\is{oho ‘to go’}
\textit{Oho} ‘go’ usually refers to physical movement; in this sense, it is the most unmarked \isi{motion verb}\is{Verb!motion}. \textit{Oho} is also used as an aspectual verb, indicating that an event is about to happen (possibly under influence of \ili{Spanish} \textit{ir}, cf. \citealt[392]{Fischer2007}). In this sense, \textit{oho} is followed by a \isi{complement} clause introduced by \textit{mo}\is{mo (preverbal)}.
...
\is{Verb!speech}
As discussed in \sectref{sec:8.6.4.2}, there are two types of speech verbs\is{Verb!speech} in Rapa Nui, ‘say’-type and ‘talk’-type verbs. Only the former, which include e.g. \textit{kī} ‘say’ and \textit{\mbox{{\ꞌ}a{\ꞌ}amu}} ‘tell’, can be followed by a clause (or longer discourse) expressing the content of speech. This can be a \isi{direct speech}, which usually follows without a specific marker:
...
When the speech verb is followed by an \isi{indirect speech}, it is often introduced by \textit{pē nei (ē)} ‘like this’ (\sectref{sec:4.6.5.1}):
...
These verbs may take a nominal \isi{complement} introduced by \textit{i} or \textit{ki} (\sectref{sec:8.6.4.2}). They may also take a clausal \isi{complement} introduced by \textit{mo}\is{mo (preverbal)} (\sectref{sec:11.5.1}):
...
\is{Verb!modal}
Various verbs can be used to express modal concepts such as ability, possibility and obligation. These verbs are followed by a \isi{complement} clause, which is in most cases introduced by \textit{mo}. Most of these verbs are also used in other constructions, e.g. with a nominal \isi{complement}. If the subject is expressed, it occurs in the \isi{main clause} (except with \textit{tiene que}, see below).
...
Possibility is often expressed by \textit{puē}\is{pue ‘can’@puē ‘can’}. This word is borrowed from \is{\ili{Spanish} influence}\ili{Spanish} \textit{puede}, the third person sg. present \isi{tense} of \textit{poder} ‘can, be able’,\footnote{\label{fn:508}It is not uncommon for \ili{Spanish} words to be borrowed in the 3\textsuperscript{rd} person sg. present \citep[197]{Makihara2001Adaptation}. The weak pronunciation of intervocalic \textit{d} in Chilean \ili{Spanish} facilitates its elision\is{Elision} (\sectref{sec:2.5.3.1}); the resulting VV sequence coalesces into a single \isi{long vowel}.} but is used in all persons and numbers. It is followed by \textit{mo V}.
\is{Verb!modal}
...
Relative clauses modify the head noun in a \isi{noun phrase}. In Rapa Nui, as in most languages, the head noun is external to the \isi{relative clause}\is{Clause!relative} itself; it is a constituent of a higher clause. As this noun has a semantic role both in the higher clause and in the matrix clause, \citet[317]{Dixon2010-2} uses the term \textit{common argument} (CA).
...
\begin{itemize}
\item 
They follow the head noun and usually occur at the end of the \isi{noun phrase}.

\item 
They are almost always predicate-initial.

\item 
Most types of relative clauses exhibit a gapping strategy: the \isi{common argument} is not expressed in the \isi{relative clause}\is{Clause!relative}.

\item 
The aspectual \textit{he}\is{he (\isi{aspect} marker)} is rare; the most common aspectuals are \textit{e} and \textit{i}.

\item 
The aspectual is often left out.

\item 
The S/A argument of the \isi{relative clause}\is{Clause!relative} may be expressed by a pre- or postnominal \isi{possessor}\is{Possession} modifying the head noun.

\item 
When the \isi{common argument} is direct object in the \isi{relative clause}\is{Clause!relative}, the subject is often \textit{e}{}-marked.

\item 
The verb in the \isi{relative clause}\is{Clause!relative} may be raised to a position adjacent to the head noun.
...
\item 
Relative clauses are always restrictive, i.e. they restrict the reference of the \isi{noun phrase}. Rapa Nui does not have nonrestrictive relative clauses\is{Clause!relative}, clauses which add information without limiting the reference. If such a clause is called for, a generic noun is placed in \isi{apposition}\is{Apposition} to the head noun to serve as an anchor for the \isi{relative clause}\is{Clause!relative} (see (\ref{ex:5.170}–\ref{ex:5.171}) on p.~\pageref{ex:5.170}).
...
In many languages, there are restrictions on the types of constituents that can be relativised. \citet{KeenanComrie1977,KeenanComrie1979} account for this by proposing a “\isi{noun phrase} accessibility hierarchy”: 

\ea\label{ex:11.88a}
\label{hierarchy}  subject {\textgreater} direct object {\textgreater} indirect object {\textgreater} oblique {\textgreater} \isi{possessor}\is{Possession}
\z
...
In this section, relativisation of different constituents in Rapa Nui will be discussed and illustrated. At the end of the section, the issue of the \isi{noun phrase} hierarchy will be revisited.

\subparagraph{\ref{sec:11.4.2}.1~ Subject} Subject relativisation is common. The subject is not expressed in the \isi{relative clause}\is{Clause!relative}.
...
\subparagraph{\ref{sec:11.4.2}.3~ Oblique} When oblique arguments\footnote{\label{fn:511}This includes “indirect objects” (\sectref{sec:8.8.1}).} are relativised, the \isi{common argument} is expressed pronominally in the \isi{relative clause}\is{Clause!relative}.\footnote{\label{fn:512}\citet[1]{Silva-Corvalán1978} gives an example of an oblique argument relativised with gapping, but such a construction does not occur anywhere in my the text corpus.} As examples in native texts are scarce, two example from the Bible translation are given.
...
\subparagraph{\ref{sec:11.4.2}.4~ Adjunct} Adjuncts are relativised without being expressed in the \isi{relative clause}\is{Clause!relative}. These usually express place as in \REF{ex:11.97} or time as in \REF{ex:11.98}, but other adjuncts are possible as in \REF{ex:11.99}.
...
\begin{enumerate}
\item 
When the \isi{relative clause}\is{Clause!relative} is a locative clause, i.e. the relativised phrase is predicate\is{Clause!locative}:
\end{enumerate}
...
\setcounter{enumi}{1}
\item 
When a \isi{preposition} is needed to specify the nature of the locative relationship, for example, when a movement is involved from (\textit{mai}) the referent:
\setcounter{enumi}{1}
\end{enumerate}
...
As these examples show, the \textit{ira} constituent is in clause-initial position in the \isi{relative clause}\is{Clause!relative}.

\subparagraph{\ref{sec:11.4.2}.5~ Possessor} Relative clauses with \isi{possessor}\is{Possession} relativisation are rare, but they do occur. The \isi{possessor}\is{Possession} is expressed pronominally in the \isi{relative clause}\is{Clause!relative}, in the same position where it would be in a \isi{main clause}. In the following example, \textit{te rāua} is coreferential to the head noun \textit{nu{\ꞌ}u}.
...
\subparagraph{\ref{sec:11.4.2}.6~ Identifying predicates} Predicates\is{Clause!identifying} of identifying clauses (\sectref{sec:9.2.2}) may also be relativised. In this case, the predicate is expressed in the \isi{relative clause}\is{Clause!relative} as a pronoun preceded by \textit{ko}.\footnote{\label{fn:513}See \sectref{sec:9.2.2} for arguments to consider the \textit{ko}{}-marked pronoun as predicate.} 
...
In this example the location (\textit{kona}) is relativised, while the Existee (the entity that exists in a given place) is expressed in the \isi{relative clause}\is{Clause!relative}, marked with the possessive \isi{preposition} \textit{o}. 

The Existee can also be relativised, with the location expressed in the \isi{relative clause}\is{Clause!relative}:
...
In possessive clauses\is{Clause!possessive}, the \isi{possessor}\is{Possession} can be relativised as in \REF{ex:11.107}; in this case, the \isi{possessee} is expressed in the \isi{relative clause}\is{Clause!relative}. The \isi{possessee} can also be relativised as in (\ref{ex:11.108}–\ref{ex:11.109}), in which case the \isi{possessor}\is{Possession} is expressed in the \isi{relative clause}\is{Clause!relative}.
...
As these examples show, possessees in the \isi{relative clause}\is{Clause!relative} are marked with genitive \textit{o} as in \REF{ex:11.107}; possessors are marked either with \textit{o} as in \REF{ex:11.108} or the general-purpose \isi{preposition} \textit{i} as in \REF{ex:11.109}. 

To summarise: there are two relativising strategies in Rapa Nui: one involving a gap (non-expressed constituent), one involving a resumptive element. It depends on the role of the relativised constituent which strategy is used; this is shown in \tabref{tab:66}.\footnote{\label{fn:514}Existential clauses are not included separately in this table. When the Existee/\isi{possessee} is relativised, it is the subject of the clause; when the \isi{possessor}\is{Possession} or location is relativised, it can be considered as an adjunct.}
...
Imperfective \textit{e}\is{e (\isi{imperfective})} in relative clauses\is{Clause!relative} often refers to events which are going on at the time of reference, as in \REF{ex:11.113}. Alternatively, it may indicate events which happen repeatedly or habitually, as in \REF{ex:11.114}. The verb is usually followed by a postverbal demonstrative\is{Demonstrative!postverbal} (\sectref{sec:7.2.5.4}).
...
Perfect \textit{ko/ku V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} indicates a state which has come about in some way: with event verbs as in \REF{ex:11.115}, the state is the result of the event described by the verb; with statives as in \REF{ex:11.116}, the situation has resulted from some unspecified process.
...
When the contiguity marker \textit{ka}\is{ka (\isi{aspect} marker)} is used in a \isi{relative clause}\is{Clause!relative}, the clause expresses an event posterior\is{Posteriority} to the events in the context. In \isi{direct speech} this means the clause refers to the future\is{Future}, as in \REF{ex:11.117}; in narrative texts the \textit{ka}{}-marked \isi{relative clause}\is{Clause!relative} is posterior\is{Posteriority} with respect to the time of the main action, as in \REF{ex:11.118}. The verb is always followed by a postverbal demonstrative\is{Demonstrative!postverbal}.
...
Neutral \textit{he}\is{he (\isi{aspect} marker)} is rarely used in relative clauses\is{Clause!relative}. In the few examples I found, its function seems to be similar to \textit{ka}:
...
In possessive-relative\is{Clause!possessive-relative|(} constructions, the head noun is preceded or followed by a \isi{possessor}\is{Possession}, which is coreferential to the subject of the \isi{relative clause}\is{Clause!relative}; the latter is not expressed in the \isi{relative clause}\is{Clause!relative} itself. (These constructions occur in Rapa Nui as well as in various other Polynesian languages.) Possessive-relative constructions only occur when a constituent other than the subject is relativised; they are found with both object and adjunct relativisation. An example is the following:
...
Syntactically, \textit{te kōrua} is a possessive pronoun modifying \textit{me{\ꞌ}e} ‘thing’; it is coreferential to the implied subject of the \isi{relative clause}\is{Clause!relative}. 

When the \isi{possessor}\is{Possession} is pronominal, it may either precede the noun as in (\ref{ex:11.123}–\ref{ex:11.124}), or follow it as in \REF{ex:11.125} (\sectref{sec:6.2.1}):
...
\glt 
‘How did the water splash at the time when I jumped?’ \textstyleExampleref{[R108.125]}\footnote{That this is a \isi{relative clause}\is{Clause!relative}, not just a modifying verb, is shown by the verb phrase \isi{particle} \textit{mai}.}
\z
...
When the \isi{possessor}\is{Possession} is a full \isi{noun phrase}, it must occur after the noun:
...
\z
\footnotetext{Examples such as \REF{ex:11.127} are potentially ambiguous. As discussed above, in object relative clauses\is{Clause!relative} the subject is sometimes preverbal and preceded by the proper article\is{a (proper article)} \textit{a} (see \REF{ex:11.94} in \sectref{sec:11.4.2} above). Now the proper article\is{a (proper article)} \textit{a} is homophonous to the possessive \isi{preposition} \textit{{\ꞌ}a}, and both may be followed by proper nouns; therefore, in examples such as \REF{ex:11.127}, the subject could also be analysed as a nominative subject marked with the proper article\is{a (proper article)} \textit{a.} However, an analysis as genitive (i.e. \textit{{\ꞌ}a} rather than \textit{a}) is more plausible, as only pronouns occur unambiguously as preverbal subjects\is{Subject!preverbal} in the \isi{relative clause}\is{Clause!relative}; \isi{noun phrase} subjects in relative clauses\is{Clause!relative} are always postverbal (see e.g. (\ref{ex:11.91}–\ref{ex:11.92}) above).}

Possessive-relative constructions occur in other Polynesian languages as well. There has been some discussion on the question whether the \isi{possessor}\is{Possession} is raised from the subject\is{Subject!raising} position of the \isi{relative clause}\is{Clause!relative} (e.g. \citealt[367]{Harlow2000}; \citealt[185]{Harlow2007Maori}), or whether it is a genuine \isi{noun phrase} \isi{possessor}\is{Possession} which happens to be coreferential to the \isi{relative clause} subject \citep[116]{Clark1976}. In Rapa Nui the second option is more plausible. First, the \isi{possessor}\is{Possession} can be in the same positions as in any other \isi{noun phrase}, which suggests that it is no different from other possessors in the \isi{noun phrase}. Second, as the examples above show, the form of the possessive construction varies between \is{Possession!o/a distinction}\textit{a-} and \textit{o-}\isi{possession}: \textit{a}{}-\isi{possession} in \REF{ex:11.123}, \REF{ex:11.125} and \REF{ex:11.127}, \textit{o}{}-\isi{possession} in \REF{ex:11.124} and \REF{ex:11.126}. As \textit{a-} and \textit{o-}\isi{possession} express different semantic relationships between \isi{possessor}\is{Possession} and \isi{possessee} (\sectref{sec:6.3.2}), this suggests that there is a direct relation between the \isi{possessor}\is{Possession} and the head noun, even though the primary function of the \isi{possessor}\is{Possession} seems to be the expression of the \isi{relative clause}\is{Clause!relative} subject.\footnote{\label{fn:517}\citet{HerdMacdonald2011} make a similar observation for other Polynesian languages. They propose a structure where there is a relation between the \isi{possessor}\is{Possession} and the relative construction as a whole. This involves a control relation (not raising) between \isi{possessor}\is{Possession} and \isi{relative clause}\is{Clause!relative} subject.} And indeed, in most of these cases the choice between \textit{{\ꞌ}a} and \textit{o} is governed by the same principles guiding this choice in possessive constructions in general. In \REF{ex:11.127}, where the relation between \isi{possessor}\is{Possession} and head noun is one between husband and wife, \textit{{\ꞌ}a} is used (\sectref{sec:6.3.3.1.1}). In \REF{ex:11.125}, the use of \textit{{\ꞌ}a} is possibly motivated by the active relationship of the \isi{possessee} to the head noun ‘work’ (\sectref{sec:6.3.3.2} item 2). In \REF{ex:11.126}, \textit{o} is used with a temporal noun, again conforming to a general pattern (\sectref{sec:6.3.3.3} item 11). In fact, given the wide range of relationships expressed by possessive constructions in Rapa Nui, all possessive-relatives seem to exhibit some kind of possessive relationship also attested in simple possessive constructions.

If this analysis is correct, the \isi{possessor}\is{Possession} is not the result of raising, but is a normal \isi{noun phrase} \isi{possessor}\is{Possession} which happens to be coreferential to the \isi{relative clause}\is{Clause!relative} subject. Under coreferentiality\is{Referentiality}, the latter is left unexpressed. 

This analysis is confirmed by the fact that there are also possessive-relative constructions where the \isi{possessor}\is{Possession} is not the subject of the \isi{relative clause}\is{Clause!relative}, but an oblique/embed\-ded constituent as in \REF{ex:11.128}:
...
Bare relative clauses\is{Clause!relative} are relative clauses\is{Clause!relative} in which the verb is not preceded by an aspectual. In Rapa Nui in general, the aspectual is obligatory, except in a few well-defined contexts (\sectref{sec:7.2.2}), one of which is when the verb is \isi{adjectival} (i.e. functions as noun modifier). Even so, in \sectref{sec:5.7.2.3} I argued that bare relatives are different from \isi{adjectival} modifiers: unlike the latter, they are truly verbal in that they indicate an event taking place at a specific time; moreover, they can be followed by verb phrase particles and verb arguments. 
...
These examples show that bare relative clauses\is{Clause!relative} are not limited to one single \isi{aspect}. In most cases they express a one-time event which has been completed as in \REF{ex:11.129}, i.e. the clause has perfective \isi{aspect}; however, they may also be habitual\is{Aspect!habitual} as in \REF{ex:11.130}, durative\is{Aspect!durative} as in \REF{ex:11.131}, or stative as in \REF{ex:11.132}.
...
Even though noun and verb can be separated by these \isi{noun phrase} elements, there is a strong tendency to place the verb adjacent to the noun. Often \is{Verb!raising}the verb is raised to a position straight after the noun, before other \isi{noun phrase} elements. In \REF{ex:11.135}, the verb \textit{hatu} is raised to a position before the \isi{quantifier}\is{Quantifier} \textit{ta{\ꞌ}ato{\ꞌ}a}, while the subject of the \isi{relative clause}\is{Clause!relative} is stranded after \textit{ta{\ꞌ}ato{\ꞌ}a}. (The status of \textit{era} is discussed below.)
...
Similarly, in \REF{ex:11.136}, the verb \textit{tu{\ꞌ}u} is raised over the postnominal \isi{possessor}\is{Possession} \textit{\mbox{{\ꞌ}ā{\ꞌ}ana}}. Notice that even though the \isi{relative clause}\is{Clause!relative} only consists of a verb, it is still a true \isi{relative clause}\is{Clause!relative}, not an “\isi{adjectival}” verb: \textit{tu{\ꞌ}u} refers to a specific event, it is not a time-stable property of the child (\sectref{sec:5.7.2.3}).
...
\z
\is{Verb!raising}
In \REF{ex:11.137} the verb \textit{hiŋa} is raised both over the \isi{particle} \textit{{\ꞌ}ā} and the \isi{possessor}\is{Possession} \textit{o te poki}.\footnote{\label{fn:518}\textit{{\ꞌ}Ā} occurs both in the \isi{noun phrase} (expressing identity) and in the verb phrase (expressing continuity); here it is a \isi{noun phrase} \isi{particle}, modifying the noun: ‘the very same day’.} The same happens in \REF{ex:11.138}, where the \isi{possessor}\is{Possession} ‘of the morning’ modifies the head noun, while the next phrase ‘to school’ is the part of the \isi{relative clause} left stranded\is{Clause!relative}.
...
\is{era (distal)!postnominal}Examples (\ref{ex:11.135}–\ref{ex:11.138}) all involve a demonstrative \textit{era}. Now this demonstrative (as well as \textit{nei} and \textit{ena}) is common both in the \isi{noun phrase} and in the verb phrase, so \textit{a priori} it may be either a postnominal \isi{particle} over which the verb has been raised, or a verb phrase \isi{particle} belonging to the \isi{relative clause}\is{Clause!relative}. The position of \textit{era} in the examples suggest that the former is the case, as indicated by the brackets. \textit{Era} occurs after the \isi{quantifier}\is{Quantifier} in \REF{ex:11.135}, but before the \isi{possessor}\is{Possession} in \REF{ex:11.136} and before the \isi{particle} \textit{{\ꞌ}ā} in (\ref{ex:11.137}–\ref{ex:11.138}); in other words, \textit{era} occurs in its usual \isi{noun phrase} position (see the chart in \sectref{sec:5.1}). If \textit{era} were a verb phrase \isi{particle}, it would be unclear why it is raised with the verb in (\ref{ex:11.136}–\ref{ex:11.138}), but left stranded in \REF{ex:11.135}.
...
In other words, even though \textit{kī era e nua} in \REF{ex:11.140} seems to be a \isi{relative clause}\is{Clause!relative}, the presence of \textit{tū} suggests that \textit{era}\is{era (distal)!postnominal} is not part of the \isi{relative clause}, but is a \isi{noun phrase} \isi{particle} which has been leapfrogged over by the verb.

Examples such as (\ref{ex:11.139}–\ref{ex:11.140}) are quite common. In fact, the tendency to leave out the aspectual and (if needed) to raise the verb is strongest with definite/\isi{anaphoric}\is{Anaphora} noun phrases like the ones illustrated here. Leaving out the aspectual has the effect of downplaying the action/event character of the \isi{relative clause}\is{Clause!relative}: what the \isi{relative clause}\is{Clause!relative} denotes is not so much an event but rather a fact; this fact is part of the referential description in the \isi{noun phrase}.
\is{Clause!relative|)}\is{Clause!relative!bare|)}\is{Verb!raising}
\section{Subordinating markers}\label{sec:11.5}

The preverbal markers \textit{mo}, \textit{ana}, \textit{ki}, \textit{{\ꞌ}o} and \textit{mai} are used to mark certain types of clauses. As these markers occur in the same position as aspectuals (\sectref{sec:7.1}), they do not co-occur with the latter, which means that a clause containing one of these particles is not marked for \isi{aspect}.
...
Conditional clauses\is{Clause!conditional} usually precede the \isi{main clause} as in these examples, though this is not a rigid rule. 

As \textit{mo} is a preverbal marker, it is always immediately followed by a verb. This means that \textit{mo} constructions would be impossible with nominal clauses\is{Clause!nominal}, or in contexts where a different preverbal marker is called for (e.g. the \isi{negation} \textit{kai}). In such cases, the existential verb \textit{ai}\is{ai ‘to exist’} can be employed as an \isi{auxiliary verb}. \textit{Ai} in turn is followed by a clause which is structured as a \isi{main clause} (this construction is further discussed in \sectref{sec:9.6.1}).
...
Occasionally preverbal \textit{mo} is used in main clauses. In these clauses, the subject\is{Subject!preverbal} is always expressed; the \isi{constituent order} is almost always SV/AVO. When the subject is a pronoun or \isi{proper noun}, it is marked with \textit{ko}. This structure reminds of clauses with \textit{ko}{}-marked topicalised\is{Topicalisation} subjects (\sectref{sec:8.6.2.1}).
...
\textit{Ana} occurs in the same structural position as \isi{aspect} markers\is{Aspect marker}; \textit{ana} and \isi{aspect} markers\is{Aspect marker} are mutually exclusive. Clauses marked by \textit{ana} are therefore not differentiated for \isi{aspect} (but see \REF{ex:11.184} below).
...
Whether the event will happen or not, may depend on a condition which is stated explicitly. Thus, \textit{ana} may occur in the apodosis, the clause expressing the consequence of a conditional\is{Clause!conditional} or \isi{temporal clause}\is{Clause!temporal}. 
...
In other cases, the question is not \textit{whether} the event in the \isi{subordinate clause} happens, but \textit{when}: the event is expected to happen or has already happened, and the same is true for the \isi{main clause} event dependent on it. However, \textit{ana} signals that the clause is still irrealis\is{Irrealis} in some way. It may indicate an event which takes or took place habitually (see the discussion about \REF{ex:11.178} above), or an event which is expected (with more or less \isi{certainty}) to take place in the future. \textit{Ana} is not used with events which have taken place at a definite moment in the past. 
...
Even though \textit{ki} is homophonous to the \isi{preposition} \textit{ki}, the two are probably etymologically distinct. The verbal marker \textit{ki} is probably derived from \is{Proto-Polynesian}PPN \textit{*kia}, which occurs in many languages with an optative and/or purposive sense.\footnote{\label{fn:524}\textit{Kia} was shortened to \textit{ki} in various languages. \citet[30]{Clark1976} mentions \ili{Kapingamarangi}, \ili{Nukumanu}, \ili{Sikaiana} and \ili{Luangiua}. \ili{Hawaiian} \textit{i} (\citealt[61]{ElbertPukui1979}) seems to represent the same \isi{particle}. As the \isi{particle} is \textit{kia}/\textit{{\ꞌ}ia} in most \is{Central-Eastern Polynesian}CE languages, the shortening to \textit{ki} in Rapa Nui must have been an independent development which took place after Rapa Nui broke off from \is{Eastern Polynesian}PEP (\sectref{sec:2.5.2} on the monophthongisation of particles). This process may have taken place relatively recently: there are a few occurrences of \textit{kia} in older texts, mostly in fossilised phrases such as \textit{ka oho,} \textit{kia tika} ‘go straight’ (Mtx-2-03.018; Mtx-6-07.014); see discussion in \citet[429]{Fischer1994}. Nowadays \textit{kia} survives in \textit{kiahio} ‘keep courage, be strong’ (cf. \textit{hiohio} ‘strong’).}  If this is correct, the \isi{preposition} and the verbal marker \textit{ki} were distinct lexemes in the protolanguage. However, because of the goal-oriented character of preverbal \textit{ki}, it is glossed ‘to’, just like the \isi{preposition}.
...
In the first place, after an \isi{imperative}\is{Imperative} or hortative. 
...
Secondly, when \textit{mo}\is{mo (preverbal)} would be potentially ambiguous. In \REF{ex:11.188}, the main verb \textit{pohe} is followed by a \isi{complement} clause marked with \textit{mo}. If the next clause were also marked with \textit{mo}, it could be read as a second \isi{complement} of \textit{pohe}; to ensure a reading as \isi{purpose clause}, \textit{ki} is used. The same happens in \REF{ex:11.189}: while the \textit{mo}{}-clause expresses the purpose of the preceding \isi{main clause}, the \textit{ki}{}-clause after that expresses the ultimate purpose, the higher-order goal of the preceding clauses as a whole.
...
\subparagraph{\ref{sec:11.5.3}.2} \textit{Ki} also marks \textsc{temporal} clauses\is{Clause!temporal}.\footnote{\label{fn:525}The double function of reflexes of PPN \textit{*kia} as both optative/purposive and temporal markers is also found with \ili{Māori} \textit{kia} (\citealt[62, 459]{Bauer1993}) and \ili{Tahitian} \textit{{\ꞌ}ia} (\citealt[138–139]{LazardPeltzer2000}); unlike Rapa Nui, in these languages the \isi{particle} is not used in temporal clauses referring to the past. In Rapa Nui, the purposive sense of \textit{*kia} has to a large degree been taken over by \textit{mo}, as discussed above.} As the examples below show, these occur in various contexts: with past\is{Past} reference, with future\is{Future} reference, or habitual\is{Aspect!habitual}. \textit{Ki}{}-clauses usually occur before the \isi{main clause}, but as \REF{ex:11.195} shows, they may also be placed after the \isi{main clause}.
...
\textit{Ki}{}-marked clauses may indicate a goal or temporal boundary: ‘until’. This occurs for example after the verb \textit{tiaki} ‘wait’.\footnote{\label{fn:526}In other contexts, ‘until’ is more commonly expressed by \textit{ka V rō}\is{ka (\isi{aspect} marker)!ka V rō}, and/or using \textit{{\ꞌ}ātā} (\sectref{sec:11.6.2.5}).}
...
The \isi{preposition} \textit{ki} has the same use, see \REF{ex:4.266} on p.~\pageref{ex:4.266}. This shows that the two particles \textit{ki}, though etymologically distinct, are closely related.
...
\is{o ‘lest’@{\ꞌ}o ‘lest’|(}
The preverbal marker \textit{{\ꞌ}o}\footnote{\label{fn:527}The origin of \textit{{\ꞌ}o} is unclear. It may be a reflex of \is{Proto-Polynesian}PPN \textit{*{\ꞌ}aua} ‘negative \isi{imperative}’, which occurs throughout Polynesia (Tongic, Samoic-Outlier and \is{Eastern Polynesian}EP). Cf. also Footnote \ref{fn:499} on p.~\pageref{fn:499} on the origin of the negator \textit{(e) ko}.

Another possible cognate is \ili{Tahitian} \textit{{\ꞌ}o}, which introduces clauses after “des verbes exprimant la crainte, la méfiance, et parfois l’eventualité” (verbs expressing fear, mistrust, and sometimes contingency), and which is followed by a nominalised verb\is{Verb!nominalised} (\citealt[197]{AcadémieTahitienne1986}). However, given the fact that Rapa Nui \textit{{\ꞌ}o} occurs in old texts already, it is relatively unlikely that it is a borrowing from \ili{Tahitian}\is{\ili{Tahitian} influence}.} indicates a consequence which is to be avoided. It can be translated as ‘lest’ or ‘so that ... not’.

\textit{{\ꞌ}O}{}-marked clauses usually occur after the \isi{main clause} and are always verb-initial. The subject is expressed in the same way as in main clauses: unmarked as in \REF{ex:11.200}, or with the agent marker \textit{e} as in \REF{ex:11.201}. 
...
In modern Rapa Nui, a verb marked with \textit{{\ꞌ}o} is usually followed by the asseverative \isi{particle} \textit{rō}\is{ro (emphatic marker)@rō (emphatic marker)} (\sectref{sec:7.4.2}), as illustrated in (\ref{ex:11.200}–\ref{ex:11.201}) above.

Occasionally \textit{{\ꞌ}o} is found in \isi{complement} clauses expressing a negative \isi{complement}: \textit{\mbox{ri{\ꞌ}ari{\ꞌ}a} {\ꞌ}o} ‘to fear lest’, \textit{haŋa {\ꞌ}o} ‘to want that not...’ \is{o ‘lest’@{\ꞌ}o ‘lest’|)}(see (\ref{ex:11.75}–\ref{ex:11.76}) in \sectref{sec:11.3.5}). 

\subsection{\textit{Mai} ‘before; while’}\label{sec:11.5.5}
\is{mai ‘while, before’|(}
\textit{Mai}, which is common as a \isi{preposition} ‘from’ (\sectref{sec:4.7.4}) and as a \isi{directional}\is{Directional} ‘movement towards deictic centre’ (\sectref{sec:7.5}), also occurs occasionally as a preverbal marker. It indicates an event prior to the event in the \isi{main clause}: ‘before’.
...
& {clause type} & {gloss} & {§}\\
\midrule
\textit{mo} & \isi{complement} & that & \ref{sec:11.3}\\
& purpose & to, in order to & \ref{sec:11.5.1.1}\\
& conditional & if & \ref{sec:11.5.1.1}\\
& \isi{main clause}: destined & be to, be destined to & \ref{sec:11.5.1.3}\\
\tablevspace
%\midrule
\textit{ana} & \isi{main clause}: intention & will, let’s & \ref{sec:11.5.2.1}\\
& \isi{main clause}: potential & may, might, could & \ref{sec:11.5.2.1}\\
& \isi{main clause}: apodosis & (if,) then & \ref{sec:11.5.2.1}\\
& \isi{main clause}: deontic & must; be allowed to & \ref{sec:11.5.2.1}\\
& \isi{main clause}: general practice & always, usually & \ref{sec:11.5.2.1}\\
...
& negative \isi{complement} & that not & \ref{sec:11.3.5}\\
\tablevspace
%\midrule
...
Adverbial clauses provide an adverbial modification of the \isi{main clause}. They can be constructed in various ways:
...
\item 
without any special marking. In this case, the relationship between the \isi{subordinate clause} and the \isi{main clause} is indicated by the aspectual marker\is{Aspect marker}, possibly in combination with certain postverbal particles;

\item 
using a nominal construction. Properly speaking, such a construction is not an \isi{adverbial clause}, but as it fulfils similar functions, it will be mentioned in this section as well.

\end{enumerate}

Type 3 clauses are subordinate, even though they lack a conjunction or subordinating marker; this is indicated by the fact that they are negated with the constituent negator \textit{ta{\ꞌ}e} (\sectref{sec:10.5.6}.5), not by a \isi{main clause} negator. Here is an example of a negated \isi{temporal clause}. Cf. also \REF{ex:11.257} on p.~\pageref{ex:11.257} (a \isi{reason clause} marked with \textit{he}).
...
\is{Clause!temporal|(}
A \isi{temporal clause}\is{Clause!temporal} is a \isi{subordinate clause} which provides a temporal framework for the event in the \isi{main clause}. Rapa Nui has a variety of \isi{temporal clause}\is{Clause!temporal} constructions. Some of these involve a conjunction\is{Conjunction} or a nominal construction; in others, the temporal relation is expressed by an aspectual marker\is{Aspect marker}.

\subsubsection[Cohesive clauses]{Cohesive clauses}\label{sec:11.6.2.1}
\is{Clause!cohesive|(}
In Rapa Nui discourse – especially in narrative – it is common to find an unmarked \isi{subordinate clause} at the beginning of a sentence, which provides a temporal framework for the \isi{main clause}. \citet[116]{WeberR2003} labels these \textit{cohesive}: they connect the events to the preceding context and provide a setting for the events that follow. Two examples: 
...
\begin{itemize}
\item 
They precede the \isi{main clause}.
...
As the examples above show, cohesive clauses\is{Clause!cohesive} marked with perfective \textit{i}\is{i (perfective)} express an event anterior to the event in the \isi{main clause} (\sectref{sec:7.2.4.2}), which provides the setting for the event in the \isi{main clause}.

Cohesive clauses marked with \isi{imperfective} \textit{e}\is{e (\isi{imperfective})} indicate events simultaneous to the event in the \isi{main clause}. They may be continuous\is{Aspect!continuous} as in \REF{ex:11.213} or habitual\is{Aspect!habitual} as in \REF{ex:11.214}:
...
The contiguity marker \textit{ka}\is{ka (\isi{aspect} marker)} in cohesive clauses\is{Clause!cohesive} expresses temporal contiguity\is{Simultaneity}: the event in the \isi{subordinate clause} marks the starting point of the event in the \isi{main clause}.
...
Perfect \isi{aspect} \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} in cohesive clauses\is{Clause!cohesive} (as in main clauses) expresses a state resulting from a process. In cohesive clauses\is{Clause!cohesive}, \textit{ko V {\ꞌ}ā} only occurs with stative verbs\is{Verb!stative}.
...
\subparagraph{Imperfective \textit{e}} Temporal clauses may also be marked with \isi{imperfective} \textit{e}\is{e (\isi{imperfective})}. These clauses express a continuous event \is{Simultaneity}simultaneous to the one in the \isi{main clause}. As discussed in \sectref{sec:7.2.5.4}, \textit{e-}marked verbs in main clauses are followed either by a postverbal demonstrative\is{Demonstrative!postverbal} (PVD) or the continuity marker \textit{{\ꞌ}ā/{\ꞌ}ana}. The same is true in temporal clauses\is{Clause!temporal}: the verb is either followed by a PVD\is{Demonstrative!postverbal} \is{e (\isi{imperfective})!e V PVD}as in (\ref{ex:11.224}–\ref{ex:11.225}), or by \textit{{\ꞌ}ā/{\ꞌ}ana}\is{e (\isi{imperfective})!e V {\ꞌ}ā} as in (\ref{ex:11.226}–\ref{ex:11.227}).
...
Though all these clauses are similar in function, there is a difference between clauses marked with \textit{e V PVD}\is{Demonstrative!postverbal} and the ones marked with \textit{e V {\ꞌ}ā}\is{e (\isi{imperfective})!e V {\ꞌ}ā}. The constructions with a PVD\is{Demonstrative!postverbal} can be characterised as true temporal clauses\is{Clause!temporal}, indicating an event which takes place at the same time as the main event. The clauses with \textit{{\ꞌ}ā} are more like circumstantial\is{Clause!circumstantial} or manner clauses, further defining the nature of the event in the \isi{main clause} or the manner in which it takes place. They have less the character of an independent event and can often be translated with a participle.
...
\begin{enumerate}
\item 
With \textit{{\ꞌ}ā}, the subject is always the same as in the \isi{main clause}; in the PVD\is{Demonstrative!postverbal} construction, the subject can be different, as in \REF{ex:11.224}.
...
\subparagraph{Contiguity marker \textit{ka}} Subordinate clauses marked with the contiguity marker \textit{ka} indicate an event which is simultaneous\is{Simultaneity} with the event expressed in the \isi{main clause}: \is{ka (\isi{aspect} marker)}
...
As these examples show, the \isi{subordinate clause} may precede or follow the \isi{main clause}. As in \REF{ex:11.229}, the verb is often followed by a postverbal demonstrative\is{Demonstrative!postverbal}.
...
\is{Conjunction}
Temporal adjuncts can be expressed by a temporal noun preceded by a \isi{preposition}; the most general temporal noun is \textit{hora} ‘time’. The adjunct can be further specified by a modifier, e.g. a genitive as in \REF{ex:11.230} or a \isi{relative clause}\is{Clause!relative} as in \REF{ex:11.231}:
...
Now as discussed in \sectref{sec:5.3.2.3}, the article can be omitted before clause-initial nouns followed by a demonstrative like \textit{era}. At the same time, the \isi{preposition} \textit{{\ꞌ}i} can be omitted as well. This results in constructions like the following:
...
In the constructions above, \textit{hora ena/era} resembles a temporal conjunction\is{Conjunction}; semantic bleaching is taking place, where \textit{hora ena/era} comes to mean little more than ‘when’. Notice however, that the construction is syntactically still a nominal phrase with \isi{relative clause}\is{Clause!relative}: as \REF{ex:11.233} shows, the aspectual can be omitted, something which is only possible in relative clauses\is{Clause!relative} (\sectref{sec:11.4.5}). (Also, the verb \textit{take{\ꞌ}a} has been raised from the \isi{relative clause}\is{Clause!relative}.)

\subsubsection{Anteriority: ‘before’}\label{sec:11.6.2.4}

Rapa Nui has a variety of devices to express that the event in the \isi{subordinate clause} takes place prior to the event in the \isi{main clause}. One of these is preverbal \textit{mai}, discussed in \sectref{sec:11.5.5}. The following strategies are also used:
...
\subparagraph{\textit{Ante}}\is{ante ‘before’} \textit{Ante} ({\textless} Sp. \textit{antes}) is used as an \isi{adverb}\is{Adverb} meaning ‘before, earlier, previously’. It is also used as a conjunction\is{Conjunction}, followed by \textit{ki} + nominalised verb\is{Verb!nominalised}:
...
In the examples above, the subject of the \isi{main clause} reaches a certain state or end point; for example, in \REF{ex:11.238}, Te Manu reaches a state of satiation after having eaten. The stative verb in the \textit{ka V rō}\is{ka (\isi{aspect} marker)!ka V rō} clause may also specify the action of the \isi{main clause}, which is performed – or is to be performed – to a certain extent or in a certain way. (Cf. the use of \textit{ka} before numerals to mark an extent, \sectref{sec:4.3.2.2}).
...
A second way to express ‘until’ is by means of \textit{{\ꞌ}ātā}\is{ata ‘until’@{\ꞌ}ātā ‘until’} ({\textless} Sp. \textit{hasta}). \textit{{\ꞌ}Ātā} is used in nominal constructions before the \isi{preposition} \textit{ki} (see \REF{ex:4.267} on p.~\pageref{ex:4.267}), but also in verbal constructions, followed by \textit{ka V rō}\is{ka (\isi{aspect} marker)!ka V rō}. As \REF{ex:11.242} shows, \textit{{\ꞌ}ātā} may be shortened to \textit{{\ꞌ}ā}:
...
Purpose clauses\is{Clause!purpose} are often marked with preverbal \textit{mo} (\sectref{sec:11.5.1.1}) or \textit{ki} (\sectref{sec:11.5.3}). Purpose may also be expressed by a bare verb, i.e. a verb without \isi{aspect} marker\is{Aspect marker}.\footnote{\label{fn:532}Clauses with a bare verb cannot be analysed as juxtaposed main clauses, as \isi{main clause} verbs always have an \isi{aspect} marker, except occasionally when the verb is followed by certain postverbal particles (\sectref{sec:7.2.2}).} This verb is always initial in the clause. Bare purpose clauses\is{Clause!purpose} are found especially after motion verbs\is{Verb!motion}. A few examples:
...
More commonly, the purpose of an action is expressed by a \isi{noun phrase} introduced by the \isi{preposition} \textit{ki}, followed by a bare verb. Here are a few examples:
...
In these examples, the main verb is a \isi{motion verb}\is{Verb!motion}; the \textit{ki}{}-marked \isi{noun phrase} is the Goal of movement. This \isi{noun phrase} is followed by a bare verb, of which the preceding noun is the Patient.\footnote{\label{fn:533}\citet[424]{Clark1983Review} points out that the same construction occurs in \ili{Marquesan} and \ili{Mangarevan}. Different from what Clark suggests, in Rapa Nui this construction is not limited to generic objects, as \REF{ex:11.247} shows.}  

The noun in this construction is not an incorporated object\is{Object!incorporation} of the following verb: it is the head of a regular \isi{noun phrase}, marked with the article \textit{te} and preceded by a \isi{preposition}. A somewhat more plausible analysis would be to consider the verb as incorporated into the noun; however, the \isi{directional}\is{Directional} \textit{mai} in \REF{ex:11.247} shows that the verb is the head of a true verb phrase. It is best to analyse these constructions simply as a combination of a \isi{noun phrase} and a bare \isi{purpose clause}, rather than assuming that the \isi{noun phrase} + verb are a single constituent. An additional reason to do so is that this construction is not an isolated phenomenon, but an instance (admittedly, the most common instance) of a group of constructions in which a locative \isi{noun phrase} and a \isi{purpose clause} occur together. Related constructions include:
...
\begin{itemize}
\item 
a Source \isi{noun phrase} (with \isi{preposition} \textit{mai} ‘from’) followed by a bare verb:
\end{itemize}
...
\begin{itemize}
\item 
a \textit{ki}{}-marked Goal \isi{noun phrase} followed by a \textit{mo}{}-marked \isi{purpose clause}:
\end{itemize}
...
\begin{itemize}
\item 
a \textit{ki}{}-marked Goal \isi{noun phrase}, with the associated action left implicit:
\end{itemize}
...
\begin{itemize}
\item 
a \textit{mo}{}-marked Goal \isi{noun phrase} followed by a \isi{purpose clause} (the latter may be either bare or marked with \textit{mo}):
\end{itemize}
...
These examples suggest that \textit{ki te N V} in (\ref{ex:11.246}–\ref{ex:11.248}) should not be analysed as a special construction involving a single NP+V constituent. Rather, it is a combination of two constituents, a nominal Goal phrase followed by a bare \isi{purpose clause}, either of which may also occur on its own. 
\is{Clause!purpose!bare|)}
...
Secondly, in modern Rapa Nui, the phrase \is{mee ‘thing’@me{\ꞌ}e ‘thing’!{\ꞌ}i te me{\ꞌ}e ‘because’}\textit{{\ꞌ}i te me{\ꞌ}e (era)} (lit. ‘in the thing’ or ‘because of the thing’) is used as a conjunction\is{Conjunction} introducing a \isi{reason clause}. As the examples show, the \isi{reason clause} either precedes or follows the \isi{main clause}.
...
Thirdly, the \isi{reason clause} may also be a \isi{subordinate clause} marked with the aspectual \textit{he}\is{he (nominal predicate marker)}. That this is a \isi{subordinate clause}, is shown by the fact that it is negated with the constituent negator \textit{ta{\ꞌ}e}\is{tae (negator)@ta{\ꞌ}e (negator)} (\sectref{sec:10.5.6}); main clauses would have a different negator. 
...
In these constructions, \textit{he} can also be considered as a nominal predicate marker followed by a nominalised verb (hence the double gloss in the examples above)\is{Verb!nominalised}. One reason to do so, is that other nominal constructions are also used to express reasons: in \REF{ex:11.258} a nominalised verb\is{Verb!nominalised} preceded by a possessive pronoun, in \REF{ex:11.259} a subordinate existential construction (an existential \isi{main clause} would be \textit{{\ꞌ}Ina he me{\ꞌ}e mo kai}):
...
\is{Clause!result|(}Results may be marked by the adverbial connector \textit{{\ꞌ}o}\is{o ‘because of’@{\ꞌ}o ‘because of’} \textit{ira}\is{ira (anaphor)} ‘because of that; therefore’ (the reason \isi{preposition} \textit{{\ꞌ}o} followed by the pro-form \textit{ira}). As \REF{ex:11.261} shows, it is possible to mark both the \isi{reason clause} (in this case, a nominal construction) and the result clause.
...
The aspectual marker\is{Aspect marker} \textit{ka}\is{ka (\isi{aspect} marker)}, in combination with the \isi{directional} \textit{atu}\is{atu ‘away’}, can be used in a concessive sense, indicating a circumstance which might be expected to prevent – but actually does not prevent – the event in the \isi{main clause}.\footnote{\label{fn:535}This does not mean that all \textit{ka V atu} constructions have a concessive sense, see e.g. example \REF{ex:11.263} above.}
...
A second way to express concession is by means of the \isi{preposition} \textit{nōatu}\is{noatu ‘no matter’@nōatu ‘no matter’},\footnote{\label{fn:537}\textit{Nōatu} {\textless} \textit{nō} ‘just’ + \textit{atu} ‘away’, but probably borrowed as a whole from \ili{Tahitian} \textit{noātu} (\citealt[310]{AcadémieTahitienne1986}).} followed by a nominalised verb\is{Verb!nominalised}:
...
Finally, concession is expressed by the adverbial expression \is{mee ‘thing’@me{\ꞌ}e ‘thing’!te me{\ꞌ}e nō ‘however’}\textit{te me{\ꞌ}e nō} ‘however, even so’, which functions as a \isi{coordinating conjunction} (\sectref{sec:5.8.2.4}):
...
When circumstances are states rather than events, they tend to be expressed in a clause in the perfect \isi{aspect} (\textit{ko V {\ꞌ}ā}), without a special marker. 
...
Perfect \isi{aspect}\is{Aspect!perfect} clauses expressing circumstances are especially common in the construction \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} \textit{e V era}\is{e (\isi{imperfective})!e V PVD}. In this construction, the second clause is marked with \textit{e V era}\is{e (\isi{imperfective})!e V PVD} and expresses an action, while the preceding \textit{ko V {\ꞌ}ā} clause expresses a quality (e.g. a feeling or attitude) possessed by the subject performing the action. Even though \textit{e V era}\is{e (\isi{imperfective})!e V PVD} in general expresses durative\is{Aspect!durative} actions, in this construction it is not necessarily durative\is{Aspect!durative}. 
...
Notice that \textit{e V era}\is{e (\isi{imperfective})!e V PVD} is obligatory when the circumstantial \textit{ko V {\ꞌ}ā}\is{ko V {\ꞌ}ā (perfect \isi{aspect})} clause comes first; when the circumstantial clause follows the \isi{main clause}, the \isi{main clause} may be \textit{he}{}-marked, as in (\ref{ex:11.274}–\ref{ex:11.275}) above.
\is{Clause!circumstantial|)}

\subsection{Summary}\label{sec:11.6.9}

Events which modify the event in the \isi{main clause}, can be expressed in several ways. Certain interclausal relationships are expressed using a subordinating marker or conjunction. In other cases no special marker is used; even so, the modifying clause is subordinate, as is shown by the fact that these clauses are negated by the subordinate negator \textit{ta{\ꞌ}e} rather than a \isi{main clause} negator. The various strategies are summarised in \tabref{tab:68}.
...
This chapter has explored the ways in which clauses are combined. A common way to combine clauses is simple juxtaposition. In fact, older Rapa Nui did not have any \isi{coordinating conjunction}. In modern Rapa Nui \textit{{\ꞌ}e} ‘and’ is used, but juxtaposition is still the default strategy for coordinating clauses. Juxtaposition is not only used to express sequential events, but also to express semantic complements of the verbs \textit{ha{\ꞌ}amata} ‘begin’ and \textit{hōrou} ‘hurry’. 

Rapa Nui has various strategies to express the argument of a matrix verb. Only some of these involve a proper \isi{complement} clause, i.e. a clause which is syntactically dependent on the main verb; they may involve the subordinating marker \textit{mo} ‘for, in order to’, or a nominalised \isi{complement}. Other verbs are followed by a juxtaposed clause or an independent clause.

The subordinator \textit{mo} marks both \isi{complement} clauses and adverbial clauses; interestingly, it marks both purpose and condition. The marker \textit{ana} has an even wider range of functions, all of which can be characterised as irrealis: an \textit{ana-}marked clause refrains from claiming the truth of the proposition expressed. \textit{Ana}{}-marked clauses express intentions, potential events and obligations, but also general truths. In subordinate clauses, \textit{ana} marks conditional clauses and dependent questions.

Relative clauses in Rapa Nui are not marked by a conjunction or preverbal marker, but they have various distinctive properties: they are invariably verb-initial and the choice of aspectuals is limited. A peculiar feature is, that the \isi{aspect} marker may be left out (in most other clause types, unmarked verbs are rare or nonexistent). In these “bare relative clauses”, the verb is often raised to a position immediately after the head noun, before any postnominal markers.

A wide range of constituents can be relativised; most of these are not expressed in the \isi{relative clause}, others are expressed as a pronoun. The distribution of these two constructions does not entirely conform to the \isi{noun phrase} hierarchy proposed by \citet{KeenanComrie1977}: while subjects, objects and adjuncts are left unexpressed, oblique arguments (which are higher in the hierarchy than adjuncts) are expressed pronominally, just like constituents low in the hierarchy like possessors.

There is a tendency to express the entity which is subject of the \isi{relative clause} as a \isi{possessor} before or after the head noun: ‘your thing [did yesterday]’ = ‘the thing you did yesterday’. Syntactically there is nothing special about these constructions: the \isi{possessor} is no different from other possessors in the \isi{noun phrase}; the \isi{relative clause} is no different from other relative clauses, apart from the fact that the subject is not expressed.
