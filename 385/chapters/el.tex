...
\begin{table}[H]
	...
	\begin{tabular}{ llll }
\lsptoprule		/i/ <\armenian{ի}> & /ʏ/ <\armenian{իւ}> & /ui̯/ <\armenian{ուⁱ}> & /u/ <\armenian{ու}> \\
...
The consonants are likewise rich with some new sounds as in Table \ref{tab:Maragha:phono:segment:cons}. 
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll ll }
		\lsptoprule & \multicolumn{2}{l }{Classical Armenian} &\multicolumn{2}{l }{> Maragha} & \multicolumn{2}{l }{cf. SEA} \\ 
...
Classical Armenian /oi̯/ <\armenian{ոյ}> became /ʏ, ui̯/ <\armenian{իւ, ուⁱ}> (Table \ref{tab:Maragha:phonology:soundChange:diph:oi}).
 ...
 \translatorHD{Adjarian provides the CA word `sleep' /kʰun/ <\armenian{քուն}>, but I think this is a mistake because it does not have a diphthong /oi̯/. (?)}
...
The consonant changes are exactly the same as in the dialects of Van or Karabakh. The Classical Armenian sound /h/ <\armenian{հ}> is always /χ/ <\armenian{խ}>. 

\section{Morphology}
\subsection{Noun inflection or declension}

\subsubsection{Vowel harmony}
In the grammar, everything is established based on the rule of analogy. Nominal and verbal formatives and endings change their vowels according to the vowel that is contained in the root of the word. For example, the definite article becomes /-ɑ/ <\armenian{ա}> if the vowel of the word-final syllable is /ɑ/ <\armenian{ա}> or /u/ <\armenian{ու}>. But it becomes /-æ/ <\armenian{ա̈}> if that vowel is /æ, e, ʏ/ <\armenian{ա̈}, \armenian{է, իւ}>. The genitive formative is /-ə/ <\armenian{ը}> if the vowel of the word-final syllable is /ɑ/ <\armenian{ա}> or /ə/ <\armenian{ը}>. But that formative becomes /-ʏ/ <\armenian{իւ}> if the vowel is /ʏ/ <\armenian{իւ}> or /œ/ <\armenian{էօ}>. It also becomes /-u/ <\armenian{ու}> when in front of the vowels /u, o/ <\armenian{ու, օ}>, and it becomes /-i/ <\armenian{ի}> in front of the vowel /i/ <\armenian{ի}>. Even the copular verb is subject to these assimilatory changes. 
...
This dialect has the following cases: nominative, genitive-dative, accusative, ablative, and instrumental. There is no locative; the accusative is like the /um/ <\armenian{ում}> branch. The ablative is formed with the formative /-en/ <\armenian{էն}>. 
...
As we said above, in the /el/ \armenian{ել} branch, the present stem is formed based on the verb's infinitive, by combining it or conjugating it with the auxiliary verb (\ref{sent:Maragha:morpho:verb:infconj}). 
...
\translatorHD{In SEA (Table \ref{tab:Maragha:morpho:verb:paradigm:presentIndc}), the indicative present is formed by  combining a non-finite form of the verb (called the imperfective converb with the suffix /-um/) with the present auxiliary. In Maragha, we see a similar periphrastic approach. However, the non-finite '.s based on the verb's infinitive. The auxiliary seems to then be cliticized onto the verb. Note how the two dialects diverge in the form of the auxiliary: /e/ for SEA, but /e, i/ for Maragha.}
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll }
		\lsptoprule & \multicolumn{2}{l }{Maragha `to like'} & \multicolumn{2}{l }{cf. SEA `to want'} \\
...
& \multicolumn{2}{l }{`I like'} & \multicolumn{2}{l }{`I want'} \\
		...
		\\
		& \multicolumn{2}{l }{$\sqrt{}$-{\thgloss}-{\infgloss}-{\aux}-{\agr}}& \multicolumn{2}{l }{$\sqrt{}$-{\impfcvb} {\aux}-{\agr}}\\ \lspbottomrule
	\end{tabular}
\end{table}
...
\begin{table}[H]
	\centering
	\caption{Indicative past iimperfective <\armenian{անկատար}> in the Maragha dialect}
...
& \multicolumn{2}{l }{`I was liking'} & \multicolumn{2}{l }{`I was wanting'} \\
		...
		\\
		& \multicolumn{2}{l }{$\sqrt{}$-{\thgloss}-{\infgloss}-{\aux}-{\agr} {\pst}}& \multicolumn{2}{l }{$\sqrt{}$-{\impfcvb} {\aux}-{\pst}-{\agr}}\\ \lspbottomrule
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	{% \resizebox{\textwidth}{!}{%
			\begin{tabular}{ l ll ll }
				\lsptoprule & \multicolumn{2}{l }{Maragha `to like' (Form I)} & \multicolumn{2}{l }{cf. SEA `to want'} \\
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
				...
				\midrule 
				 & \multicolumn{2}{l }{Maragha `to like' (Form II)} & \multicolumn{2}{l }{cf. SEA `to want'} \\
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
...
\translatorHD{Morphologically, the non-finite form seems to be constructed by adding the suffix /-u/ to the infinitive; the theme vowel becomes /o/. This non-finite form seems to be a cognate of the SEA future converb as in (\ref{sent:Maragha:morpho:verb:fut:sea}), and thus I gloss /-u/ as {\futcvb}. The 3SG has a covert auxiliary.}
...
\begin{table}[H]
	\centering \caption{Complex future forms of the verb `to like' in the Maragha dialect}
	\label{tab:Maragha:morpho:verb:futComplex}
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{ l ll ll }
		\lsptoprule & \multicolumn{2}{l }{Complex future <\armenian{բարդ ապառնի}>}& \multicolumn{2}{l }{Past version <\armenian{անցեալ}>} \\ 
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
...
\translatorHD{In Maragha, the future and past future are constructed from the subjunctive (Table \ref{tab:Maragha:morpho:verb:paradigm:complexSubjunctive}). For the verb `to like', we simply add the future prefix /k-/. SEA behaves essentially the same and I do not provide its paradigm.}


\begin{table}[H]
	\centering
	\caption{Forms that are constructed from the subjunctive forms of the verb `to like' in the Maragha dialect}
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll l }
		\lsptoprule & \multicolumn{2}{l }{Maragha `like!'} & \multicolumn{2}{l }{cf. SEA `want!'} & \\
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll l }
		\lsptoprule & \multicolumn{2}{l }{Maragha `do not like!'} & \multicolumn{2}{l }{cf. SEA `do not want!'} & \\
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
...
\translatorHD{On the original page, Adjarian did not list any participles or non-finite forms. But on page 306, Adjarian provides an erratum with the following non-finite forms (Table \ref{tab:Maragha:morpho:verb:paradigm:participle}).}
...
\translatorHD{Adjarian means that the Armenian possessive article here is acting as an object clitic. See \citet[\S7.1]{DolatianEtAl-prep-IranianGrammar} for similar data from other Iranian Armenian dialects.}
...
Adjarian's note: Communicated by Mr. Kaloust Iskenderian (\armenian{պր. Գալուստ Իսքէնդէրեան}; \translatorHD{SEA: /ɡɑlust iskʰendeɾjɑn/}), a provincial inspector of Urmia schools. 
...
The dialect of Khoy has still not been studied. There are writings in this dialect in \citetitle{Eminian} volume 2 (\armenian{Բ}.), page 300-304 and volume 4 (\armenian{Դ}), page 343-350. What is more important are N. Ter Avetikian's \armenian{«Ոտանաւոր աշխատութիւններ եւ Նշանագրութիւն Պարսկաստանից գաղթած Խոյեցւոց բարբառով» (Վաղարշապատ} 1900) and \armenian{«Բանաստեղծութիւններ եւ Կիրակոսի հարսանիքը» (Վաղարշապատ} 1903).\footnote{\translatorHD{Unfortunately, I could not track down these two bibliographic items, and thus could not add them to the bibliography. Furthermore, the page quality makes it unclear if the fourth word is \armenian{Նշանադրութիւն} or \armenian{Նշանագրութիւն}. (?)}}

By examining these excerpts, it seems that the dialect of Khoy occupies a middle position between the dialects of Maragha and Van. Its grammatical structure is the same as in the Maragha dialect, but its phonological rules are like those of the Van dialect. In other words, the Khoy dialect is closer to Classical Armenian than Maragha is. 

Because we think it is unneeded to further discuss these simplified phenomena, we direct the reader to the subsequent text samples. 
...
Adjarian's note: See \armenian{Ն. Տէր-Աւետիքեանի, Ոտանաւոր աշխատութիւններ եւ նշանագրութիւն} (\translatorHD{\armenian{նշանադրութիւն}(?)}), \armenian{էջ} 46-49. 
...
The Classical Armenian diphthong /ɑi̯/ <\armenian{այ}> becomes /e/ <\armenian{է}> (Table \ref{tab:Artvin:phonology:soundChange:diph:ai}). 


\begin{table}[H]
	\centering
	\caption{Change from Classical Armenian /ɑi̯/ <\armenian{այ}> to /e/ <\armenian{է}> in the Artvin dialect}
	\label{tab:Artvin:phonology:soundChange:diph:ai} 
	\begin{tabular}{ l ll ll ll }
		\lsptoprule & \multicolumn{2}{l }{Classical Armenian} &\multicolumn{2}{l }{> Artvin} & \multicolumn{2}{l }{cf. SEA} \\ 
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
...
The Classical Armenian word /hɑmɑɾ/ <\armenian{համար}> `for' has become /hɑmɑ/ <\armenian{համա}>, like in Tbilisi.

\subsubsection{Loss of initial /v/ in `on'}

An interesting phenomenon is the loss of the sound /v/ <\armenian{վ}> from the Classical Armenian word /veɾɑi̯/ <\armenian{վերայ}> (\translatorHD{cf. SWA: /vəɾɑ/ <\armenian{վրայ}>}), which has become /ɾɑ/ <\armenian{րա}> (\ref{sent:Artvin:phono:change:vra}). 
...
The Classical Armenian sound /h/ <\armenian{հ}> does not become /χ/ <\armenian{խ}>, unlike the dialects of Maragha and Khoy.
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll }
		\lsptoprule &\multicolumn{2}{l }{Artvin} & \multicolumn{2}{l }{cf. SWA} \\ 
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5}\ 
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{ l ll ll ll }
		\lsptoprule &\multicolumn{2}{l }{Artvin} & \multicolumn{2}{l }{cf. SWA}& \multicolumn{2}{l }{cf. SEA} \\ 
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
...
\begin{table}[H]
	...
	\begin{tabular}{ l ll ll }
		\lsptoprule &\multicolumn{2}{l }{Artvin} & \multicolumn{2}{l}{cf. SEA} \\ 
		 \cmidrule(lr){2-3}\cmidrule(lr){4-5} 
...
\begin{exe}
	...
	\begin{xlist}
		...
		search-{\thgloss}-{\infgloss}-{\impfcvb} {\aux}-3{\sg} \\
		\trans `He searches.'\\
		\armenian{փնտռէլիս է}
...
\translatorHD{In SEA, one way to form the future is to add the prefix /k(ə)-/ to the finite verb. It seems that Artvin uses a different prefix form /ku/. In SEA, the formative /k(ə)/ is immediately before the verb. But in Artvin, other words can intervene between the /ku/ and the verb (\ref{sent:Artvin:morpho:verb:fut:kumobile}).}
...
\translatorHD{Adjarian did not explain what the following sentences meant Nikita Bezrukov suggested the following translations based on his knowledge of relevant dialects. I thank him for his help. Though without an Artvin speaker to double check, we cannot be fully sure. (?)}
...
\ex \begin{xlist}
		...
		\trans `They will look.' \\
		\armenian{կու աշին}
	\ex SEA\footnote{\translatorHD{The verb `to look' in SEA does not start with a vowel. The Artvin datapoint shows how Artvin uses a single morph /ku/ before both consonants and vowels, while SEA uses [k-, kə-] based on the type of the following segment.}} \\
\gll k-ɑbr-e-n \\
{\fut}-live-{\thgloss}-3{\pl} \\
\trans `They will live.' \\
\armenian{կապրեն}
\end{xlist}
\ex `He will go outside.' \label{sent:Artvin:morpho:verb:fut:kumobile} 
\begin{xlist}
		\ex Artvin \\ 
...
In both the perfective and the future, the Classical Armenian theme vowel /e/ <\armenian{ե}> changes to /i/ <\armenian{ի}> (\ref{sent:Artvin:morpho:verb:theme}). 
...
\translatorHD{I think what Adjarian means is that these Artvin imperatives seem to be derived from subjunctive verbs; the Istanbul verbs seem subjunctive based on how they would be interpreted in SWA.}
