...
\ChapterDOI{10.5281/zenodo.7353609}
...
\keywords{Negative existential Cycle. Nominalization. Grammaticalization. Ancient Egyptian}
...
% \todo{the D character should be a lower case d with line below (unicode 1E0F)}
...
% \todo[inline]{D should be lower case d with a line below}
...
% \todo{in sDm-f, D should be lower case d with line below}
\ea \label{ex:AE36}
...
% \todo{the \# should be a small 3 for aleph, please correct}
...
\end{description}
%  \todo{Please make sure the suggested phases appear on the same page}
...
\item[(ii)] As was already mentioned in the preceding \sectref{s:AE2}, a new type C never obtains in Middle Egyptian or in Late Egyptian, for \textit{bn} no longer functions as a negative existential when its development as a negator reaches its maximal extension and becomes something close to the Standard Negation in the language. Moreover, its pathway of change was even shown to be the same as was the case for \textit{ni} in the preceding evolution from type C to type A: the relaxing of the constraint of indefinite reference on the subject made \textit{nn} capable of expressing the negation in all sorts of stative predications. From there, its spread to the nominal predication appears to fall within the scope of analogy more than anything else. On the other hand, its use with a modal verbal form (sometimes called subjunctive \textit{sḏm-f}) also used in positive modal main clauses by insubordination may indeed be considered to derive from a need for emphasis. Such an explanation would account for its gradual replacing of the more ancient modal construction based on the negation \textit{ni} and a nominalization. However, as seen in \sectref{s:AE2}, there is a gap in the attestation of the spoken register in Earlier Egyptian that allows for an unattested intermediary step *\textit{ni wn} subjunctive \textit{sDm-f}. In that case, the construction using the dedicated negative existential \textit{nn} would be inherited, deriving from a former construction involving the existential verb and the Standard Negation. As a consequence, it would seem questionable to speak of the negative existential extending to Standard Negation.\footnote{In such cases, \citet{Veselinova2014} states that the cycle is not operational.}
...
% \todo{Please make sure the table is placed before the references and not in the middle of them}
\begin{table}[t]
