...
\multirow{2}{*}{Old Chinese} & \begin{tabular}[c]{@{}l@{}}《論語》\\ The Analects\end{tabular} & \begin{tabular}[c]{@{}l@{}}480–350 BC\\ Warring States period\end{tabular} & \begin{tabular}[c]{@{}l@{}}Dialogue\\ collection\end{tabular} \\ \cmidrule(l){2-4} 
 & \begin{tabular}[c]{@{}l@{}}《史記》\\ Shiji\end{tabular} & \begin{tabular}[c]{@{}l@{}}109–91 BC\\ Western Han\end{tabular} & History \\ \midrule
\multirow{2}{*}{Middle Chinese} & \begin{tabular}[c]{@{}l@{}}《三國志》\\ Records of the\\ Three Kingdoms\end{tabular} & \begin{tabular}[c]{@{}l@{}}AD 265–300\\ Wei-Jin period\end{tabular} & History \\ \cmidrule(l){2-4} 
 & \begin{tabular}[c]{@{}l@{}}《世說新語》\\ A New Account\\ of the Tales of\\ the World\end{tabular} & \begin{tabular}[c]{@{}l@{}}420–581\\ Southern \&\\ Northern dynasties\end{tabular} & \begin{tabular}[c]{@{}l@{}}Short\\ stories\end{tabular} \\ \midrule
\multirow{4}{*}{\begin{tabular}[c]{@{}l@{}}Pre-Modern\\ Chinese\end{tabular}} & \begin{tabular}[c]{@{}l@{}}《太平廣記》\\ Taiping Guangji\end{tabular} & \begin{tabular}[c]{@{}l@{}}977–978\footnotemark\\ Northern Song\end{tabular} & Anthology \\ \cmidrule(l){2-4} 
...
collection were written during the time of the Tang dynasty (AD 618–907).}\todo{this footnote should appear after Table 8.5, not prior to it as is in the current compiled version}
...
Based on the selected texts, 弗\todo{Please make sure that this Chinese character prints} \textit{fú} and 蔑 \todo{Please make sure that this Chinese character prints}\textit{miè} never co-occurred with the existential predicate. Three others also rarely occurred with the existential predicate, namely 勿 \todo{Please make sure that this Chinese character prints} \textit{wù}, 匪 \todo{Please make sure that this Chinese character prints} \textit{fěi} and 毋 \todo{Please make sure that this Chinese character prints}\textit{wú}. The first two only appeared with the existential predicate less than ten times in the eight selected texts, and the last one, 毋 \todo{Please make sure that this Chinese character prints} \textit{wú}, only appeared with the existential predicate \textit{yǒu} `have' in one text – \emph{Shiji} with twelve tokens (that is, 7\% of the total \textsc{neg}+\textsc{have} tokens in the text). Excluding these five negative markers, the pattern that emerged is represented in Figure \ref{fig:lam1}. \todo{fig:lam1 should come immediately after this sentence}\footnote{In Figures \ref{fig:lam1} and \ref{fig:lam2}, the numerals next to the Pinyin stand for tones: 1 refers to a high level tone, 2 to a rising tone, 3 to a dipping tone, and 4 to a falling tone.} 
...
Example (\ref{ex:lam19a}) is a clear case in point. The parallelism of the two sentences is deliberately used to highlight the contrast in content. The first clause in the first sentence is `when father lives', and in the second sentence, the first clause expresses the opposite, `when father dies', and the meaning of `to die' is encoded by \textit{mò}. At a glance, example (\ref{ex:lam19c}) appears to present a case of \textit{mò yǒu} (also known as \textit{méiyǒu}), where \textit{yǒu} is the possessive predicate and 沒\todo{Please make sure that this Chinese character prints} \textit{méi}/\textit{mò} is the negator, but this would be a misinterpretation. Similar to example (\ref{ex:lam19a}), this sentence contains two clauses with parallel structure, expressing a contrastive meaning: the first clause states that the person in question (although pro-dropped) attains remarkable achievements when he lives, and the second clause contrasts that by stating what he possesses when he dies. In both clauses, the verb \textit{yǒu} `have' means `to possess/own', and 沒\todo{Please make sure that this Chinese character prints} \textit{méi}/\textit{mò} in the second clause means `dead' (hence it is glossed as \textit{mò}, not \textit{méi}), the opposite of \textit{shēng} `live' in the first clause. 
...
As far as Hong Kong Cantonese is concerned, \citeauthor{Zhang2002}'s discovery has been supported by \citet{Law2014} where the phonological process involved is suggested to be as follows: \todo{ex:lam31 should come right after this sentence. The figure is messed up in the compilation from March 18, 2021}
...
\begin{figure}[!h] \todo{make sure the figure with periodization prints. There are two png files because the table is really big}
    \captionsetup{name=Proposed periodisation of the Chinese language}
    \includegraphics[width=\textwidth]{figures/fig_lamA1a.png}
    \includegraphics[width=\textwidth]{figures/fig_lamA1b.png}
...
\begin{figure} \todo{Please make sure that the map has a landscape orientation and is spead across the page}
  \includegraphics[height=.5\textheight]{figures/NEC_in_Chinese.jpg}
...
\begin{sidewaystable}[]
	...
	\lsptoprule
	\textbf{Texts} & \begin{tabular}[c]{@{}l@{}}勿\\ \textit{wù}\end{tabular} & \begin{tabular}[c]{@{}l@{}}毋\\ \textit{wú}\end{tabular} & \begin{tabular}[c]{@{}l@{}}弗\\ fú\end{tabular} & \begin{tabular}[c]{@{}l@{}}匪\\ \textit{fěi}\end{tabular} & \begin{tabular}[c]{@{}l@{}}非\\ \textit{fēi}\end{tabular} & \begin{tabular}[c]{@{}l@{}}未\\ \textit{wèi}\end{tabular} & \begin{tabular}[c]{@{}l@{}}微\\ \textit{wēi}\end{tabular} & \begin{tabular}[c]{@{}l@{}}蔑 \\ \textit{miè}\end{tabular} & \begin{tabular}[c]{@{}l@{}}莫\\ \textit{mò}\end{tabular} & \begin{tabular}[c]{@{}l@{}}不\\ \textit{bù}\end{tabular} & \begin{tabular}[c]{@{}l@{}}無\\ \textit{wú}\end{tabular} & \begin{tabular}[c]{@{}l@{}}沒\\ \textit{méi}\end{tabular} & \begin{tabular}[c]{@{}l@{}}\textbf{Total no. of}\\ \textbf{{[}NEG+yǒu{]}}\\ \textbf{tokens}\end{tabular} \\ \midrule
