...
\ChapterDOI{10.5281/zenodo.7353613}
...
\shorttitlerunninghead{The negative existential cycle in Moksha Mordvin}


\abstract{Moksha (Mordvin, Uralic) has a complex negation system with several negative markers. I examine two of these markers: the negative existential \textit{ɑš} ({\textasciitilde} \textit{ɑjɑš}) and the past tense negative auxiliary \textit{ɑš}-. This auxiliary is generally assumed to have developed when the negative existential \textit{ɑš} acquired the additional function of an auxiliary. This study demonstrates that the negative existential cycle provides a framework to understand the development of the negative existential into a negative auxiliary; the negative existential entered the verbal domain as the short answer `no' and was fused with the older negative auxiliary \textit{iź}-, which continues to be used as an alternative to \textit{ɑš}-. This study is based on the analysis of corpus data: First, to clarify their relationship in the contemporary language, the different functions of the negative existential are introduced. Second, the competing paradigms of the two negative auxiliaries \textit{ɑš}- and \textit{iź}- are investigated.

\keywords{Uralic, Moksha, negative existential, negative auxiliary, past tense}
}
...
\newlength{\colmood}\settowidth{\colmood}{{Desiderative}}
%\newlength{\colindic}\settowidth{\colindic}{{Indicative}}
\newlength{\coltense}\settowidth{\coltense}{{Second}}
...
&                  & auxiliary \textit{ťɑ} & \parbox[t]{4cm}{\gll \textit{ťɑ-z-ɑt} \textit{morɑ}\\
                                                        ...
                                                        \glt `may you not sing'}                     &     \parbox[t]{3cm}{\gll \textit{morɑ-z-ɑt}\\
                                                                                                                        sing-\textsc{opt-2sg}\\
                                                                                                                        \glt `may you sing'}\\
\multicolumn{2}{@{} l}{{Desiderative}}\\
\midrule
&                   & auxiliary \textit{ɑfəľ}-& \parbox[t]{4cm}{\gll \textit{ɑfəľə-ksəľəń} \textit{morɑ}\\
                                                                \textsc{neg-des.1sg} sing.\textsc{cng}\\
                                                                \glt `I didn't intend to sing'} & \parbox[t]{3cm}{\gll \textit{morɑ-ľəksəľəń}\\
                                                                                                                        sing-\textsc{des.1sg}\\
                                                                                                                        \glt `I intended to sing'}\\
\multicolumn{2}{@{} l}{{Conjunctive}}\\
\midrule
&                   & auxiliary \textit{ɑfəľ}-	&\parbox[t]{4cm}{\gll \textit{ɑfəľəń}\textit{morɑ}\\
                                                                    \textsc{neg.conj.1sg} sing.\textsc{cng}\\
                                                                    \glt `if I did not sing'}& \parbox[t]{3cm}{\gll \textit{morɑ-ľəń}\\
														 sing-\textsc{conj.1sg}\\
														 \glt `if I sang'}\\
\multicolumn{2}{@{} l}{{Conditional}}\\
\midrule
&                    &particle \textit{ɑf} & \parbox[t]{4cm}{\gll \textit{ɑf} \textit{morɑ-ńďäŕɑ-n}\\
                                                            \textsc{neg} sing-\textsc{cond-1sg}\\
							   \glt `if I do not sing'
							   }                                 & \parbox[t]{3cm}{\gll \textit{morɑ-ńďäŕɑ-n}\\									                        				sing-\textsc{cond-1sg}\\
							   							\glt `if I sing'}\\
\multicolumn{4}{@{} l}{{Conditional-conjunctive}}\\
\midrule
&                    & particle \textit{ɑf} & \parbox[t]{4cm}{\gll \textit{ɑf} \textit{morɑ-ńďäŕɑľəń}\\
                                                                \textsc{neg} sing-\textsc{cond.conj.1sg}\\
								\glt `if I hadn't sung'}      & \parbox[t]{3cm}{\gll \textit{morɑ-ńďäŕɑľəń}\\
...
\begin{table}
\small
\newlength{\colclausfunc}\settowidth{\colclausfunc}{{Existential}}
\settowidth{\colnegator}{{Negator}}
\settowidth{\colneggloss}{\textsc{1sg.gen}\hspace{1ex}\textit{ajaš}\hspace{1ex}con-\textsc{poss.1sg.sg}}
\newlength{\colnegex}\settowidth{\colnegex}{{Negator}\hspace{1ex}\textsc{1sg.gen}\hspace{1ex}\textit{ajaš}\hspace{1ex}son-\textsc{poss.1sg.sg}\hspace{1ex}}
...
&\textit{ɑf}       &\parbox[t]{5cm}{\gll (\textit{mon})	\textit{ɑf}	\textit{od-ɑn}\\
											...
											\glt `I am not young'}            &\parbox[t]{4cm}{\gll (\textit{mon})	\textit{od-ɑn}\\
                                                                                                                                            ...
                                                                                                                                            \glt `I am young'}\\
\multicolumn{4}{l}{{Existential}}\\
\midrule
&a) \textit{ɑš} & \parbox[t]{5cm}{\gll \textit{pɑkśɑ-sɑ}	\textit{ɑš}	\textit{trɑktər}\\
                                        field-\textsc{ine}	\textsc{neg}	tractor\\
                                \glt  `there is no tractor in the field' }               & \multirow{2}{*}{\parbox[t]{5cm}{~\newline~\newline \gll \textit{pɑkśɑ-sɑ}  \textit{uľ-i}	\textit{trɑktər}\\
...
&a) \textit{ɑš} &\parbox[t]{5cm}{\gll \textit{moń} \textit{ɑš} \textit{ćora-źä}\\
                                ...
                                \glt `I don't have a son'} &\multirow{2}{*}{\parbox[t]{5.2cm}{~\newline~\newline \gll \textit{moń} \textit{uľ-i} \textit{ćora-źä}\\
                                                                                                                        \textsc{1sg.gen} be-\textsc{prs.3sg} son-\textsc{poss.1sg.sg}\\
                                                                                                                \glt `I have a son'}}\\
\tablevspace
&b) \textit{ɑjɑš}&\parbox[t]{5cm}{\gll \textit{moń} \textit{ɑjɑš} \textit{ćora-źä}\\
                                        \textsc{1sg.gen} \textsc{neg} son-\textsc{poss.1sg.sg}\\
                                \glt `I don't have a son'}                                      \\
\multicolumn{4}{l}{{Locative}}\\
\midrule
&a) \textit{ɑf}	&\parbox[t]{5cm}{\gll (\textit{mon}) \textit{ɑf}	\textit{pɑkśɑ-s-ɑn}\\
                                        \textsc{1sg} \textsc{neg} field-\textsc{ine-prs.1sg}\\
                                \glt `I am not in the field'}  & \multirow{2}{*}{\parbox[t]{5cm}{~\newline~\newline \gll (\textit{mon}) \textit{pɑkśɑ-s-ɑn}\\
                                                                                                                        \textsc{1sg} field-\textsc{ine-prs.1sg}\\
                                                                                                                \glt  `I am in the field'}
                                                                                                                }\\
\tablevspace
...
\begin{table}
\small
\settowidth{\colclausfunc}{{Existential}}
\settowidth{\colnegator}{{Negator}}
\settowidth{\colneggloss}{(\textit{mon})\hspace{1ex}\textsc{neg}\hspace{1ex}field-\textsc{ine-pst.1sg}}
\settowidth{\colnegex}{{Negator}\hspace{1ex}(\textit{mon})\hspace{1ex}\textsc{neg}\hspace{1ex}field-\textsc{ine-pst.1sg}\hspace{1em}}
\settowidth{\colaffgloss}{{Corresponding affirmative clause}}
...
&a) \textit{ɑf} 	&\parbox[t]{5cm}{\gll (\textit{mon}) \textit{ɑf} \textit{pɑkśɑ-sə-ľəń}\\
                                        ...
                                        \glt `I was not in the field'} &\multirow{2}{*}{\parbox[t]{5cm}{~\newline~\newline \gll (\textit{mon})	\textit{pɑkśɑ-sə-ľəń}\\
                                                                                                        \textsc{1sg}		field-\textsc{ine-pst.1sg}\\
                                                                                                    \glt `I was in the field'
                                                                                                    }}\\
\tablevspace
...
\midrule
\multicolumn{4}{l}{{1. Clausal function}}\\
\midrule
...
\tablevspace
\multicolumn{4}{l}{{3. Subject encoding}}\\
\midrule
...
\tablevspace
\multicolumn{4}{l}{{4. Objective conjugation}}\\
\midrule
...
\label{tab:2:9}
\fittable{%
\begin{tabular}{l lrlr  lrlr    lrlr }
\lsptoprule
O →			&\multicolumn{4}{@{}c}{{\textsc{1sg}}}				&\multicolumn{4}{@{}c}{{\textsc{2sg}}}						&\multicolumn{4}{@{}c}{{\textsc{3sg}}}\\
\cmidrule(r){2-5}\cmidrule(r){6-9}\cmidrule{10-13}
S ↓			&{\textit{ɑš}-}	&		&{\textit{iź}-}&	&{\textit{ɑš}-}		&		&{\textit{iź}-}		&	&{\textit{ɑš}-}	&	&{\textit{iź}-}&\\
\midrule
\textsc{1sg}	&-- 						&		&-- 					&	&\textit{ɑši\textsc{j}ťəń}	&1		&\textit{iźi\textsc{j}ťəń}&0 &\textit{ɑšińä}			&74	&\textit{iźińä}	&5\\
\textsc{2sg}	&\textit{ɑšəmɑjť}		&3		&\textit{iźəmɑjť}	&0	&-- 							&		&-- 						&	&\textit{ɑšiť}			&35	&\textit{iźiť}		&0\\
\textsc{3sg}	&\textit{ɑšəmɑń}		&8		&\textit{iźəmɑń}	&1 &\textit{ɑšəńźä}			&0 		&\textit{iźəńźä}		&0	&\textit{ɑšəźä}	&242	&\textit{iźəźä}	&11\\
                &						&		&					&	&							&		&						&	&\textasciitilde{} \textit{ɑšəź}	&4\\
\textsc{1pl}	&-- 						&		&-- 					&	&\textit{ɑšəďäź}			&1		&\textit{iźəďäź}		&0	&\textit{ɑš əśk}	&25	&\textit{iźəśk}	&3\\
\textsc{2pl}	&\textit{ɑšəmaśť}		&1		&\textit{iźəmaśť}	&0	&-- 							&		&-- 						&	&\textit{ɑšəśť}	&5		&\textit{iźəśť}	&0\\
\textsc{3pl}	&\textit{ɑšəmaź}		&6		&\textit{iźəmaź}	&1	&\textit{ɑšəďä\'ź}			&1		&\textit{iźəďäź}		&0	&\textit{ɑšəź}	&66	&\textit{iźəź}	&4\\
\lspbottomrule
\end{tabular}}
\end{table}
...
\begin{tabular}{l lrlr  lrlr    lrlr }
\lsptoprule
O →			&\multicolumn{4}{@{}c}{{\textsc{1pl}}}					&\multicolumn{4}{@{}c}{{\textsc{2pl}}}							&\multicolumn{4}{@{}c}{{\textsc{3pl}}}\\\cmidrule(lr){2-5}\cmidrule(lr){6-9}\cmidrule(lr){10-13}
S ↓			&{\textit{ɑš}-}	&		&{\textit{iź}-}	&	&{\textit{ɑš}-}		&		&{\textit{iź}-}		&	&{\textit{ɑš}-}	&	&{\textit{iź}-}&\\
\midrule
\textsc{1sg} 	&-- 						&		&-- 					&	&\textit{ɑšəďäź}			&0		&\textit{iźəďäź}		&0	&\textit{ɑšińä}		&8	&\textit{iźińä}	&1\\
\textsc{2sg}	&\textit{ɑšəmaśť}		&1		&\textit{iźəmaśť}	&0	&-- 							&		&-- 						&	&\textit{ɑšiť}			&0	&\textit{iźiť}	&0\\
\textsc{3sg}	&\textit{ɑšəmaź}		&1		&\textit{iźəmaź}	&1	&\textit{ɑšəďäź}			&1		&\textit{iźəďäź}		&0	&\textit{ɑšəźəń}		&33	&\textit{iźəźəń}&2\\
\textsc{1pl}	&-- 						&		&-- 					&	&\textit{ɑšəďäź}			&0		&\textit{iźəďäź}		&0	&\textit{ɑšəśk}		&2	&\textit{iźəśk}	&0\\
\textsc{2pl}	&\textit{ɑšəmaśť}		&0		&\textit{iźəmaśť}	&0	&-- 							&		&-- 						& 	&\textit{ɑšəśť}    		&4	&\textit{iźəśť}	&0\\
\textsc{3pl}	&\textit{ɑšəmaź}			&0		&\textit{iźəmaź}	&1	&\textit{ɑšəďäź}			&1		&\textit{iźəďäź}		&0	&\textit{ɑšəź}			&40	&\textit{iźəź}	&0\\
\lspbottomrule
\end{tabular}}
\end{table}
