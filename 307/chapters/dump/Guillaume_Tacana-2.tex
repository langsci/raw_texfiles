...
\markuptitle{Negation in Tacana (Amazonian Bolivia): synchronic description and diachronic reconstruction}{Negation in Tacana (Amazonian Bolivia): synchronic description and diachronic reconstruction}
...
\todo{underline is usually not allowed, but should be ok here}%Ljuba: Antoine is asking for the underline to stay
...
\todo{boldface is not allowed in running text, only for highlighting in examples. Ljuba: Antoine is fine with removing the bold face}
...
two subtypes (\sectref{sec:tacana-8}).\footnote{Essentially, two further
...
\end{exe} \todo{Antoine wants take_out with an underscore between take and out}
...
\textbf{=\textit{mawe}} [maße] \todo{Antoine says that ß should be ß̞  This is valid for all occurrences of maße or aimaße or ajmaße}
...
seen in \REF{ex:tacana-inhuman}, the predicate-preposed negation marker can show up in a longer (more conservative) form \textbf{\textit{aimawe}} [ajmaße]. \todo {ß should be ß̞} As for =\textit{mawe}, I have too few examples of SN applied to non-verbal clauses to know if it displays the shorter variant =\textit{mue} found in SN applied to verbal clauses.
...
-\textit{iti-a} \todo {Antoine requests that this complex past tense marker be kept on the same line} `\textsc{tdm-pst'}. A locative oblique (or, for that
...
\ex\label{ex:tacana-posture-sit-anoppi}
 \gll \overnote{S}{\ob}Piada deja{\cb} \overnote{V}\textbf{ani-(i)na}
    \overnote{com}{\ob}mesa ewane=sa kwara=neje{\cb}.\\
...
{\textasciitilde} [mwe]). \todo {ß should be ß̞}
...
\textbf{\textit{aimawe}} [ajmaße] \todo {ß should be ß̞} or variant \textbf{\textit{aimue}}
...
\makebox[0pt][l]{\raisebox{\baselineskip}{O}} \todo {O appears to be in italics; it should be the same as V and AUX }{\ob}jida mesa e-wane{\cb} beu.\\
...
\hspace{\stretch{2}} ] & N\textbackslash A & 4 \\
& [\hspace{\stretch{1}} \textbf{\textit{mawe}} \hspace{\stretch{2}} ] & N\textbackslash A
&\\\midrule
\underline{Non-clausal} &  &  & \\
...
\todo {the section below should start after the table, not before it}
%Antoine requests for this section to come after the table, not start before it as it does in the current compiled version. For this reason Ljuba inserted a newpage command.
...
\todo{not sure if shading in table is allowed, so we might need to change it. Antoine is asking for the shading to stay, if possible}
...
in \REF{ex:tacana-negexadj} (`there were no villages' \slash{} `villages were nonexistent') \footnote{\label{fn:tacana_crossling}According to \textcite[157]{Veselinova2016}, ``the use of
...
\todo{the footnote should be together with the paragraph that ends in 'villages were nonexistent'. It was placed at the beginning of the next paragraph. Ljuba would like to make sure that it is where it belongs now.}
...
\REF{ex:tacana-negexadj} (`there were no villages' / `villages were nonexistent'). As commented in footnote \ref{fn:tacana_crossli}, this evolutionary
...
no seeing of my brother-in-law by me'\slash 'the seeing of my brother in law is nonexistent to me' for \REF{ex:tacana-aimue-brother} and
`there is no fast going out for me'\slash 'the fast going out is nonexistent to me' for \REF{ex:tacana-aimue-fast}. An argument in favor of this hypothesis is that here, unlike
