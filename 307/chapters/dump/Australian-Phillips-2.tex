...
\shorttitlerunninghead{Semantic change in three Australian languages}
...
\textit{\textsc{\scriptsize(Wa}}\tikzmark{o}{\centering\scriptsize\textit{\textsc{t}}}\textit{\textsc{\scriptsize
    i)}}	& \textsc{negex\slash priv} & SN & {\small`cannot'/`not yet'}
    \\ \midrule
Wirangu [wgu] & \begin{tabular}[c]{@{}l@{}}\tikzmark{c}{\textit{\textcolor{teal}{-yudu}}}\\ \textcolor{violet}{\textit{-maga}}\tikzmark{d}\end{tabular}   & \tikzmark{b}\textcolor{violet}{\textit{-maga}}   & \textcolor{brown}{\textit{guda}}   \\ \midrule
Nauo [nwo] & ? & \textcolor{violet}{\textit{makka}} & \\\midrule
Bangarla [bjb] & \textcolor{violet}{\textit{-maga}}  & \textcolor{violet}{\textit{makka}}  & \tikzmark{a} \textcolor{brown}{\textit{kutta}}\\ \midrule
\begin{tabular}[c]{@{}r@{}}Adnyamathanha [adt]\\Kuyani  [gvy]\end{tabular} & \tikzmark{pari}{\textit{pari- }}  & \textcolor{brown}{\textit{(g)uda}}   & \tikzmark{e}--\tikzmark{ee}   \\\midrule
Nukunu [nnv] & \textcolor{violet}{\textit{-wakanha}} & & \\\midrule
\textit{proto-TY} &
...
%
\protectedex{\begin{exe}\ex\label{ex:austr-negstrat-wirangu} Examples of Wirangu negation strategies (from \citealt{Hercus1999})
    \begin{xlist}
\ex \textbf{\emph{maga} SN}\\
\gll Warlba marnaardu-nga \textbf{maga} wina-rn!\\
wind big\textsc{-loc} \textsc{\textbf{neg}} go\textsc{-pres}\\
\glt  `(I am) not going out in a gale!' \hfill (142)
%
\ex \textbf{\emph{-maga} privative}\\
\gll Nganha gidya-maga\\
1s child-\textsc{\textbf{priv}}\\
\glt `I haven't got any children.' \hfill (57)
%
\ex
\textbf{ \emph{-yudu} privative }(``most commonly used'')\\
\gll Nganha barnda-\textbf{yudu}\\
1s money-\textsc{\textbf{priv}}\\
\glt `I haven't got any money.' \hfill (57)
%
        \ex \textbf{\emph{guda} SN (modalised)}\\
\gll Ngadhu \textbf{guda} wangga-rn\\
1s.\textsc{erg} \textsc{\textbf{neg.}irr} speak\textsc{-pres}\\
...
\footnote{This remains to be demonstrated, but
    ...
    \parencites{Veselinova2013}{Gelderen2019}\todo{this should be Gelderen this volume}. An argument
...
Kaurna\il{Kaurna} [zku] (not represented in Figure \ref{fig:austr-thurayura-tree} for a lack of
...
\begin{table}[h]
	\centering
	\caption{Partitioning of the negative space in some Yolŋu languages.\\`\textsc{proh}' negates imperatives. `\textsc{priv}' is taken to denote a suffix of the type described above. `\textsc{negex}' (Wilkinson's \textsc{negq}) are independent words that appear to quantify over the NP which they precede.}
	\label{tab:austr-neg-comparison-yolngu}
	\begin{tabular}{lllll}
\lsptoprule
    & {\sc proh} & {\sc sn} & {\sc negex} & {\sc priv} \\\midrule
    Djambarrpuyŋu [djr] & \textit{yaka} & 
        \begin{tabular}[c]{@{}l@{}}\textit{yaka}\\ \textit{bäyŋu}\end{tabular}               
    & \textit{bäyŋu} & \textit{-miriw} \\\midrule
    Ritharrŋu [rit] & \textit{yaka} & -\textit{'may'} & \textit{yakaŋu} & \textit{-miriw} \\\midrule
    Wangurri [dhg] & 
        \begin{tabular}[c]{@{}l@{}}\textit{yaka}\\
            \textit{ŋangawul}\\ \textit{bayaŋu}\end{tabular} 
    & \begin{tabular}[c]{@{}l@{}}?\textit{yaka}\\\textit{ŋangawul}\\
                ?\textit{bayaŋu}\end{tabular} &
        \begin{tabular}[c]{@{}l@{}}\textit{ŋangawul}\\
        \textit{\textit{bayaŋu}}\end{tabular} 
    & \textit{-nharra} \\\lspbottomrule   
	\end{tabular}
\end{table}
...
Yolŋu dialect, appears to make use an additional particle with the
...
%
    ...
    \gll Kweye, the ng-enhe aw-\textbf{etye\textdblhyphen{akenhe}}\\
...
with reflexes in other Arandic varieties, attaches (see also
    ...
    \REF{ex:austr-arrernte-east-central}. Seizing on this argumentation, \citet[411-26]{Henderson2013} goes to some lengths to demonstrate the nominal status of verbal roots inflected with \textit{-etye\textdblhyphen akenhe}; some of these arguments are rehearsed here in view of better understanding the diachrony of Arrernte negation, although the reader is referred to his work for more evidence in favour of this analysis.
%
...
Data for related Arandic languages is sparse, it is therefore not possible
        ...
        Nevertheless, Katetye\il{Katetye}, the sole Arandic outlier
        ...
        Arandic that makes use of a similar strategy),\footnotemark is found at both ends of this subgroup, suggests a scenario in which
privative markers came to displace other strategies of standard negation
relatively early in its history.
\footnotetext{Note however that (some) Wati varieties (including
...
\protectedex{
\begin{exe}\exi{(i)}\begin{xlist} 
    \ex \textit{wiya} + nominalisation for sentential negation in
    Yangunytjatjara\il{Yangunytjatjara} [kdd]\\
\gll ngayulu kati-nytja wiya, Anti-lu kati-ngu\\
1s.\textsc{erg} take-\textsc{nmlzr} \textsc{neg} Andy-\textsc{erg} take-\textsc{pres}\\
\glt `I didn't take it. Andy took it.' \hfill \citep[244]{Goddard1983}
\ex \textit{wiya} + noun for negative existential in Yangunytjatjara\\
\gll mitjini wiya-ngka panya, iriti...\\
medicine \textsc{neg}-\textsc{loc} \textsc{ana} long~ago\\
\glt `(That was) in the old days, you know, when there was no medicine.'\\
\hfill \citep[39]{Goddard1983} \end{xlist}\end{exe}
}}\todo{example in footnote 22 should be foonotesize as well, need to solve that}
...
Examples are given for Djambarrpuyŋu in \REF{ex:austr-djamb-coda}, where the ``coda'' is underlined.\todo{Underline usually not accepted by lsp, but I think it's ok here}
...
\end{longtable}
%%%
%\printbibliography[heading=subbibliography,] \end{document} \begin{comment}
%%%
