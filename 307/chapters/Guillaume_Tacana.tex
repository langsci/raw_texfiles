...
\ChapterDOI{10.5281/zenodo.7353623}
...
\keywords{Takanan languages, negation, Jespersen Cycle, Negative Existential Cycle}
}
...
%{Antoine is asking for the underline to stay. Please let me know if this is possible or not.}
...
two subtypes (\sectref{sec:tacana-8}).\footnote{Essentially, two other
...
\ex\label{ex:tacana-let-go}
 \gll Beu  =pa \overnote{V}ja-mesia-ti-\textbf{\uline{(i)dha}}
...
\gll \textbf{Aimue} \overnote{O}ejije=kwana \overnote{A}yama \overnote{V}\textbf{\uline{e}}-shanapa-eni-\textbf{\uline{(i)nia}}=\textbf{mawe}.\\
...
\gll  Jade, \overnote{A}{\ob}ye=base=ja{\cb}  =mu  \textbf{aimue},  sai-da
 \overnote{V}\textbf{\uline{y}}-a-\textbf{\uline{ta-(a)ni}}=\textbf{mawe}.\\
...
\overnote{V}jadusuti-\textbf{\uline{(i)na}}=\textbf{mawe}.\\
...
seen in \REF{ex:tacana-inhuman}, the predicate-preposed negation marker can show up in a longer (more conservative) form \textbf{\textit{aimawe}} [ajma\d{ß}e]. As for =\textit{mawe}, I have too few examples of SN applied to non-verbal clauses to know if it displays the shorter variant =\textit{mue} found in SN applied to verbal clauses.
...
\ex\label{ex:tacana-posture-sit-anoppi}
 \gll \overnote{S}{\ob}Piada deja{\cb} \overnote{V}\textbf{ani-(i)na}
    \overnote{COM}{\ob}mesa e-wane=sa kwara=neje{\cb}.\\
...
\overnote{LOC}\textbf{ena=su} e-(ja-)id'i-ti-neti.\\
...
\moreovernote{V}\overnote{{\ob}tu\textbf{ʔ}twa{\cb}}\textbf{tutua} =da \moreovernote{A}etse \moreovernote{AUX}\textbf{\uline{y}-a.}\\
...
\gll  Beu, \moreovernote{V}\overnote{{\ob}ha\textbf{ʔ}mesjati{\cb}}\textbf{ja-mesia-ti} \moreovernote{S}ema \moreovernote{AUX}\textbf{pu-\uline{ana}}.\\
...
\fnref{fn:tacana-phonemes}) and apparently a different stress
pattern.\footnote{See \fnref{fn:tacana-phonemes} on the stress system in Tacana.}
...
(\ref{ex:tacana-mue-realize-answer}b--c).\footnote{As already mentioned in  \fnref{fn:tacana-phonemes}, the rule in Tacanan is that
...
\gll Corregidor=ja  =mi,  e-kisaba-me-ta-(a)ni  apa  =mi
 acompaña a-kwa,  misha,  Semana\_Santa  misha=su, awa  \textbf{mawe}?\\
    ...
    \textsc{ipfv}-ask-\textsc{caus}-3A\textsc{-ipfv(.sitting)}  if  =\textsc{2sg}
    accompany do\textsc{-pot}  church\_service  Holy\_Week
    church\_service=\textsc{loc} \textsc{q}  no\\
\glt `El corregidor te hace preguntar si puedes acompañarles a la misa de
...
no \textsc{neg} \textsc{=prt} \textsc{1sg} \textsc{fut}-go=\textsc{neg}\\
\glt   `No, no voy a ir.' su028\\
 `\uline{No}! I wo\uline{n't} go!'}
...
\gll \textbf{Mawe} tiyu! Be =tsu ema dia-ji!\\
    ...
    eat-\textsc{imp.neg}\\
\glt  `No tío, no me comas todavía!' bu029\\
...
\section{Summary of negation constructions}\label{sec:tacana-9}% \todo{Antoine is asking to  make sure there is a space between '...' and either the preceding or following material throughout the table. Ljuba perused table and thinks that there are now spaces where Antoine wants them. It would be good to double-check this in the compiled version.}

The negation constructions described above are summarized in \tabref{tab:tacana-summary} on the next page. For practical reasons, in the schematized constructions I provide the most commonly attested variants of the negation markers, \textit{aimue} and =\textit{mawe}.
...
\hspace{\stretch{2}} ] & N$\slash$A & 4 \\
& [\hspace{\stretch{1}} \textbf{\textit{mawe}} \hspace{\stretch{2}} ] & N$\slash$A
&\\
\midrule
\tablevspace
\uline{Non-clausal} &  &  & \\
...
\gll Enekita  beu  =pa  \textbf{ai}=kwana  ja-ba-ti-ana.\\
    really  \textsc{prt}  \textsc{=rprt}  thing=\textsc{pl}
...
in \REF{ex:tacana-negexadj} (`there were no villages' \slash{} `'villages were nonexistent').\footnote{\label{fn:tacana-nominalized}According to \textcite[157]{Veselinova2016}, ``the use of
...
\REF{ex:tacana-old-catecism}, and which look basically similar to our synchronic construction […  \textbf{\textit{mué=}}  …  V  …  (be/do-\textsc{infl}) …].
...
\REF{ex:tacana-negexadj} (`there were no villages' / `villages were nonexistent'). As commented in \fnref{fn:tacana-nominalized}, this evolutionary
...
existential\slash locative predicate. These are translatable literally as `'the seeing of my brother-in-law is nonexistent to me' for \REF{ex:tacana-aimue-brother} and
`the fast going out is nonexistent to me' for \REF{ex:tacana-aimue-fast}. An argument in favor of this hypothesis is that here, unlike
