...
\ea \label{ex:2:15}
\gll [m\'{ɔ}-kɔɬ-ɔkʷ na] {\hspace{5pt}} /ma-  ka ɬ  -ak\textsuperscript{o} na/\\
...
The labio-dental flap /\dentalflap  / was first described by \citet{OlsonHajek2004} and is typical of many of the Chadic languages in the Far North Province of Cameroon. In Moloko it is found in ideophones (\ref{ex:2:22}--\ref{ex:2:23}, see \sectref{sec:3.6}). 
...
\begin{table}
\resizebox{\textwidth}{!}{%\begin{sideways}
...
\lsptoprule
/p/ &	p  & [p] &     voiceless bilabial unaspirated stop & /nz/&   nj  & [nz] &     pre-nasalised voiced alveolar nasal occurring in unpalatalised syllables\\
/b/ &    b & [b] &     voiced biliabial stop &     &       & [nʒ] &     pre-nasalised voiced alveopalatal nasal occurring in palatalised syllables\\
/mb/&   mb & [mb]&     pre-nasalised voiced bilabial stop        & /ɬ/ &   sl  & [ɬ] &     voiceless alveolar lateral fricative\\
...
/f/ &   f  & [f] &     voiceless labio-dental fricative & /r/ &   r  & [r] &     voiced alveolar flap\\
/v/ &   v  & [v] &     voiced labio-dental fricative & /\dentalflap / &   vb & [\dentalflap ] &     voiced labiodentals flap\\
...
/n/ &   n  & [n] &     voiced alveolar nasal & 	&      & [kʷ] &     voiceless labialised velar stop occurring in labialised words\\
    &      & [ŋ] &     voiced velar nasal occurring word-finally & /kʷ/&  kw/wk\footnote{Orthographically, ‘kw’ is word-initial and word medial, ‘wk’ is word-final.}&[kʷ] &   voiceless labialised velar stop          \\
/nd/&   nd & [nd]&     pre-nasalised voiced alveolar stop        & /g/ &   g  & [g]  &  voiced velar stop occurring in unlabialised syllables\\
...
\largerpage
Bow found there are several cases in the data where it was impossible to tell whether the consonant is underlyingly labialised or there is a labialisation prosody across the word, as in \REF{ex:2:26} and \REF{ex:2:27} (from \citealt{Bow1997c}).
...
Further work on verb conjugations clarified that \REF{ex:2:26} actually contains a labialised velar (i.e., the underlying form is /s kʷ m/). \REF{ex:2:28} shows the nominalised form of the verb which is palatalised, yet the labialised velar is still present. If there was no underlyingly labialised velar, the nominalised form would have been *[mɪsɪkɪmɛ]
...
Labialisation affects the back consonants such that [k, g, ŋg, h] are in complementary distribution with [kʷ, gʷ, ŋgʷ, hʷ], respectively, with the second group only appearing in labialised words. Note however that there is a set of underlyingly labialised back consonant phonemes (see \sectref{sec:2.2.2}).
...
\citealt{Bow1997c} demonstrates that [n] and [ŋ] are allophones of /n/ with a distribution as shown in \figref{fig:2.2}.

\begin{figure}\caption{Word final allophone of /n/\label{fig:2.2}}
n → ŋ / \_ \#
\end{figure}

\tabref{tab:2.8} (adapted from \citealt{Bow1997c}) illustrates [n]{ }and [ŋ] in complementary distribution (with [n]{ }word initial and word medial and [ŋ] word final).
...
{Neutral} & [nax] & ‘ripen’ & [gənaw] & ‘animal’ & [=ahaŋ] & =\SSS.{\POSS}\\
{Labialised} & [nɔkʷ] & ‘you’ & [ana] & ‘to’ (dative) & [gəlaŋ] & ‘threshing area’\\
{Palatalised} & [nɛ] & ‘me' & [mɪtɛnɛŋ] & ‘bottom’ & [ɓərɮaŋ] & ‘mountain’\\
\lspbottomrule
\end{tabular}}
\caption{Complementary distribution for /n/\label{tab:2.8}}
\end{table}

Likewise, Bow demonstrates that [h] and [x] are allophones of /h/ with a distribution as shown in \figref{fig:2.3}. 
...
\largerpage
\tabref{tab:2.9} shows [x] and [h] in complementary distribution (with [h]{ }word initial and word medial and [x] word final).
...
Likewise, the verb /wal/ ‘hurt’ exhibits similar changes, where the word-final [r] in \REF{ex:2:31} becomes /l/ when the indirect object pronominal enclitic attaches \REF{ex:2:32}. 
...
& & & & ‘tamarind’ & & & ‘milk’ \\
{6} & {Adjacent to an inherently}  & [œ] & \textbf{e} & [ʃɪlœkʷ] \textbf{səlewk} & [ø]  &  \textbf{ə} & [lʊkʷøjɛ]     \textbf{ləkwəye}\\
& {labio-velar and /j/} & & & ‘broom’ & &  & ‘you’ (Pl)\\
\lspbottomrule
\end{tabular}}
...
Bow reported that besides the prosodies of labialisation and palatalisation, the epenthetic vowel allophones are conditioned by the phonemes /j/ and /w/ as well as the underlyingly labialised consonants.  The rules governing these two conditioning environments follow, along with examples of each. Bow found that the epenthetic vowel assimilates to the palatal and labial features of an adjacent semi-vowel even when there is a prosody on the root.  \figref{fig:2.4} and \figref{fig:2.5} illustrate the rules for the influence of /j/\footnote{We found no cases of *[ji].} and /w/ with examples of each. 
...
Lexical tone itself is not marked in the orthography since there are only a few minimal pairs. The orthography does distinguish minimal pairs by a diacritic on one of the words in each pair. Lexical tone is not marked in examples in the morphosyntax part of this work. Imperfective and Perfective aspect\is{Tense, mood, and aspect!Perfective aspect} on verbs\is{Tense, mood, and aspect!Imperfective aspect} (indicated by grammatical tone) are distinguished by a diacritic on the subject pronominal verb prefix (see \sectref{sec:7.4}). 
...
\midrule
\multirow{2}{3cm}{{Across morpheme boundary}} & [ɬàlà] +[aháj] & [ɬàlàh\={a}j] & LL+H → LLM & ‘villages’\\
...
\largerpage Word breaks are determined in this work by the phonological criteria discussed in \sectref{sec:2.6.1} as well as using the grammatical criteria discussed in \sectref{sec:2.6.2}. Using these criteria, affixes, clitics, and extensions\footnote{Note that the term ‘extension’ for Chadic languages has a different use than for Bantu languages. In Chadic languages, ‘extension’ refers to particles or clitics in the verbal complex (\sectref{sec:7.5}).} can be distinguished from separate words in Moloko. Phonological criteria are illustrated for both nouns and verbs, when applicable (\sectref{sec:2.6.1}). ‘Affix,’ ‘clitic,’ and ‘extension’ are categorised for Moloko in \sectref{sec:2.6.2}.
...
Word-final changes where /n/ is realised as [ŋ] (\citealt{Bow1997c}) indicate a word break between the noun \textit{həlaŋ} ‘back’ and \textit{na} ‘\PSP' (\ref{ex:2:68}). \REF{ex:2:69} is more complicated. The initial consonant of the adverbiser [ŋa] (see \sectref{sec:3.5.2}) has assimilated to the final consonant of the noun, indicating that they are phonologically bound. However, the fact that the noun [dedeŋ] ‘truth’ exhibits  word-final changes indicates that [ŋa] cliticises after word final changes in the noun have occurred. 
...
\gll /n- {s \textsuperscript{e}} {-am \textsuperscript{o}/} \hspace{1.5pt} →  [nɔsɔm]\\
     \oldstylenums{1}- drink -\oldstylenums{1}\textsc{Pex} {} {} {‘We drink.’}\\
\z
...
The adverbiser /Ca/ (see \sectref{sec:3.5.2}) is an interesting clitic in the way it is phonologically bound to its noun. The noun displays word-final changes, which would normally indicate a word break. However, initial consonant of the adverbiser enclitic is a reduplication of the final consonant of the noun (\ref{ex:2:69} in \sectref{sec:2.6.1.2}) which indicates that the reduplication occurs after phonological word-final alterations are made to the noun. 
