...
main.snd: main.bbl
	...
	sed -i s/.*Access.*// main.adx  
	sed -i s/.*Blu-ray.*// main.adx  
	sed -i s/Hvelplund Jensen/Hvelplund/ main.adx  
	sed -i s/.*IMDb.*// main.adx  
	sed -i s/.*ITC.*// main.adx  
	sed -i s/.*Leuven.*// main.adx  
	sed -i s/.*Media.*// main.adx  
	sed -i s/.*NDA.*// main.adx  
	sed -i s/.*Project.*// main.adx  
	sed -i s/.*Screen.*// main.adx  
...
# 	echo git clone https://github.com/langsci/`cat ID`.git
	mkdir -p `cat ID`
# 	mv `cat ID` ..
	
	
paperhive: proofreading githubrepo	
	grep lsID localmetadata.tex |egrep -o "[0-9]*" > ID	
	...
	cp proofreading.pdf versions.json Makefile `cat ID`	
	git branch gh-pages
