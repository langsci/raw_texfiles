...
The lexical tone of a noun is determined using the copula and the possessive as diagnostic contexts, since their surface tone varies according to the tone category of the target word. 
The use of different elicitation contexts can be likened to the application of chemical agents in the development of photographic films: just as a~blend of chemicals is required to render a~latent image visible, several contexts need to be used to reveal underlying tone. Together, the copula and the possessive suffice to bring out all the categories of nouns. For nominal classifiers (\tabref{tab:TONECLREF}), nine tonal categories were identified by examining tonal patterns in association with numerals. For verbs (\tabref{tab:UtonesofverbsREF}), seven categories were arrived at by piecing together evidence from four contexts: in isolation, with a~preceding \textsc{negation} prefix /\ipa{mɤ˧-}/, with a~preceding \textsc{accomplished} prefix /\ipa{le˧-}/, and in association with the object ‘a~bit’ (/\ipa{ɖɯ˧-kʰwɤ˥\$}/).
...
%\vspace{15mm}
	...
	\begin{table}
		...
		\begin{tabularx}{\textwidth}{ P{21mm} l Q Q P{19mm} Q }
			...
			//LM+\#H// & L.H & L.M+H & L.M+M & \ipa{nɑ˩hĩ\#˥} & Naxi\\
...
\vspace{-0.4cm} % To avoid a large vertical space between title and table. The table is typeset as a float, hence a variable distance from the title, which is not desirable in view of the contents. (This is unlike the 'enumerate' environment used for the list of rules.)
...
\begin{table}[H]
	{\renewcommand{\arraystretch}{1.35}
		\begin{tabularx}{\textwidth}{ @{}l Q } 
			...
			\#H & Floating High tone: this H gets anchored \textit{after} the word boundary, i.e.\ on a~following morpheme, if one is available and can serve as a~host. In some contexts, the floating H tone does not surface (for want of syllabic anchoring) but instead lowers following tones to L.\\
			...
			H\$ & A~type of H tone (analyzed in \sectref{sec:wordfinalandmorphologicalnucleusfinalHtones}) exemplified by the noun ‘flea’ and  nicknamed the ‘flea’ tone or ‘hopping’ tone. When a~word carrying this tone is pronounced in isolation, the H tone associates to its last syllable: /\ipa{kv̩˧ʂe˥}/ ‘flea’ surfaces with an M.H tone sequence. When followed by the copula, the H associates with the copula: /\ipa{kv̩˧ʂe˧ ɲi˥}/ ‘is \mbox{(a/the)} flea’. Finally, when the noun is followed by the possessive, no H tone reaches the phonological surface: the observed form is /\ipa{kv̩˧ʂe˧=bv̩˧}/ ‘of \mbox{(a/the)} flea’, with M tone throughout.\\
			-- & In the representation of tones in expressions containing two or more morphemes (e.g.\ compound nouns), this symbol marks the morpheme boundary. Thus, \textit{--L} refers to an~L tone that attaches after the morpheme break, i.e., in the case of a~compound noun, to the head noun. \textit{LM--L} indicates that the morpheme before the break (i.e., in a~compound, the determiner) receives LM tone, while the noun following the break receives L tone.\\
		\end{tabularx}}
	\end{table}
